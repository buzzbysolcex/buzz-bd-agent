...
# v9.5+ Almanax HIGH: credentials written below are 0600 by default
umask 077

# ─── Environment ───
# v8.2.1: Source persistent env overrides (Copilot PAT, etc.)
# v9.5+ Almanax HIGH: replace xargs-based export with ownership-checked sourcing
if [ -f /data/.env.copilot ]; then
  OWNER=$(stat -c '%U' /data/.env.copilot)
  PERMS=$(stat -c '%a' /data/.env.copilot)
  if [ "$OWNER" != "root" ] || [ "$PERMS" != "600" ]; then
    echo "[boot] ✗ /data/.env.copilot has unsafe ownership/perms ($OWNER:$PERMS). Skipping."
  else
    set -a
    . /data/.env.copilot
    set +a
    echo "[boot] ✓ Copilot env loaded (safe mode)"
  fi
fi
...
MOLTEOF
  chmod 600 /data/.openclaw/credentials/moltbook.json
...
MOLTENEOF
  chmod 600 /data/.openclaw/credentials/molten.json
...
if [ -n "$GMAIL_CLIENT_ID" ] && [ -n "$GMAIL_CLIENT_SECRET" ] && [ -n "$GMAIL_REFRESH_TOKEN" ]; then
    ...
    chmod 600 /data/.openclaw/credentials/gmail.json
...
# Verify all credentials already written by earlier blocks (4a/4b/4c/9)
# BEFORE the gateway starts, so crons never see missing creds.
# v9.5+ Almanax HIGH: /data/credential-gen.sh execution removed —
# writable-path RCE vector. Credentials are baked in Docker image
# or generated by the earlier Block 4 sequence.
# ══════════════════════════════════════════════════
echo "[boot] Block 17: Pre-gateway credential verification"
export BANKR_AGENT_API_KEY=${BANKR_PARTNER_KEY}
WAIT=0
CREDS_READY=0
while [ $WAIT -lt 30 ]; do
