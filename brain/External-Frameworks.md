...
## NVIDIA SkillSpector — methodology mirror (intake 2026-05-30, Ogie SkillSpector directive)

**What it is:** NVIDIA's Apache-2.0 security scanner for AI-agent skills (github.com/NVIDIA/SkillSpector). **NOT a bounty target** (NVIDIA's own tool, no bounty). Two uses: (1) defensive self-audit of our own skills; (2) methodology mirror. Architecture = **structurally our own detector pipeline pointed at skills instead of contracts**: 2-stage (static high-recall → optional LLM precision filter ~87%), 64 patterns / 16 categories (prompt-injection, data-exfil, priv-esc, supply-chain, excessive-agency, output-handling, system-prompt-leak, memory-poisoning, tool-misuse, rogue-agent, trigger-abuse, dangerous-code-AST, taint-tracking, YARA, MCP-least-privilege, MCP-tool-poisoning), 0-100 risk score.

**Self-audit result (2026-05-30, `--no-llm` static-only, secrets-local, scan output kept out-of-repo at `/home/claude-code/skillspector-audit/`):** 20 skills scanned (`.claude/skills/*` + `/mnt/skills/user/aibtc-agent-intel`). Scores 0–50, all LOW/MEDIUM, **zero externally-exploitable vulns; zero secret VALUES leaked; no internal IP**. Every fired finding = **FP-by-design** (exactly the predicted classes): E1 data-exfil = documented public APIs (cdp.coinbase / heyanon / our own buzzbd.ai), PE2/PE3 = `chmod 600` HARDENING + `.env` handling instructions (opposite of a vuln), MP1 memory-poisoning = our "permanent rule/directive" header phrasing, EA2/RA2 = documented auto-approve-behind-Ogie-gate, YR1 CRITICAL (aibtc-genesis) = YARA on the prose "send 100 sats sBTC" (AIBTC micropayment docs), SC2/TM2 = official `sh.rustup.rs` install. **Bankr-published flagship `buzz-bd-agent` = 30/MEDIUM**, all 3 findings FP. **ONE real info-hygiene item ([REVIEW], LOW):** the published flagship (+ `alpha-buzz-master-ops`) ENUMERATE internal secret-file paths and which creds they hold (`/data/buzz/persistent/env/.env.cdp`→CDP_API_KEY_ID+SECRET, `.env.pashov`, `.env.buzz`) — no values, but a public marketplace skill shouldn't ship an attacker-map of the secret layout. Recommend scrubbing `/data/...` paths + env-layout tables from the PUBLIC plugin copy (verify what `buzz-crypto-bd` actually bundles). Operator-discretion.

**Methodology borrow into brain (compound, not code-copy):**

1. **Taint source→sink taxonomy as an on-chain lens.** SkillSpector `behavioral_taint_tracking` (SADD B.2.2) classifies by (source-sensitivity × sink-reachability), most-specific wins: TT1 HIGH (direct), TT2 MED (indirect), **TT3/TT5 CRITICAL** (credential/secret source → network/exfil sink), TT4 HIGH. **Mirror for contracts:** enumerate reachable (untrusted-source → privileged-sink) pairs — tainted external input (user calldata / oracle return / cross-chain message payload / bridge attestation) → privileged sink (mint / transfer / upgrade / delegatecall / admin-state-write / quorum-mutation). DIRECT source→sink = HIGH; a path through an UNVALIDATED intermediate (the validation gap) = the bug. Cross-ref **DC-7** (Validating-Field ≠ Consuming-Field on adjacent pipelines), **Detector #166** (cache-before-validate), standing-intake 5-target "External Calls" class.

2. **TP4 Description-Behavior Mismatch as a reusable lens.** A skill/tool/contract whose advertised DESCRIPTION (name, interface, docs, metadata, MCP tool-schema) diverges from its actual BEHAVIOR (execution). The MCP "tool-poisoning / rug-update" vector, generalized. **On-chain analogues:** proxy whose interface implies X but `delegatecall`s to Y; a token whose `transfer` carries hidden side-effects; selector/name vs body divergence; metadata-vs-execution drift on upgrade. **RHYMES with Doctrine #44** (identity-binding-gap: the dedup-id / merkle-leaf / message-id is computed over a STRICT SUBSET of the content the consumer acts on) + the lookalike-domain instinct. **Unifying primitive: what the consumer TRUSTS (the description / the bound id) covers LESS than what actually EXECUTES / is consumed (the behavior / the full content) → the uncovered delta is the attack surface.** Run as a paired lens with Doctrine #44 on any target with a description/metadata/id layer separate from its execution/content layer.

### SkillSpector Status

INTAKE filed 2026-05-30. Self-audit = clean (FP-only + 1 LOW info-hygiene REVIEW). Methodology compounds (taint source→sink lens + TP4 description-behavior-mismatch) banked as Gate-1 lenses, cross-ref Doctrine #44 + DC-7 + Detector #166. SkillSpector clone + scan reports retained out-of-repo (`/home/claude-code/skillspector-audit/`, never committed — secrets-local hold). Authority: Ogie SkillSpector directive (single-agent, secrets-stay-local).

### SkillSpector Cross-references

- `brain/Doctrine.md` Doctrine #44 — identity-binding-gap (paired lens with TP4)
- `brain/Patterns-Defense-Classes.md` DC-7 + Detector Seed #166 (taint source→sink convergence)
- self-audit scan reports: `/home/claude-code/skillspector-audit/reports/*.json` (out-of-repo, secrets-local)

---

_brain/External-Frameworks.md | v1.6 | 2026-05-30 (Ogie SkillSpector directive — NVIDIA SkillSpector intake: defensive self-audit of 20 skills = zero real vulns / all FP-by-design + 1 LOW info-hygiene REVIEW (published flagship leaks secret-file-path layout, no values); Bankr flagship buzz-bd-agent = 30/MEDIUM all-FP. Methodology mirror: taint source→sink taxonomy (TT1-5) as on-chain untrusted-source→privileged-sink lens + TP4 Description-Behavior Mismatch lens, both cross-ref'd Doctrine #44. Single-agent, secrets-local, scan output out-of-repo.)_
