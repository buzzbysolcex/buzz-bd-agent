...
**#2 Symbiotic = STEP-4 FORECLOSE-RECOMMEND (2026-05-29, post-clone Phase-0 dedup).** Inventory found **6 audit firms incl. Certora formal-verification on 6,574 LOC + HEAD 2025-10-30 (~7mo stale)** → fails this queue's OWN Doctrine-#42-refined dispatch criterion (≤2 audits AND ≤1-3mo) on BOTH axes. Demote exactly as Resolv (14-audit) was. Realizable-EV recomputed ~$5K (not ~$21K). **Default = foreclose-pivot.** Operator-override angle only: cross-module `VetoSlasher↔Delegator↔Vault` economics (the one class formal-verif misses) — narrow-scope if greenlit. Clone KEPT (disk 67%). See `hunts/2026-05-29-symbiotic-immunefi-gate1.md`. **LESSON: the queue's ≤2-audit criterion must be checked at CLONE-INVENTORY (audit-dir count), not just at WebFetch PROFILE — Symbiotic's audit dir wasn't visible pre-clone.**

**NEXT ACTION = audit-LIGHT-fresh pivot: Usual / Falcon** (queue row 6 — fresh 2025 yield-stablecoins, likely 0-2 audits = high-p, $100-500K, HIGH PoC pure collateral/share-math). Verify audit-count + cap + KYC at Step-1 dispatch (the [EST] tags). Fallback if both audit-heavy: Morpho Vaults V2 (row 3, net-new curator/allocator) or fresh Solana restaking (row 8, DC-8). **HOLD: Hyp-C = Immunefi slot 1 (21:40Z, untouched, operator-gated submit); all Gate-1 work is parallel, NO submission conflict.**
