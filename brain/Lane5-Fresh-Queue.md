...
- **Granite Protocol** — ✅ **GATE-1 DONE = NEGATE [INSPECTED]** (2026-05-30, autonomous; `hunts/2026-05-30-granite-immunefi-gate1.md`). Full core read (18 contracts / 5,279 LOC, real deployer `SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE` — the queue's `…X8EF5B.granite-btc` was a **stx.city namesake meme token**, re-located via docs.granite.world + Hiro). No externally-exploitable in-scope candidate: tracked-var accounting (donation-inflation structurally foreclosed), liquidation bounded by 3 solvency guards (post-liq health buffer + `liqLTV<inverted-discount` + same-block-borrow guard), oracle staleness/confidence-gated (price=0 residuals OOS), governance multisig+timelock with deployer-gated init (no takeover). ONE real code defect — `utilization-calc` zero-divide DoS — but catastrophic-precondition-gated (bad-debt ≥ entire pool ≈ ~100% collateral crash = OOS oracle/market-risk) ⇒ **LOW realizable EV, NOT banked Critical** (Doctrine #43 honest-counts); operator-discretion low-sev/QA candidate only. **Win banked: 2nd CLARITY ANCHOR** → arsenal lenses promoted seed→detector + 2 new detector seeds (tracked-var-vs-raw-balance; #167 guard-wrong-quantity-divide) in `brain/Patterns-Defense-Classes.md`. Clone purge-eligible.
- **YO Protocol** — Immunefi, specifics unverified. **= autonomous NEXT unit** (Step-1 verify platform+funded-bounty+scope before clone; if platform unconfirmable → fresh-scout for next audit-light BTC/Stacks-finance target). Operator fork (msg 8041: USDtb/Euler/fresh-scout) still open for steer.
