...
| **DISC-021 Balancer V3 BatchRouterHooks slippage double-count** | Immunefi | **#80150** | **CRITICAL** | **2026-05-28 00:12Z** | **SUBMITTED — awaiting triage (CANDIDATE-O multi-anchor with Pancake P-1, $1M cap no-KYC, 7-rule AI-Report refactor applied, Foundry PoC HEAD `80fd29ce4eb6` 2-hop leak 1.09% vs 1-hop 0.55% on StableSurgeHook composition surface). **SELF-REEXAM 2026-05-29 (Doctrine #43, post-PancakeSwap #80247 INVALID): MISFRAME — REJECTION-BOUND.** Source-confirmed `BatchRouterHooks` clean output→input single-chain accounting + ONE aggregate floor (`path.minAmountOut`), NO literal double-debit; PoC output 4945 USDT > minAmountOut 4750 (aggregate satisfied); the "double" = 2-hop-vs-1-hop apples-to-oranges; no second consumer → fails the CANDIDATE-O genuine test (vs Rhea $18.4M swap→oracle). **RECOMMEND OPERATOR PROACTIVELY WITHDRAW with a good-faith note before triage rejection — operator-gated, NOT actioned.** Re-exam: `hunts/2026-05-29-balancer-b1-reexam.md`** |
| **DISC-022 PancakeSwap Infinity Router slippage double-count** | Immunefi | **#80247** | **HIGH** | **2026-05-29** | **CLOSED INVALID 2026-05-29 — PancakeSwap rebuttal ACCEPTED (no mediation).** Their point: the multi-hop loop's missing per-step floor is NOT theft — the user's end-to-end `amountOutMinimum` already bounds total loss (tighten 47.5→49.7 ether → PoC reverts). A true structural fact (no per-step floor) MISFRAMED as silent theft, with a 1-hop-vs-2-hop apples-to-oranges comparison. PRODUCTIVE closure: anchored **Doctrine #43 (Aggregate-Bound ≠ Per-Step Vulnerability)** + the CANDIDATE-O EXCLUSION + Standing-Intake Step 5.11 QC gate, and triggered the DISC-021 B-1 same-night self-reexam (also MISFRAME → recommend-withdraw). Wallet `0x46D6...8868` verified.** |
...
**Scope (operator-anchored msg 7956 2026-05-28):** DISC-022b is **FORWARD-LOOKING ONLY**. Do NOT add proactive operator-validation comments or AI-clause supplements to already-submitted reports DISC-021 #80150 or DISC-022 #80247 — these stand on technical merit, and retroactive defensive annotation draws unnecessary triager attention. The 5 binding requirements apply from 2026-05-29 onward to all NEW Immunefi submissions only.

**Cross-pillar impact:**
- DISC-021 + DISC-022 NOT flagged for retroactive supplement — they stand on technical merit per operator directive. Outcome correlation tracking continues per CANDIDATE-O observation below.
...
**CANDIDATE-O family observation — RESOLVED 2026-05-29: BOTH are MISFRAMES → the per-step-absence framing of CANDIDATE-O was a structural mis-read.** PancakeSwap #80247 CLOSED INVALID (rebuttal accepted); Balancer B-1 #80150 self-reexam → MISFRAME (recommend withdraw). CANDIDATE-O is **DEMOTED to an EXCLUSION** (Doctrine #43 + Patterns CANDIDATE-O EXCLUSION): genuine ONLY when a value feeds a SECOND, aggregate-UNprotected consumer (a different victim — e.g. Rhea Finance $18.4M swap→oracle, flash-loan self-trade), NOT when it's a missing per-step floor on a single-victim aggregate-bounded multi-hop swap. The dismissal vector (aggregate-bound) is now codified as Gate-1 QC gate Step 5.11. Lesson > the two non-rewards: it sharpened the whole DEX-composition class + validated tonight's foreclose-cheap rigor.
