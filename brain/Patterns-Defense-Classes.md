...
**Status:** SUBSTRATE-ARSENAL (Clarity arsenal v1.0, **2 anchors = Hermetica NEGATE + Granite NEGATE** → lenses PROMOTED from seed to active detectors per the 2-anchor rule). DC-3-Clarity, CANDIDATE-I-Clarity (+ tracked-var sub-check), oracle-gating, as-contract, map-insert-replay = active Clarity detector lenses. Clarinet still uninstalled (no Clarity CONFIRM yet to PoC). Authority: `hunts/2026-05-30-hermetica-immunefi-gate1.md` + `hunts/2026-05-30-granite-immunefi-gate1.md` (Ogie msg 8042 autonomous loop; Granite = operator-scoped 2nd Clarity anchor).

---

## Detector Seed #167 — Guard-Wrong-Quantity Divide (added 2026-05-30 — Granite `utilization-calc` anchor)

**Class:** a divide is guarded against zero by checking a DIFFERENT quantity than the divisor — typically a SUM or an OR of operands — so the guard passes while the actual denominator is 0 → division-by-zero runtime abort (Clarity panic / EVM revert). Often a DoS: if the divide sits on a hot path that gates other functions (interest accrual, price, health), a 0-divisor bricks every dependent entry point.

**Anchor ([INSPECTED], low-EV/catastrophic-precondition):** Granite `linear-kinked-ir-v1.utilization-calc` — `(if (> (+ total-assets open-interest) u0) (/ (* open-interest one-12) total-assets) u0)` guards the SUM but divides by `total-assets`. `total-assets==0` while `open-interest>0` → panic in `accrue-interest` → all of borrow/repay/withdraw/deposit/liquidate/stake brick (they all call accrue-interest). Reachable only via bad-debt socialization ≥ entire pool (system-wide insolvency) → NOT banked as Critical, but the PATTERN is the keeper.

**Signature to grep:** any `(/ X Y)` / `X / Y` whose nearest zero-guard tests `(A + B)`, `(or A B)`, or a sibling variable rather than `Y` itself. Correct guard is on the divisor. Cross-substrate (Clarity `/` aborts on 0; Solidity `/` reverts; Rust `/` panics) → universally a DoS-or-worse seed.
