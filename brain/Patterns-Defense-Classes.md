...
- DC-12 sub-7 (wrapper-strips-staleness-from-feed) — productizable detector spec: (a) grep for solidity interfaces returning only `(uint256)` when sibling protocols use Chainlink-style multi-return — sub-7a flavor; (b) grep for `.latestAnswer()` consumers WITHOUT a sibling `.latestRoundData()` call — sub-7b flavor; (c) grep for hardcoded `updatedAt = 0` / `updatedAt = block.timestamp` constants in oracle adapter implementations — sub-7c flavor. Cross-pollination targets (next sweep): Compound V3, Spark, Inverse Finance, Notional V3, Silo, Sturdy, Frax Lend — all Chainlink-consumer protocols without Aave-tier audit-saturation foreclosure. Productization value HIGH: every wrapper-class Chainlink-consumer protocol may surface sub-7b. Filed per Ogie msg 7741.
