...
**DC-8 NEGATING-EXAMPLES (properly-designed Anchor programs where signer/authority validation lives in the `Accounts<'info>` struct — the OPPOSITE of the DC-8 defect; catalog sharpens the class boundary):**

1. **Squads V4** (reference, per cross-protocol enumeration) — `Signer<'info>` constraint pinned via `#[account(signer)]` / typed account refs at the struct level; handler body is post-auth mechanical work.
2. **Wormhole NTT** `solana/programs/example-native-token-transfers/src/instructions/redeem.rs` @ `4a15527c` (2026-05-26) — Gate 1 over-generously classified this as a "DC-8 partial hit" via WebFetch; Gate 2 commit-pinned source-read NEGATES (`hunts/2026-05-29-wormhole-ntt-hyp-e-gate2-foreclosure.md`). Defense is entirely in the `Accounts<'info>` struct: (a) `Account<'info, RegisteredTransceiver>` typed PDA + owner-discriminator (cannot forge a transceiver); (b) `owner = transceiver.transceiver_address` on `transceiver_message` (only the real transceiver program can have written it); (c) `constraint = config.enabled_transceivers.get(transceiver.id)?` pre-handler enable-check (rejects deregistered transceivers at Anchor preflight); (d) content-addressed `inbox_item` PDA by `keccak256(payload)` → mathematical idempotency; (e) `Bitmap::count_enabled_votes(enabled)` bitwise-AND mask at count time → auto-invalidates stale votes. The handler-body `votes.set` + threshold-count are post-auth bookkeeping, NOT auth gates. Three DC-8-class attack paths (vote-twice, forge-transceiver-identity, vote-against-deregistered) all NEGATE. **W-5-NEG: this REPLACES the Gate 1 W-5 candidate that proposed NTT redeem.rs as a 4th DC-8 anchor — it is a negating-example, not an anchor.** (DC-8 positive anchors remain Indentura + M0 Extensions + OnRe.)
...
5. **notification-callback-informational-only (Phase 0 FORECLOSE gate)** (added 2026-05-28 from DISC-020 Filecoin #79987 CLOSED NOT-A-BUG productive-closure) — when scanning a notification/callback surface that LOOKS like sub-4 (user-set notifee + state-mutating send), Phase 0 MUST trace the economic-outcome code path BEFORE Gate 2 escalation. If the notification result is NOT consumed by the authorization/crediting logic — i.e., the notify is informational-only and a SEPARATE authorization path (cryptographic validation, registry-actor lookup, batch-claim verification) independently controls the economic state — the notification surface is **NOT exploitable for economic impact regardless of who controls the receiver**. FORECLOSE the notification surface at Phase 0, do NOT invest Gate 2 time. Canonical anchor: Filecoin FIP-0109 — `notify` field is informational; `verified_allocation_key + batch_claim_allocations` path independently validates FIL+ allocations through the verified registry actor. DISC-020 conflated these two paths; project rebuttal accepted as correct. This sub-pattern is the **structural negation** of sub-4: same SHAPE on the surface, opposite outcome because the economic-outcome path is separate. Mandatory check: for every DC-13 sub-4 candidate, identify the SEPARATE authorization/crediting code path; if it exists and is independent of the notification, fire sub-5 FORECLOSE. Cross-reference: Doctrine #39 CANDIDATE ("Notification Path ≠ Authorization Path"). [INSPECTED] (DISC-020 closure, 2026-05-28.)
...
## DC-9 sub-4 sub-variant — Cross-Substrate Quorum-Bitmap State-Not-Invalidated-Across-Config-Change (added 2026-05-29 — Wormhole NTT EVM/Sui POSITIVE + Solana NEGATING tri-substrate anchor; W-2-NEG)
...
**Status:** CANDIDATE (EVM PoC-confirmed via Hyp-C; promotion to canonical DC-9 sub-pattern gated on the Hyp-C Gate 2 submission outcome — TODO item 3). Authority: `hunts/2026-05-29-wormhole-ntt-hyp-e-gate2-foreclosure.md` §5 (W-2-NEG) + Hyp-C paste-ready.
...
**Status:** SUBSTRATE-ARSENAL (Clarity arsenal v1.2, **4 anchors = Hermetica + Granite + Zest + StackingDAO, all NEGATE**; lending sub-arsenal = 2 anchors, liquid-staking sub-arsenal = 1 anchor). Active Clarity detector lenses: DC-3-Clarity, CANDIDATE-I-Clarity (+ tracked-var sub-check, canonical + recoverability refinement), oracle-gating, as-contract, map-insert-replay, ERC4626-share-vs-index pricing-consistency, cross-protocol-rate-source manipulation, liquidation-3-guard checklist. Clarinet still uninstalled (no Clarity CONFIRM yet to PoC). Authority: hunts 2026-05-30-hermetica + 2026-05-30-granite + 2026-05-31-zest-v2 + 2026-05-31-stackingdao (Ogie msgs 8042 + 8067/8069/8072 autonomous EV-ordered loop).

---

## Detector Seed #167 — Mandatory-Path Arithmetic Edge → DoS/Brick/Halt (added 2026-05-30, GENERALIZED 2026-05-31)
...
**Signature to grep:** any `(/ X Y)` / `X / Y` / `(- A B)` on a mandatory accrual/charge/health/util path whose zero/underflow guard tests `(A+B)`, `(or A B)`, a sibling var, or is ABSENT. Correct: guard the divisor/minuend itself, or cross-multiply to avoid the division. Cross-substrate (Clarity `/` & `to-uint` neg abort; Solidity `/` revert + checked `-` underflow revert; Rust/Move `/` & `-` panic) → universal DoS/halt seed.
...
**Detection:** flag a dedup-set INSERT where (a) it precedes a validation/verify/require in the same function AND (b) the validation-failure branch (return-err / revert / early-return) lacks a matching REMOVE/DELETE/reset of the same key. Requires control-flow + data-flow (track the inserted key across the failure branch). **Build status: SPEC** — a Go-AST heuristic for the Cosmos-Go/IBC case is build-able in the #138/#165 framework (flag `Set<X>` insert with no matching `Delete<X>` on an error-return path); deferred (PoC-mandatory hunts take priority). Interim: **Standing-Intake Step 5.12 source-read checklist** (the manual check is reliable now).
...
**Gate-1 rule (direction-error discipline):** a `C138-SEQ-NO-MONOTONIC` hit is `[ASSUMED]`-reachable until the CALLERS are traced. If every non-genesis caller is a monotonic loop or the key is consensus-monotonic → NEGATE cheap. **Directly sharpens the June Heimdall run** — Heimdall checkpoint/span/milestone finalization setters follow the same setter-in-monotonic-loop shape; trace the caller before flagging. Genesis/migration callers (`InitGenesis`/`MigrateStore`) are run-once trusted → always discard. Governance setters (`SetParams`/version-map) = admin-trust OOS.

---

_Patterns: Defense Classes | v2.8 | 2026-05-29 | **Detector #138 NEGATING-EXAMPLE — Caller-Loop / Consensus-Key Monotonicity** (Babylon finality `setNextHeightToFinalize`/`SetNextHeightToReward` caller-loop + btcstaking `IndexBTCHeight` consensus-key anchors, Ogie msg 8025). A `C138-SEQ-NO-MONOTONIC` hit is `[ASSUMED]`-reachable until callers traced; monotonic-loop caller OR consensus-monotonic key → NEGATE cheap. Genesis/migration callers = run-once discard; governance setters = admin-trust OOS. Sharpens the June Heimdall run (same checkpoint/span/milestone setter-in-loop shape). Babylon Gate-1 NEGATE-on-prioritized-leads / PARK-residual (checkpointing BLS-bitmap W-2 analogue deferred). Authority: `hunts/2026-05-29-babylon-cosmos-gate1.md`._

_Patterns: Defense Classes | v2.7 | 2026-05-29 | **Detector Seed #166 — Cache-Before-Validate-No-Cleanup (CWE-459/460)** + companion **Doctrine #44 (Identity-vs-Content Binding Gap)**. Zebra GHSA-4m69-67m6-prqp ground-truth intake (PATCHED public advisory — NOT a hunt target; value = reusable seed). #166: dedup/seen/nonce/packet-receipt insert BEFORE validation with no cleanup-on-failure → legit same-id item rejected = lockout/DoS; substrate-agnostic (EVM · Solana · Cosmos-Go); SPEC + Standing-Intake Step 5.12 checklist + Go-AST build-candidate. First lit instance of the P3-corpus→P4-detector wire. Sibling-hunt: Hyperbridge #2 (both merkle binding-gap + cache-before-validate). Authority: Ogie msg 8021._

_Patterns: Defense Classes | v2.6 | 2026-05-29 | Wormhole NTT Hyp-E Gate 2 FORECLOSURE compounds (reboot-recovery TODO 2, Ogie msg 7962). Adds: (1) DC-8 NEGATING-EXAMPLES sub-catalog (Squads V4 + Wormhole NTT redeem.rs @ 4a15527c) — **W-5-NEG REPLACES the Gate 1 W-5 4th-anchor proposal**; NTT redeem.rs is a negating-example (defense IS in the Accounts struct), positive DC-8 anchors remain Indentura + M0 + OnRe; (2) DC-9 sub-4 sub-variant "Cross-Substrate Quorum-Bitmap State-Not-Invalidated-Across-Config-Change" — **W-2-NEG** tri-substrate anchor: EVM ManagerBase + Sui transceiver_registry POSITIVE, Solana bitmap.rs count_enabled_votes bitwise-AND mask NEGATING. Companion: Doctrine #41 CANDIDATE (Gate-1 PARTIAL-HIT requires Gate-2 positive/negative source-read before EV credit). Authority: `hunts/2026-05-29-wormhole-ntt-hyp-e-gate2-foreclosure.md`._

_Patterns: Defense Classes | v2.5 | 2026-05-27 | Day 27 evening — DC-9 sub-2 DEFENSE PATTERN 2nd anchor added (Multisig-with-Threshold-≥3-of-≥5 = Centralization-Accepted soft-foreclose). Anchor: DeFi Saver AdminVault `0xCCf3d848e08b94478Ed8f46fFead3008faF581fD` controlled by Gnosis Safe v1.1.1 `0x25eFA336886C74eA8E282ac466BdCd0199f85BB9` at 3-of-6 threshold, no attached timelock module, no on-chain incident in 5-year operational history. Pattern: programs with zero-timelock changeOwner/changeAdmin but **N-of-M multisig with N >= 3, M >= 5** are typically classified by triage as "Centralization Risk - Known/Accepted" or excluded under "Attacks requiring privileged access from within the organization." Pre-flight detection at Gate 1.5 (post-Gate 1, pre-Gate 2 Foundry): probe `getThreshold()` + `getOwners()` via cast on the admin address. If threshold/N ratio satisfies the 3-of-≥5 rule → SOFT FORECLOSE; if 1-of-N, EOA, or threshold == 1 → escalate to HIGH-severity DC-9 sub-2 finding. Cumulative DC-9 sub-2 DEFENSE PATTERN now has 2 anchors (Sky LockstakeMigrator governance-deprecation + DeFi Saver multisig-with-no-timelock) — both demonstrate operational defenses that neutralize the in-source DC-9 sub-2 substrate. Companion: NEW Doctrine #38 (Pure Pass-Through `*WithSig` Wrappers Are STRUCTURAL FORECLOSE — DeFi Saver 7-contract anchor). Authority: DeFi Saver Gate 2 foreclosure receipt (`hunts/2026-05-27-defisaver-c1-gate2-foreclosure.md`)._
...
## STRANDED-FUNDS RECOVERY-PATH TAXONOMY (NEW lane, 2026-05-31 — Ogie Stranded-Funds Scout directive; P3 corpus detector class + Pillar-4 arsenal cross-pollination)
...
4. **ANYONE-CALLABLE** — a function callable by ANYONE moves the funds (missing auth). **MAXIMUM-SENSITIVITY.** Publishing the target+path ARMS a black-hat to drain it. **CONTENT-LOCKED (never publish). Record to the 4B claimant-priority queue (legal-gated). Do NOT touch, do NOT drain (rescue-draining is the legally-risky move = OUT), do NOT publish. Record and move on.**
5. **UNRECOVERABLE** — no path (lost key + no admin fn; `selfdestruct`-only with no balance egress; burned-to-0x0; permanently-locked timelock). Funds are gone; generalized-content-only.

**Analysis method (guardrail-bound):** read source/bytecode → enumerate every fund-moving function (`transfer`/`call`/`send`/`withdraw`/`selfdestruct`) + its auth condition → classify per above → **VERIFY in a LOCAL mainnet-fork ONLY** (`anvil --fork-url <archive-rpc>` / foundry), prove the path moves funds in FORKED state, mark fork-verified vs analysis-only → **NEVER a mainnet tx** → resolve the legitimate claimant **RECORD-ONLY, NEVER contact**. Disposition: REACHABLE-OWNER (→ 4B hand-off queue, legal-gated) | TRULY-ABANDONED (generalized-content-only).

**HARD GUARDRAILS (define the lane's legality):** (1) zero mainnet execution — fork-only; (2) zero outreach — claimant resolution is record-only, outreach=Stage-4B=legal-gated=not built; (3) leak-safe content — never publish a live, not-yet-recovered, ANYONE-CALLABLE target+path; publishable = post-recovery case studies (funds home) OR fully-generalized methodology; (4) no rescue-draining / no fee / no execution-for-others. Any step crossing 1–4 → STOP + flag operator.
