...
| **5b PLATFORM-DIRECT-BOUNTY-NOT-LISTED** (NEW 2026-05-28 night) | Brief assumed Immunefi-listed program; live = protocol runs DIRECT bounty (private GitHub gist + Twitter/Telegram/Discord/Signal). `/frax/` + `/fraxfinance/` slugs both 404 across 219 Immunefi programs. Submission path is protocol-direct, not platform-mediated. **Cap payment currency = FRAX+FXS tokens (NOT USD)** — additional sub-axis. Cap realizability haircut: 10% exploit OR $10M (lesser) in protocol-native tokens; FX-realized cap ~33% of nominal at typical FRAX/FXS prices. | FRAX core Gate 1 2026-05-28 night |
| **5c PRODUCT-LINE-VS-BOUNTY-SCOPE-COLLAPSE** (NEW 2026-05-29) | Brief described the PRODUCT LINE ("Sky.money: SKY + USDS + sUSDS + LayerZero multi-chain"); live Immunefi SCOPE bounties only the legacy DSS classic L1 core (Vat/Dai/Spot/Pot/Vow/Flap/Flop/Jug/Median/OSM/RwaInputConduit2/WETH). The product-rich substrate (USDS upgradeable + sUSDS + LayerZero OFT in `makerdao/usds` + `sky-ecosystem-token`) is OUT-OF-SCOPE. **product ≠ bounty-scope.** Sub-rule: when a brief names a product line AND references protocol-canonical multi-chain architecture, the Step 1 PROFILE preflight MUST enumerate which contracts the program page actually bounties. When the brief's PRIMARY substrate lands OOS, the drift IS the FORECLOSE signal (ties to Step 3a §3a NEGATIVE-WORKED-EXAMPLE 4th anchor). [Hunt file labeled this "Axis 5a"; filed as 5c since 5a was already PLATFORM-CONFUSION.] | Sky DSS Immunefi $10M Gate 1 2026-05-29 (FORECLOSED) |
| **5d+ (reserved)** | Sub-classes d-f reserved for future direct-bounty / private-disclosure / platform-routing-edge cases | — |
...
_v1.14 INFO #19 EXPANSION 3 — 2026-05-28 night: Aave V3 Gate 1 anchors Axis 9 ORG-NAME drift 2nd anchor (`aave/aave-v3-core` → `aave-dao/aave-v3-origin`; canonical org migrated to DAO control). Axis 9 now 2-anchor (Flux `flux-finance/` + Aave-DAO `aave-dao/`) — pattern is organizational-control-handoff to DAO, not random rebrand. Doctrine #27 J corollary auto-FORECLOSURE-RECEIPT trigger fired cleanly on Aave V3 (HIGH-J 30+ audits 0.25× upfront NEGATE, NO CLONE, ~80-180MB disk saved). CANDIDATE-J cross-pollination cascade DIRECTION INVERSION discovered: actual flow is Spark→Aave V3→Aave-V3-forks (not Aave V3→Spark as predicted on cross-pollination map 2026-05-17). Cascade saturated at parent layer; fork hunts (Radiant/Sonne/Hashflow Lend) inherit short-circuit eligible per Doctrine #27 F MAXIMUM tier inheritance. Brain compound velocity: 4 background-agent hunts today (Stargate + Flux + Spark + Aave V3) = 1 PERMANENT doctrine promotion + 2 new drift sub-classes (Axis 5a PLATFORM-CONFUSION + Axis 9 2nd anchor) + 5+ anchor increments + 1 cross-pollination map inversion in 2h15min wall-time. ZERO Foundry investment. Compound engine at saturation tier approaches asymptotic returns._

_v1.15 INFO #19 EXPANSION 4 — 2026-05-29 (Sky DSS Gate 1, reboot-recovery TODO item 1, Ogie msg 7962): NEW Axis 5 sub-class **5c PRODUCT-LINE-VS-BOUNTY-SCOPE-COLLAPSE** — operator brief described the Sky.money product line (SKY+USDS+sUSDS+LayerZero multi-chain), but the live Immunefi SCOPE bounties only legacy DSS classic L1 core; the hypothesis-rich substrate (USDS upgradeable + sUSDS + LayerZero OFT) lives in separate OUT-OF-SCOPE repos. product≠bounty-scope. Pairs with the Step 3a §3a NEGATIVE-WORKED-EXAMPLE 4th anchor (substrate-OOS drift = standalone FORECLOSE signal → Doctrine #27 J). Hunt file proposed "Axis 5a"; filed as 5c (5a already PLATFORM-CONFUSION). Drift catalog now 9 axes + Axis 5 sub-classes {5a PLATFORM-CONFUSION, 5b PLATFORM-DIRECT-BOUNTY-NOT-LISTED, 5c PRODUCT-LINE-VS-BOUNTY-SCOPE-COLLAPSE}._

_v1.15 INFO #19 EXPANSION 4 — 2026-05-28 deep night: FRAX core Gate 1 anchors Axis 5 sub-class 5b PLATFORM-DIRECT-BOUNTY-NOT-LISTED (brief assumed Immunefi platform; live = FRAX runs direct bounty via private GitHub gist + Twitter/Telegram/Discord/Signal). 5b adds CAP-PAYMENT-CURRENCY sub-axis: FRAX+FXS tokens NOT USD; realized cap ~33% nominal at typical prices. **Standing-Intake Step 1 PROFILE Axis 2 (CAP) enrichment**: add `cap_payment_currency` field with token-cap haircut multiplier (USD/USDC = 1.0×; protocol-native = ~0.33× typical; high-volatility-native = 0.10-0.25×). FRAX Doctrine #27 J corollary auto-FORECLOSURE-RECEIPT triggered (MAXIMUM tier 20 audits / 6 firms / ToB×8 / Certora FV). CANDIDATE-J target-map top-3 picks now ALL FORECLOSED today (Aave V3 + Comet G2 NEGATE + FRAX) — cross-pollination cascade saturated at Pillar-1 lens-layer. Doctrine #37 Sub-Type B 6th anchor (FRAX V1/V2 frozen ~3y since V3 transition). Day-28 compound-engine total: 6 background-agent hunts in ~4h wall-time = 1 PERMANENT doctrine promotion + 3 new drift sub-classes (Axis 5a + 5b + Axis 9 2-anchor) + 1 new Pattern E EXCLUSION Class 3 + 6+ anchor increments + 1 cross-pollination cascade saturation. ZERO Foundry investment, ZERO false positives across 6 hunts._
