...
_v2.19 Addendum: 2026-05-29 (reboot-recovery TODO item 1) | row 347 sky-makerdao "core foreclosed" CONFIRMED via direct Sky DSS classic Gate 1 (`hunts/2026-05-29-sky-immunefi-gate1.md`, FORECLOSED EV ~$80, Doctrine #27 J auto-trigger). DSS classic core (Vat/Dai/Spot/Pot/Vow/Flap/Flop) = Doctrine #27 F **MAXIMUM 4th canonical anchor** (0.20×, 8y hardening, 4 Tier-1 firms) + Doctrine #37 Sub-Type B 6th anchor (longest-lived non-bridge core) + Pattern E EXCLUSION Class 3 2nd confirming anchor + Step 3a NEGATIVE-WORKED-EXAMPLE 4th anchor (brief's LayerZero/USDS substrate OOS — `makerdao/usds`+`sky-ecosystem-token` not bountied; product≠scope, INFO #19 Axis 5c). Submodules (Lockstake/dss-flappers/endgame-toolkit/SubDAO) remain the ONLY non-foreclosed HIGH surface per row 347. No DSS-classic-core re-dispatch without net-new module._

_v2.20 Addendum: 2026-05-29 (single-agent hunt loop, Ogie msg 8005) | **row #1 Lido FORECLOSED** (`hunts/2026-05-29-lido-immunefi-gate1.md`) — Doctrine #27 F MAXIMUM **5th canonical anchor** ($18.77B LST, $2M no-KYC, V1/V2/V3 maximally audited). V3 stVaults net-new surface (LazyOracle→VaultHub) source-read = layered-defended (freshness-delta-at-action-site + real-time inOutDelta + reserveRatio buffer + value-jump quarantine) → **DC-12 sub-6 Bounded-Staleness-Window EXCLUSION, 1st anchor**. CANDIDATE-I N/A (isolated per-vault, not pooled); DC-7 not clean (available-vs-required, same fresh report). WebFetch-only, ZERO disk. Next un-hunted queue target: **Renzo (#4, $182M restaking, $500K)** or Beefy (#10, $118M, $75K)._

_v2.21 Addendum: 2026-05-29 (hunt loop, Ogie msg 8008) | **Renzo (#4) WATCHLIST-PARK** (`hunts/2026-05-29-renzo-immunefi-gate1.md`) — EV <$3K. Step-1 EV-reducer stack: oracle-testing-prohibited clause (narrows oracle-MANIPULATION, NOT consumption-logic) + 12mo-stale scope (last-updated 2025-05-16 → frozen, Doctrine #34 sub-b window CLOSED) + KYC + cross-chain ezETH rate = April-2024 depeg vector (post-hardened). Source NOT cheaply reachable (renzo contracts repo at non-obvious org; assumed path + repo-root both 404). DC-12 sub-6 consumption-logic surface PARKED (Bounded-Staleness defense [ASSUMED]-not-confirmed — honest, not a claimed defense-FORECLOSE). NEW Step-1 sub-check (oracle-clause manipulation-vs-consumption distinction). Next un-hunted: **Beefy (#10, $118M, $75K)**. WebFetch-only, ZERO disk._
