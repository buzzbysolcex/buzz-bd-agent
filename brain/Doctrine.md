...
## Doctrine #41 CANDIDATE — Gate-1 PARTIAL-HIT Requires Gate-2 Positive-or-Negative Source-Read Before EV Credit (added 2026-05-29 — Wormhole NTT Hyp-E NEGATING-EXAMPLE anchor, reboot-recovery TODO 2)

**Rule (CANDIDATE — needs 2nd anchor):** A "PARTIAL HIT" defense-class classification produced during a Gate 1 surface map (especially WebFetch-only source reads) is a HYPOTHESIS, not a confirmed gap. Do NOT bank Gate-2-level EV credit for a "partial hit" until a commit-pinned Gate 2 source-read explicitly resolves it to POSITIVE (gap present, exploit path survives) or NEGATIVE (defense present → negating-example).
...
**Promotion path:** needs a 2nd anchor — any future Gate-1 PARTIAL-HIT that Gate 2 resolves to NEGATIVE (or a POSITIVE confirming the partial-hit was real, calibrating the discount). Cross-references: DC-8 NEGATING-EXAMPLES sub-catalog (Patterns-Defense-Classes.md); R8 Calibrated Reporting ([ASSUMED] tag on un-source-read claims); Doctrine #40 (companion — both guard against over-investing on under-confirmed signals).
