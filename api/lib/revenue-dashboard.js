...
function getRevenueVelocity() {
  ...
  const growth = prev7d.total > 0 ? ((current7d.total - prev7d.total) / prev7d.total) * 100 : 0;
  return {
...
function getKPIs() {
  const db = getDB();
  const total = db.prepare('SELECT COALESCE(SUM(amount_usd), 0) as total, COUNT(*) as count, COALESCE(AVG(amount_usd), 0) as avg FROM revenue_events').get();
  const mtd = db.prepare("SELECT COALESCE(SUM(amount_usd), 0) as total FROM revenue_events WHERE strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')").get();
  const pipeline = db.prepare("SELECT COUNT(*) as count, COALESCE(SUM(estimated_revenue_usd), 0) as value FROM pipeline_revenue_attribution WHERE current_stage NOT IN ('listed', 'rejected')").get();
  const conversion = db.prepare("SELECT CASE WHEN COUNT(*) > 0 THEN ROUND(SUM(CASE WHEN current_stage = 'listed' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 1) ELSE 0 END as rate FROM pipeline_revenue_attribution").get();
...
module.exports = { getDailyRollup, getWeeklyRollup, getMonthlyRollup, getTopTokensByRevenue, getRevenueByChain, getRevenueVelocity, getKPIs };
