...
function trackFailure(providerName) {
  ...
  if (tracker.failures.length >= FAILURE_THRESHOLD) {
    ...
    console.warn(
      `[LLM Cascade] ${providerName} hit ${FAILURE_THRESHOLD} failures — cooldown until ${new Date(tracker.cooldownUntil).toISOString()}`,
    );
  }
}
...
function trackTimeout(providerName) {
  ...
  timeoutTracker[providerName][hour] =
    (timeoutTracker[providerName][hour] || 0) + 1;
  ...
  if (keys.length > 6) {
    keys
      ...
      .forEach((k) => delete timeoutTracker[providerName][k]);
  }
}
...
function checkAdaptiveCascade(providerName) {
  ...
  if (
    timeouts > 3 &&
    (timeouts / totalAttempts) * 100 > TIMEOUT_THRESHOLD_PCT
  ) {
    adaptiveOverride = {
      provider: "bankr",
      until: Date.now() + ADAPTIVE_COOLDOWN_MS,
    };
    console.warn(
      `[LLM Cascade] ADAPTIVE: ${providerName} timeout rate ${Math.round((timeouts / totalAttempts) * 100)}% > ${TIMEOUT_THRESHOLD_PCT}% — switching to Bankr for 2h`,
    );
  }
}
...
function getProviderCascade(agentName) {
  ...
  if (
    subAgents.includes(agentName) ||
    agentName?.startsWith("sim-") ||
    agentName?.startsWith("interview-") ||
    agentName === "debate-analyst"
  ) {
  ...
  if (adaptiveOverride && Date.now() < adaptiveOverride.until) {
    console.log(
      `[LLM Cascade] Adaptive override active — Bankr primary until ${new Date(adaptiveOverride.until).toISOString()}`,
    );
    return [PROVIDERS.bankr, PROVIDERS.minimax, PROVIDERS.anthropic].filter(
      (p) => !isInCooldown(p.name),
    );
  }
  if (adaptiveOverride && Date.now() >= adaptiveOverride.until) {
    console.log(
      "[LLM Cascade] Adaptive override expired — M2.7 primary restored",
    );
    adaptiveOverride = null;
  }

  // Normal cascade
  return [PROVIDERS.minimax, PROVIDERS.bankr, PROVIDERS.anthropic].filter(
    (p) => !isInCooldown(p.name),
  );
}
...
async function callProvider(provider, body, timeout = 120000) {
  const apiKey =
    ...
    (provider.keyEnvFallback && process.env[provider.keyEnvFallback]);
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            reject(new Error(`Parse error from ${provider.name}`));
            return;
          }
          ...
          if (res.statusCode >= 400) {
            reject(
              new Error(
                `HTTP ${res.statusCode}: ${parsed.error?.message || raw.slice(0, 200)}`,
              ),
            );
            return;
          }
  ...
  for (let i = 0; i < providers.length; i++) {
    ...
    try {
      ...
      if (isPrimary && isTimeout) {
        console.warn(
          `[LLM Cascade] ${provider.name} timeout (${latency}ms) — retrying with ${RETRY_TIMEOUT_MS}ms...`,
        );
        ...
        logProviderCall({
          ...
          latency_ms: latency,
          success: false,
          error_message: `timeout_attempt_1 (${latency}ms)`,
        });
        ...
        try {
          ...
          console.log(
            `[LLM Cascade] ${provider.name} retry succeeded (${retryResult.latency_ms}ms)`,
          );
          return retryResult;
        } catch (retryErr) {
          const retryLatency = Date.now() - retryStart;
          console.warn(
            `[LLM Cascade] ${provider.name} retry also failed (${retryLatency}ms) — falling to next provider`,
          );
          trackTimeout(provider.name);
          ...
          logProviderCall({
            ...
            success: false,
            error_message: `timeout_attempt_2 (${retryLatency}ms)`,
          });
      ...
      console.warn(
        `[LLM Cascade] ${provider.name}/${provider.model} failed: ${err.message}`,
      );
      trackFailure(provider.name);
...
function getCascadeStatus() {
  ...
  for (const [name, provider] of Object.entries(PROVIDERS)) {
    ...
    status[name] = {
      ...
      cooldown_until: inCooldown
        ? new Date(tracker.cooldownUntil).toISOString()
        : null,
