...
async function getHyperliquidOI(symbol = "BTC") {
  ...
  if (idx === -1 || idx === undefined) {
    return {
      symbol,
      found: false,
      available: meta.universe?.map((u) => u.name).slice(0, 20),
    };
  }
...
async function getAaveMarkets(chain = "ethereum") {
  if (!ethers)
    return { chain, supported: false, error: "ethers not installed" };
...
async function calculateARIADepth(tokenAddress, symbol) {
  ...
  try {
    ...
    if (perps.found) {
      score += 5;
      factors.push({
        factor: "hyperliquid_perps",
        points: 5,
        data: { oi: perps.open_interest },
      });
    }
  } catch {}
  ...
  if (tokenAddress && tokenAddress.startsWith("0x")) {
    try {
      ...
      if (lending.has_lending) {
        score += 3;
        factors.push({
          factor: "lending_markets",
          points: 3,
          data: { chains: lending.lending_markets },
        });
      }
    } catch {}
  }
