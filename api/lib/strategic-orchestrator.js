...
function initialize(db, llmCaller) {
  ...
  initialized = true;
  console.log(
    "[StrategicOrchestrator] Initialized with Decision + Playbook + Context engines",
  );
}
...
function registerActionHandlers(engine) {
  ...
  engine.registerAction("dispatch_sub_agents", async (instance, ctx) => {
    ...
    console.log(
      `[PB-001] Dispatching 5 sub-agents for ${instance.token_ticker || instance.token_address}`,
    );
  ...
  engine.registerAction("generate_outreach_email", async (instance, ctx) => {
    console.log(
      `[PB-002] Generating outreach email for ${instance.token_ticker}`,
    );
  ...
  engine.registerAction("send_via_gmail_oauth", async (instance, ctx) => {
    console.log(
      `[PB-002] Sending outreach via Gmail for ${instance.token_ticker}`,
    );
  ...
  engine.registerAction("escalate_for_approval", async (instance, ctx) => {
    console.log(
      `[PB-003] Escalating ${instance.token_ticker} to Ogie for listing approval`,
    );
  ...
  engine.registerAction(
    "generate_listing_tweet_and_email",
    async (instance, ctx) => {
      console.log(
        `[PB-004] Generating listing announcement for ${instance.token_ticker}`,
      );
  ...
  engine.registerAction("archive_cold", async (instance, ctx) => {
    console.log(
      `[Archive] ${instance.token_ticker} archived as COLD, revisit in 30 days`,
    );
  ...
  engine.registerAction("set_rescan_timer", async (instance, ctx) => {
    ...
    console.log(
      `[Watch] ${instance.token_ticker} scheduled for rescan in ${hours}h`,
    );
 ...
 */
async function processTokenEvaluation(
  db,
  llmCaller,
  tokenData,
  subAgentOutputs,
  ...
  const score =
    subAgentOutputs.scorer?.score || subAgentOutputs.scorer?.final_score || 0;
  ...
  if (decision.playbook && decision.autoExecute) {
    try {
      ...
      console.error(
        `[StrategicOrchestrator] Playbook start failed: ${err.message}`,
      );
  ...
  if (
    decision.action === "IMMEDIATE_OUTREACH" ||
    decision.action === "QUEUE_PRIORITY"
  ) {
    try {
      db.prepare(
        `
  ...
  console.log(
    `[StrategicOrchestrator] ${tokenTicker || tokenAddress} → ${decision.type} (confidence: ${decision.confidence}%, ${Date.now() - startTime}ms)`,
  );
  ...
  try {
    ...
    for (const seq of overdueSequences) {
      const hoursSinceContact = Math.floor(
        (Date.now() - new Date(seq.last_sent_at || seq.created_at).getTime()) /
          (1000 * 60 * 60),
      );
    ...
    console.error(
      `[StrategicOrchestrator] Follow-up processing failed: ${err.message}`,
    );
