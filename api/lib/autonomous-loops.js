...
function completeLoopRun(runId, { tokensProcessed = 0, alertsGenerated = 0, summary = '', error = null }) {
  const db = getDB();
  const run = db.prepare('SELECT started_at FROM loop_cron_runs WHERE id = ?').get(runId);
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(error ? 'error' : 'completed', tokensProcessed, alertsGenerated, summary, error, duration, runId);
}

function saveLoopOutput(runId, { outputType, title, content, tokensMentioned = [], sentTo = null }) {
  const db = getDB();
  return db.prepare(`
    INSERT INTO loop_cron_outputs (run_id, output_type, title, content, tokens_mentioned, sent_to)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(runId, outputType, title || null, content, JSON.stringify(tokensMentioned), sentTo);
}
...
function generateMorningBrief() {
  ...
  try {
    ...
    const brief = {
      ...
      action_items: hotTokens.map(t => `Follow up on ${t.token_ticker || t.token_address.slice(0, 8)} (${Math.round(t.conversion_probability * 100)}% conversion)`)
    };
    ...
    completeLoopRun(runId, { tokensProcessed: hotTokens.length, alertsGenerated: 1, summary: `Pipeline: ${pipeline.length} stages, Revenue: $${todayRevenue.total}` });
    return brief;
  } catch (e) {
...
function generateDiscoveryAlert() {
  ...
  try {
    ...
    const highPriority = recentTokens.filter(t => t.conversion_probability >= 0.25);

    const alert = {
      period: '4h',
      new_discoveries: recentTokens.length,
      high_priority: highPriority,
      sources: [...new Set(recentTokens.map(t => t.discovery_source).filter(Boolean))]
    };
    ...
    completeLoopRun(runId, { tokensProcessed: recentTokens.length, alertsGenerated: highPriority.length, summary: `${recentTokens.length} discovered, ${highPriority.length} high-priority` });
    return alert;
  } catch (e) {
    completeLoopRun(runId, { error: e.message });
    throw e;
...
function generateEveningRecap() {
  ...
  try {
    ...
    const recap = {
      date: new Date().toISOString().split('T')[0],
      revenue: { total_usd: todayRevenue.total, deal_count: todayRevenue.count },
    ...
    completeLoopRun(runId, { tokensProcessed: stageChanges.reduce((s, c) => s + c.count, 0), alertsGenerated: 1, summary: `Revenue: $${todayRevenue.total}, ${stageChanges.length} stage groups active` });
    return recap;
  } catch (e) {
    completeLoopRun(runId, { error: e.message });
    throw e;
...
function getLoopHistory(loopType, limit = 20) {
  ...
  const params = [];
  if (loopType) { sql += ' WHERE loop_type = ?'; params.push(loopType); }
  sql += ' ORDER BY started_at DESC LIMIT ?';
  params.push(limit);
  return db.prepare(sql).all(...params);
}

module.exports = { generateMorningBrief, generateDiscoveryAlert, generateEveningRecap, getLoopHistory, startLoopRun, completeLoopRun, saveLoopOutput };
