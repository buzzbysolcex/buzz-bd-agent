...
const PERSONA_PROMPTS = {
  degen:
    "Aggressive degen trader. Loves hype, momentum, FOMO. Bets big on narratives.",
  whale:
    "Deep-pocketed whale. Focuses on liquidity, market cap, entry/exit without slippage.",
  institutional:
    "Skeptical institutional analyst. Demands audits, doxxed teams, compliance. Rejects most tokens.",
  community:
    "Community analyst. Evaluates social presence, dev activity, holder growth, engagement.",
  market_dynamics:
    "Market microstructure analyst. Focuses on spread, depth, MEV, order flow, cross-exchange arb.",
};
...
async function callOllama(prompt) {
  try {
    ...
    return {
      response:
        '{"action":"hold","belief":0.5,"reasoning":"timeout","post":""}',
      model: "fallback",
      tier: "error",
    };
  }
...
function parseAction(raw) {
  ...
  try {
    ...
    return {
      action: ["buy", "sell", "hold"].includes((p.action || "").toLowerCase())
    ...
    return { action: "hold", belief: 0.5, reasoning: "json error", post: "" };
  }
}
...
async function run1000(tokenData, options = {}) {
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    const recentPosts = socialFeed
      .slice(-20)
      .map((p) => `[${p.cluster}] ${p.post}`)
      .join("\n");
...
Respond JSON only: {"action":"buy"|"sell"|"hold","belief":0.0-1.0,"reasoning":"1 sentence","post":"1 sentence take"}`;
      ...
      const result = useOpus
        ? await callOpus(prompt)
      ...
      if ((i + 1) % 50 === 0) {
        log(
          `  R${round} agent ${i + 1}/${shuffled.length} (${useOpus ? "Opus" : "Ollama"})`,
        );
      }
    }
    ...
    for (const c of CLUSTERS) {
      ...
      clusterBeliefs[c] = ca.length
        ? +(ca.reduce((s, a) => s + a.belief, 0) / ca.length).toFixed(3)
        : 0;
    }
    const avgBelief = +(
    ...
    log(
      `R${round}: belief=${avgBelief} B/S/H=${roundBuys}/${roundSells}/${roundHolds} (${roundTime}s)`,
    );
  ...
  const result = {
    ...
    consensus:
      finalBuys > finalSells
        ...
        : finalSells > finalBuys
          ...
          : "NEUTRAL",
    consensus_strength: +(
      Math.abs(finalBuys - finalSells) / agentCount
    ).toFixed(3),
    ...
    opus_pct: +(
      (modelStats.opus / (modelStats.opus + modelStats.ollama)) *
      100
    ).toFixed(1),
  ...
  log(`\n=== COMPLETE ===`);
  log(
    `Final: ${finalBelief} | ${result.consensus} (${(result.consensus_strength * 100).toFixed(0)}%)`,
  );
  log(
    `Opus: ${modelStats.opus} | Ollama: ${modelStats.ollama} | Errors: ${modelStats.errors}`,
  );
  log(`Time: ${result.total_time_hours} hours`);

  // Save to file for pickup
  fs.writeFileSync(
    "/tmp/mirofish-1000-result.json",
