...
const PLAYBOOKS = {
  ...
  "PB-003": {
    ...
    transitions: {
      ...
      QUESTIONS: {
        next: "NEGOTIATING",
        action: "auto_answer_from_listing_package",
      },
  ...
  "PB-004": {
    ...
    transitions: {
      LISTED: {
        next: "ANNOUNCEMENT",
        action: "generate_listing_tweet_and_email",
      },
      ANNOUNCEMENT: {
        next: "MONITORING",
        action: "start_performance_tracking",
      },
      MONITORING: {
        next: {
          success: "UPSELL",
      ...
      WARNING: {
        next: "MONITORING",
        action: "alert_ogie_lp_drain_or_volume_crash",
      },
...
class PlaybookEngine {
    ...
    try {
      ...
      `,
        ...
        .run(
          ...
          initialState,
          JSON.stringify([
            { state: initialState, at: now, reason: "Playbook started" },
          ]),
          "{}",
          now,
          now,
      ...
      console.log(
        `[PlaybookEngine] Started ${playbookType} for ${tokenTicker || tokenAddress} (ID: ${result.lastInsertRowid})`,
      );
      ...
      console.error(
        `[PlaybookEngine] Failed to start ${playbookType}: ${err.message}`,
      );
      throw err;
    }
  }
  ...
  async advance(instanceId, transitionKey = null, context = {}) {
    ...
    if (!instance.is_active)
      throw new Error(`Playbook instance ${instanceId} is already completed`);
    ...
    if (!transition) {
      throw new Error(
        `No transition defined from state ${instance.current_state} in ${instance.playbook_type}`,
      );
    }
    ...
    if (typeof transition.next === "string") {
      ...
      typeof transition.next === "object" &&
      transition.next !== null
    ) {
      if (!transitionKey)
        throw new Error(
          `Transition key required for branching state ${instance.current_state}`,
        );
      nextState = transition.next[transitionKey];
      if (!nextState)
        throw new Error(
          `Invalid transition key '${transitionKey}' for state ${instance.current_state}`,
        );
    } else {
    ...
    if (transition.action && this.actionHandlers[transition.action]) {
      try {
        actionResult = await this.actionHandlers[transition.action](
          instance,
          context,
        );
      } catch (err) {
        console.error(
          `[PlaybookEngine] Action ${transition.action} failed: ${err.message}`,
        );
    ...
    history.push({
      ...
      to:
        nextState ||
        (transition.chains_to ? `→ ${transition.chains_to}` : "TERMINAL"),
      at: now,
      key: transitionKey,
    ...
    try {
      ...
      console.error(
        `[PlaybookEngine] Failed to update instance ${instanceId}: ${err.message}`,
      );
      throw err;
    }
