...
function generateDailyReport(date) {
  ...
  const prevDay = new Date(new Date(d).getTime() - 86400000).toISOString().split('T')[0];
  ...
  return {
    ...
    revenue: today,
    comparison: {
      vs_yesterday: yesterday.total_usd > 0 ? Math.round(((today.total_usd - yesterday.total_usd) / yesterday.total_usd) * 100) : 0,
      vs_7d_avg: avg7d.avg > 0 ? Math.round(((today.total_usd - avg7d.avg) / avg7d.avg) * 100) : 0
    },
...
function generateWeeklyReport(weekStart) {
  ...
  const prevWeekStart = new Date(new Date(ws).getTime() - 7 * 86400000).toISOString().split('T')[0];
  ...
  return {
    week_start: ws,
    revenue: thisWeek,
    wow_change_pct: lastWeek.total_usd > 0 ? Math.round(((thisWeek.total_usd - lastWeek.total_usd) / lastWeek.total_usd) * 100) : 0,
    by_source: bySource,
...
function generateMonthlyReport(yearMonth) {
  ...
  const daysPassed = ym === new Date().toISOString().slice(0, 7) ? new Date().getDate() : daysInMonth;
  const projectedTotal = daysPassed > 0 ? Math.round((thisMonth.total_usd / daysPassed) * daysInMonth * 100) / 100 : 0;

  return {
    month: ym,
    revenue: thisMonth,
    mom_change_pct: lastMonth.total_usd > 0 ? Math.round(((thisMonth.total_usd - lastMonth.total_usd) / lastMonth.total_usd) * 100) : 0,
...
function forecastRevenue(daysAhead = 30) {
  ...
  if (dailyRevenues.length < 2) {
    return { forecast_days: daysAhead, projected_daily: 0, projected_total: 0, confidence: 'low', data_points: dailyRevenues.length };
  }
  ...
  const xMean = (n - 1) / 2;
  const slope = values.reduce((s, v, i) => s + (i - xMean) * (v - avg), 0) / values.reduce((s, _, i) => s + (i - xMean) ** 2, 0);
  ...
  return {
    ...
    confidence_band: { low: Math.round(Math.max(0, projectedDaily - stddev) * daysAhead * 100) / 100, high: Math.round((projectedDaily + stddev) * daysAhead * 100) / 100 },
    data_points: n
  };
}
...
function getExecutiveSummary() {
  ...
  const ytd = db.prepare("SELECT COALESCE(SUM(amount_usd), 0) as total FROM revenue_events WHERE strftime('%Y', created_at) = strftime('%Y', 'now')").get();
  const pipelineHealth = db.prepare("SELECT COUNT(*) as total, SUM(CASE WHEN current_stage IN ('contacted', 'negotiating', 'approved') THEN 1 ELSE 0 END) as active FROM pipeline_revenue_attribution WHERE current_stage NOT IN ('listed', 'rejected')").get();
  ...
  return {
    ...
    health_label: healthScore >= 80 ? 'excellent' : healthScore >= 60 ? 'good' : healthScore >= 40 ? 'fair' : 'needs_attention'
  };
}
...
module.exports = { generateDailyReport, generateWeeklyReport, generateMonthlyReport, forecastRevenue, getExecutiveSummary };
