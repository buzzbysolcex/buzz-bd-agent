...
const CONTRACT_RULES = {
  ...
  pumpFunDetection: (address) => typeof address === 'string' && address.toLowerCase().endsWith('pump'),
  nameAloneIsTrusted: false,
};

const FRESHNESS_RULES = {
  maxAge_seconds: 3600,
  mcapChangeThreshold: 0.50,
  liquidityChangeThreshold: 0.50,
...
const OUTPUT_RULES = {
  ...
  requiredFields: ['contract_address', 'chain', 'verification_timestamp', 'data_sources'],
...
function validateContractFormat(address, chain) {
  ...
  return { valid: pattern.test(address), reason: pattern.test(address) ? null : `invalid_format_for_${chain}` };
}
...
function checkMcapChange(oldMcap, newMcap) {
  ...
  return { changed: change > FRESHNESS_RULES.mcapChangeThreshold, changePct: Math.round(change * 100) };
}

module.exports = {
  CONTRACT_RULES, FRESHNESS_RULES, CROSSREF_RULES, OUTPUT_RULES,
  validateContractFormat, isDataStale, checkMcapChange,
};
