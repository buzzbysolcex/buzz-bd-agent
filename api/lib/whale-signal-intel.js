  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        if (res.statusCode >= 400) { reject(new Error(`Nansen ${res.statusCode}: ${raw.slice(0, 200)}`)); return; }
        try { resolve(JSON.parse(raw)); } catch { resolve(null); }
      });
    });
    req.on('timeout', () => { req.destroy(); reject(new Error('Nansen timeout')); });
    req.on('error', reject);
    req.write(bodyStr);
    req.end();
  });
...
function logCost(endpoint, success, latencyMs) {
  try {
    const db = getDB();
    db.prepare(`INSERT INTO llm_provider_log (provider, model, agent, tokens_in, tokens_out, latency_ms, success, error_message)
      VALUES (?, ?, ?, 0, 0, ?, ?, ?)`).run('nansen', 'hyperliquid-api', 'whale-signal-intel', latencyMs, success ? 1 : 0, success ? null : endpoint);
  } catch (e) { /* silent */ }
}
...
async function fetchPerpScreener(smartMoneyOnly = false) {
  ...
  try {
    const body = {
      ...
      pagination: { page: 1, per_page: 50 },
      order_by: [{ direction: 'DESC', field: smartMoneyOnly ? 'net_position_change' : 'volume' }],
    };
...
async function getHyperliquidPairs() {
  if (hlPairsCache.data && Date.now() - hlPairsCache.ts < 24 * 60 * 60 * 1000) return hlPairsCache.data;
  try {
    const data = await new Promise((resolve, reject) => {
      ...
      req.on('error', reject);
      req.on('timeout', () => { req.destroy(); reject(new Error('HL timeout')); });
      req.write(bodyStr);
      req.end();
    });
    ...
    return new Set(['BTC', 'ETH', 'SOL', 'DOGE', 'PEPE', 'WIF', 'BONK', 'JUP', 'ONDO', 'SUI', 'APT', 'ARB', 'OP', 'AVAX', 'LINK', 'UNI', 'AAVE', 'MKR', 'SNX', 'CRV']);
  }
}
...
function computeWhaleSignalScore(smTrades, screenerData, positions) {
  ...
  const totalValue = (positions || []).reduce((sum, p) => sum + (p.position_value_usd || 0), 0);
  const buyVol = screenerData?.smart_money_buy_volume || screenerData?.buy_volume || 0;
  const sellVol = screenerData?.smart_money_sell_volume || screenerData?.sell_volume || 0;
  const funding = screenerData?.funding || 0;
  const oi = screenerData?.open_interest || 0;
  const traderCount = screenerData?.smart_money_longs_count || screenerData?.trader_count || 0;
  ...
  const score = flowScore + countScore + pressureScore + valueScore + fundingScore;
  const netFlow = longs > shorts ? 'LONG_BIAS' : shorts > longs ? 'SHORT_BIAS' : 'NEUTRAL';
  ...
  return {
    ...
    bearishFlag,
    netFlow,
    breakdown: { flow: flowScore, count: countScore, pressure: pressureScore, value: valueScore, funding: fundingScore },
    metrics: { longs, shorts, totalValue, buyVol, sellVol, funding, oi, traderCount, pressureRatio: Math.round(ratio * 100) / 100 },
  };
}
...
async function runWhaleSignalSweep() {
  ...
  let tokens = [];
  try {
    tokens = db.prepare("SELECT address, ticker, score, chain FROM pipeline_tokens WHERE score >= 60 AND ticker IS NOT NULL").all();
  } catch (e) { /* empty pipeline */ }
  ...
  for (const token of tokens) {
    ...
    if (!hasPerp) {
      results.push({ ticker: token.ticker, address: token.address, score: 50, hasPerp: false });
      continue;
    }
    ...
    const whaleScore = computeWhaleSignalScore(smTrades, screenerData, positions);

    // Cache
    try {
      db.prepare(`INSERT OR REPLACE INTO whale_signal_cache
      ...
      `).run(
        symbol, token.address, whaleScore.score, whaleScore.netFlow,
        whaleScore.metrics.longs, whaleScore.metrics.shorts, whaleScore.metrics.totalValue,
        whaleScore.metrics.pressureRatio, whaleScore.metrics.funding, whaleScore.metrics.oi,
        whaleScore.bearishFlag ? 1 : 0,
        JSON.stringify(whaleScore.breakdown),
        JSON.stringify({ screener: screenerData, positions: positions.slice(0, 5) })
      );
    } catch (e) { console.error('[whale-signal] Cache write error:', e.message); }

    results.push({ ticker: token.ticker, address: token.address, score: whaleScore.score, hasPerp: true, ...whaleScore });
  }

  console.log(`[whale-signal] Sweep complete: ${results.length} tokens, ${results.filter(r => r.hasPerp).length} with perps`);
  return results;
}
...
function getWhaleSignalFromCache(tokenSymbolOrAddress) {
  try {
    const db = getDB();
    let row = db.prepare("SELECT * FROM whale_signal_cache WHERE token_symbol = ? COLLATE NOCASE").get(tokenSymbolOrAddress);
    if (!row) row = db.prepare("SELECT * FROM whale_signal_cache WHERE pipeline_address = ?").get(tokenSymbolOrAddress);
    if (!row) return null;
    // Check TTL
