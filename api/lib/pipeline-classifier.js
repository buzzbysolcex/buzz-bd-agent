...
function dualGateCheck(scoreBreakdown) {
  if (!scoreBreakdown)
    return { pass: true, reason: "no breakdown available — skipping gate" };

  let breakdown;
  try {
    breakdown =
      typeof scoreBreakdown === "string"
        ? JSON.parse(scoreBreakdown)
        : scoreBreakdown;
  } catch {
  ...
  const technical =
    (breakdown.token_age || 0) +
    (breakdown.deployer_history || 0) +
    (breakdown.momentum || 0);
  const social =
    (breakdown.team_identity || 0) + (breakdown.social_presence || 0);
  const market =
  ...
  return {
    ...
    fundamentals: {
      ...
      max: fundamentalsMax,
      threshold: fundamentalsThreshold,
      pass: fundamentalsPass,
    },
    market: {
      score: marketScore,
      max: marketMax,
    ...
    reason: pass
      ? "dual-gate passed"
      : !fundamentalsPass && !marketPass
        ? `both gates failed (fundamentals ${fundamentalsScore}/${fundamentalsThreshold}, market ${marketScore}/${marketThreshold})`
        : !fundamentalsPass
          ? `fundamentals failed (${fundamentalsScore}/${fundamentalsThreshold})`
...
function classifyAndGate(score, scoreBreakdown) {
  ...
  const effectiveClassification =
    (classification === "hot" || classification === "qualified") && !gate.pass
      ? "watch"
      : classification;
  ...
  return {
    ...
    effective_stage:
      CLASSIFICATION_TO_STAGE[effectiveClassification] || "discovered",
  };
}
