...
async function syncPrices() {
  const db = getDB();
  console.log('[FinDatasets Sync] Starting price sync for qualified pipeline tokens...');
  ...
  console.log(`[FinDatasets Sync] Complete: ${synced} synced, ${failed} failed out of ${tokens.length} tokens`);
}

function start() {
  console.log(`[FinDatasets Sync] Scheduled — first run in ${STARTUP_DELAY_MS / 60000}min, then every ${SYNC_INTERVAL_MS / 3600000}h`);

  setTimeout(() => {
    syncPrices().catch(e => console.error('[FinDatasets Sync] Error:', e.message));
