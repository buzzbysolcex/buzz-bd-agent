      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode === 429 && retries < MAX_RETRIES) {
            ...
            console.warn(
              `[FinDatasets] 429 rate limited — retry in ${delay}ms (attempt ${retries + 1}/${MAX_RETRIES})`,
            );
            setTimeout(
              () =>
                apiGet(path, retries + 1)
                  .then(resolve)
                  .catch(reject),
              delay,
            );
...
async function getCryptoPrice(ticker) {
  try {
    return await apiGet(
      `/crypto/prices/snapshot/?ticker=${encodeURIComponent(ticker)}`,
    );
  } catch (err) {
    console.error(
      `[FinDatasets] getCryptoPrice(${ticker}) failed:`,
      err.message,
    );
    return null;
  }
 ...
 */
async function getHistoricalCryptoPrices(
  ticker,
  startDate,
  endDate,
  interval = "day",
  ...
  try {
    ...
    console.error(
      `[FinDatasets] getHistoricalCryptoPrices(${ticker}) failed:`,
      err.message,
    );
    return null;
  }
...
async function getAvailableCryptoTickers() {
  ...
  try {
    ...
    console.error(
      "[FinDatasets] getAvailableCryptoTickers failed:",
      err.message,
    );
    return null;
  }
...
async function getCompanyNews(ticker, limit = 10) {
  try {
    ...
    console.error(
      `[FinDatasets] getCompanyNews(${ticker}) failed:`,
      err.message,
    );
    return null;
  }
...
async function healthCheck() {
  try {
    ...
    return {
      status: "ok",
      configured: true,
      btcPrice: result?.snapshot?.price || null,
    };
  } catch (err) {
    return {
      status: "error",
      configured: !!process.env.FINANCIAL_DATASETS_API_KEY,
      error: err.message,
    };
  }
}
