...
describe('Phase 1: Revenue Metrics Tables', () => {
  test('revenue_events table exists', () => {
    const info = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='revenue_events'").get();
    expect(info).toBeTruthy();
  });

  test('listing_fees table exists', () => {
    const info = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='listing_fees'").get();
    expect(info).toBeTruthy();
  });

  test('monthly_revenue_summary table exists', () => {
    const info = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='monthly_revenue_summary'").get();
    expect(info).toBeTruthy();
  });
  ...
  test('chain constraint enforced', () => {
    expect(() => {
      db.prepare("INSERT INTO revenue_events (token_address, chain, event_type, amount_usd) VALUES ('x', 'ethereum', 'fee', 100)").run();
    }).toThrow();
  });
});
...
describe('Phase 3: Pipeline Revenue Attribution', () => {
  ...
  test('trackAttribution creates record', () => {
    const id = attribution.trackAttribution('So1token1', 'solana', 'twitter_brain', 'TKN1');
  ...
  test('recordAgentTouch increments count', () => {
    const result = attribution.recordAgentTouch('So1token1', 'solana', 'scanner-agent');
    expect(result.touch_count).toBe(1);
  });

  test('calculateConversionProbability returns value', () => {
    const result = attribution.calculateConversionProbability('So1token1', 'solana');
...
describe('Phase 4: Autonomous Loop Crons', () => {
  ...
  test('loop_cron_runs table exists', () => {
    const info = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='loop_cron_runs'").get();
    expect(info).toBeTruthy();
  });

  test('loop_cron_outputs table exists', () => {
    const info = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='loop_cron_outputs'").get();
    expect(info).toBeTruthy();
  });
...
describe('Phase 4: Agent Authority Matrix', () => {
  ...
  test('agent_authority_matrix table exists', () => {
    const info = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='agent_authority_matrix'").get();
    expect(info).toBeTruthy();
  });

  test('authority_audit_log table exists', () => {
    const info = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='authority_audit_log'").get();
    expect(info).toBeTruthy();
  });
  ...
  test('checkPermission denies unknown action', () => {
    const result = authority.checkPermission('scanner-agent', 'delete_database');
  ...
  test('grantPermission creates new entry', () => {
    authority.grantPermission('test-agent', 'test_action', { permissionLevel: 'read', maxDailyCalls: 10 });
...
describe('Phase 5: Revenue Dashboard', () => {
  ...
  beforeAll(() => {
    ...
    const insert = db.prepare('INSERT INTO revenue_events (token_address, chain, token_ticker, event_type, amount_usd, source) VALUES (?, ?, ?, ?, ?, ?)');
