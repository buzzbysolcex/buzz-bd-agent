...
describe("PlaybookEngine", () => {
  ...
  beforeAll(() => {
    ...
    engine.registerAction("check_duplicate", async () => ({
      duplicate: false,
    }));
    engine.registerAction("dispatch_sub_agents", async () => ({
      dispatched: true,
    }));
  ...
  test("should reject invalid playbook type", () => {
    expect(() => engine.start("PB-999", "0x", "solana", "X")).toThrow(
      "Unknown playbook",
    );
  });
  ...
  test("should require transition key for branching states", async () => {
    ...
    await expect(engine.advance(instance.id)).rejects.toThrow(
      "Transition key required",
    );
  });
});
