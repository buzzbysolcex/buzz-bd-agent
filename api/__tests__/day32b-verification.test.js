...
test('verifyToken returns VERIFIED when all checks pass', async () => {
  ...
  mockFetchResponses['dex/tokens/EPjFWdd5'] = {
    pairs: [{ chainId: 'solana', baseToken: { name: 'USD Coin', symbol: 'USDC', address: goodAddress },
      marketCap: 2000000000, liquidity: { usd: 500000000 }, priceUsd: '1.00',
      volume: { h24: 100000000 }, dexId: 'orca', pairCreatedAt: Date.now() - 365*86400000,
      pairAddress: 'abc123', url: 'https://dexscreener.com/solana/abc123' }]
  };
  mockFetchResponses['contract/EPjFWdd5'] = {
    name: 'USD Coin', symbol: 'usdc', id: 'usd-coin',
    market_data: { market_cap: { usd: 2100000000 }, total_volume: { usd: 90000000 } }
  };
...
test('verifyToken returns QUARANTINED when name mismatches between sources', async () => {
  ...
  mockFetchResponses['dex/tokens/FakeAddr'] = {
    pairs: [{ chainId: 'solana', baseToken: { name: 'Pygmy Hippo', symbol: 'HIPPO', address: addr },
      marketCap: 424000000, liquidity: { usd: 17000000 }, priceUsd: '0.0004',
      dexId: 'orca', pairCreatedAt: Date.now() - 30*86400000, pairAddress: 'pair1' }]
  };
  mockFetchResponses['contract/FakeAddr'] = {
    name: 'Moo Deng', symbol: 'moodeng', id: 'moo-deng',
...
test('verifyToken returns QUARANTINED when mcap differs >20% between sources', async () => {
  ...
  mockFetchResponses['dex/tokens/McapTest'] = {
    pairs: [{ chainId: 'solana', baseToken: { name: 'TestToken', symbol: 'TEST', address: addr },
      marketCap: 100000000, liquidity: { usd: 5000000 }, priceUsd: '1.00',
      dexId: 'orca', pairCreatedAt: Date.now() - 90*86400000, pairAddress: 'pair2' }]
  };
  mockFetchResponses['contract/McapTest'] = {
    name: 'TestToken', symbol: 'test', id: 'test-token',
    market_data: { market_cap: { usd: 50000000 } }  // 50% off
  };

  const result = await verifier.verifyToken(addr, 'solana');
  expect(result.overall).toBe('QUARANTINED');
  expect(result.mismatches.some(m => m.includes('MCAP_DIVERGENCE'))).toBe(true);
});
...
test('verifyToken returns UNVERIFIED when CoinGecko has no data', async () => {
  ...
  mockFetchResponses['dex/tokens/NoGecko'] = {
    pairs: [{ chainId: 'solana', baseToken: { name: 'NewCoin', symbol: 'NEW', address: addr },
      marketCap: 500000, liquidity: { usd: 50000 }, priceUsd: '0.01',
      dexId: 'raydium', pairCreatedAt: Date.now() - 7*86400000, pairAddress: 'pair3' }]
  };
...
test('verifyToken flags invalid contract format for chain', async () => {
  ...
  expect(result.check3_consistency.warnings.some(w => w.includes('CONTRACT_FORMAT_INVALID'))).toBe(true);
});

// ═══════ Test 6: Pump.fun detection ═══════
test('isPumpFun detects pump.fun tokens', () => {
  expect(verifier.isPumpFun('EUfRMccYw5E8Jv7NFmcpgKVFABqTr3QVputRQrbZpump')).toBe(true);
  expect(verifier.isPumpFun('2FcsqRrhvgSfYxJWh32xW873vyqBZ9jmMyXqYSPgNtuZ')).toBe(false);
  expect(verifier.isPumpFun('FZvhhggPbBn5j1CiSRUvy4iD9cENpWdWtAFF9KAupump')).toBe(true);
});
...
test('requireVerified middleware blocks unverified token', () => {
  ...
  const req = { body: { token_address: 'UnverifiedAddr1111111111111111111111111', chain: 'solana' }, query: {} };
...
test('requireVerified blocks quarantined tokens', () => {
  ...
  db.prepare("INSERT INTO quarantined_tokens (contract_address, chain, reason, quarantined_at) VALUES (?, ?, ?, datetime('now'))").run('QuarantinedAddr111111111111111111111111', 'solana', 'test quarantine');

  const { requireVerified } = require('../middleware/verification-gate');
  const req = { body: { token_address: 'QuarantinedAddr111111111111111111111111', chain: 'solana' }, query: {} };
  const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
  const next = jest.fn();

  requireVerified(req, res, next);
  expect(next).not.toHaveBeenCalled();
  expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'TOKEN_QUARANTINED' }));
});
...
test('verification log records every attempt', async () => {
  const countBefore = db.prepare('SELECT COUNT(*) as c FROM verification_log').get().c;

  // This will fail (no mock set up) but should still log
  await verifier.verifyToken('AuditTest111111111111111111111111111111111', 'solana');

  const countAfter = db.prepare('SELECT COUNT(*) as c FROM verification_log').get().c;
...
test('verified snapshot expires after max age', () => {
  ...
  const expiredAt = new Date(Date.now() - 3600000).toISOString().replace('T', ' ').replace('Z', '');
  const verifiedAt = new Date(Date.now() - 7200000).toISOString().replace('T', ' ').replace('Z', '');
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run('ExpiredAddr1111111111111111111111111111111', 'solana', 'Old Token', 'OLD', 1000, 500,
    verifiedAt, expiredAt);

  const result = verifier.isVerified('ExpiredAddr1111111111111111111111111111111', 'solana');
...
test('internal consistency catches name mismatch between DexScreener and CoinGecko', async () => {
  const dexData = { pass: true, data: { name: 'Pygmy Hippo', mcap: 424000000 }, warnings: [] };
  const cgData = { pass: true, data: { name: 'Moo Deng', mcap: 50000000 }, warnings: [] };
  ...
  const result = await verifier.verifyInternalConsistency(validAddr, 'solana', dexData, cgData);
...
test('contract format validation works per chain', () => {
  expect(rules.validateContractFormat('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v', 'solana').valid).toBe(true);
  expect(rules.validateContractFormat('0x1234567890abcdef1234567890abcdef12345678', 'base').valid).toBe(true);
  expect(rules.validateContractFormat('0xinvalid', 'base').valid).toBe(false);
  expect(rules.validateContractFormat('EPjFWdd5AufqSSqeM2qN', 'ethereum').valid).toBe(false);
});

// ═══════ Test 14: Quarantine resolution ═══════
test('resolveQuarantine marks token as resolved', () => {
  db.prepare("INSERT OR REPLACE INTO quarantined_tokens (contract_address, chain, reason, quarantined_at) VALUES (?, ?, ?, datetime('now'))").run('ResolveMe11111111111111111111111111111111', 'solana', 'test');

  verifier.resolveQuarantine('ResolveMe11111111111111111111111111111111', 'solana', 'ogie');

  const q = db.prepare('SELECT * FROM quarantined_tokens WHERE contract_address = ?').get('ResolveMe11111111111111111111111111111111');
