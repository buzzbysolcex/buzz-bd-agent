...
const MOLTBOOK_KEY = process.env.MOLTBOOK_API_KEY || 'moltbook_sk_zMehEPpryApvazWlEx2CBhkTcMp8kolr';
...
function initMoltbookPulse() {
  ...
  const seed = db().prepare('INSERT OR IGNORE INTO moltbook_pulse_state (key, value) VALUES (?, ?)');
...
function getState(key) {
  const row = db().prepare('SELECT value FROM moltbook_pulse_state WHERE key = ?').get(key);
  return row ? row.value : null;
}

function setState(key, value) {
  db().prepare('INSERT OR REPLACE INTO moltbook_pulse_state (key, value, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)').run(key, String(value));
}

function logEngagement(action, targetPostId, targetAgent, servicePromoted, content, responseCode) {
  db().prepare('INSERT INTO moltbook_engagement_log (action, target_post_id, target_agent, service_promoted, content, response_code) VALUES (?, ?, ?, ?, ?, ?)').run(action, targetPostId, targetAgent, servicePromoted, content, responseCode);
}
...
async function scanComments() {
  ...
  resetDailyCounters();
  if (getDailyCommentCount() >= 5) return { action: 'skip', reason: 'daily comment limit (5) reached' };

  try {
    const submolts = ['listing-strategy', 'crypto-history', 'crypto', 'general'];
    let newComments = 0;

    for (const submolt of submolts.slice(0, 2)) {
      const data = await moltbookFetch(`/submolts/${submolt}/posts?sort=new&limit=5`);
      if (!data || !data.posts) continue;

      for (const post of data.posts) {
        if (post.comment_count > 0 && post.author_id === 'c606278b-365f-473e-9203-3a517042a641') {
...
async function scanFeeds(tickCount) {
  ...
  try {
    ...
    for (const submolt of feeds) {
      const data = await moltbookFetch(`/posts?submolt=${submolt}&sort=new&limit=10`);
      if (!data || !data.posts) continue;
      ...
      for (const post of data.posts) {
        ...
        for (const [keyword, services] of Object.entries(SERVICE_PROMOTION_MAP)) {
          if (content.includes(keyword)) {
            relevant.push({ post_id: post.id, title: post.title, submolt, matched_keyword: keyword, services });
            break;
          }
        }
      }
    ...
    if (relevant.length > 0) {
      ...
      emit('pulse-moltbook', 'moltbook.service.promoted', { count: relevant.length });
    }
  ...
  try {
    ...
    for (const post of data.posts) {
      ...
      if (content.includes('agent') || content.includes('autonomous') || content.includes('ai')) {
        agents.push({ post_id: post.id, title: post.title, author: post.author?.name || 'unknown' });
      }
    }

    if (agents.length > 0) {
      emit('pulse-moltbook', 'moltbook.agent.found', { count: agents.length, agents: agents.slice(0, 3) });
    }
...
module.exports = { initMoltbookPulse, runMoltbookTick, scanComments, scanFeeds, discoverAgents };
