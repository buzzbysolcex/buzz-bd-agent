...
async function runBacktest(db, { daysBack = 7, checkAfterDays = 3 } = {}) {
  ...
  console.log(
    `[${runId}] 📊 Backtester: Starting (${daysBack}d lookback, check after ${checkAfterDays}d)`,
  );
  ...
  try {
    ...
    const cutoffEnd = new Date(
      Date.now() - checkAfterDays * 86400000,
    ).toISOString();

    const scoredTokens = db
      .prepare(
        `
    ...
    if (scoredTokens.length === 0) {
      console.log(
        `[${runId}] ⚠️ No scored tokens in window ${cutoffStart} → ${cutoffEnd}`,
      );
    ...
    console.log(
      `[${runId}] 📊 Found ${scoredTokens.length} tokens to backtest`,
    );
    ...
    for (const token of scoredTokens) {
      try {
        ...
        const subAgentAccuracy = evaluateSubAgentAccuracy(
          token,
          priceChangePct,
        );
        ...
        `,
          ...
          entry.symbol,
          entry.score_at_time,
          entry.price_at_score,
          entry.price_at_check,
          entry.price_change_pct,
          entry.days_elapsed,
          entry.signal_correct,
          entry.sub_agent_accuracy_json,
          entry.persona_accuracy_json,
          entry.scored_at,
        );
    ...
    if (backtestResults.length > 0) {
      ...
      `,
        ...
        summary.precision_rate,
        summary.avg_return_bullish,
        summary.avg_return_bearish,
        summary.best_agent,
        summary.best_persona,
    ...
    console.log(
      `[${runId}] ✅ Backtest complete: ${results.tokensEvaluated} tokens, accuracy ${results.summary?.accuracy_rate || "N/A"}%`,
    );
  } catch (err) {
...
async function fetchCurrentPrice(address) {
  try {
    ...
    const primary = pairs.reduce(
      (best, p) =>
        parseFloat(p.liquidity?.usd || 0) > parseFloat(best.liquidity?.usd || 0)
          ? p
          : best,
      pairs[0],
    );
...
function evaluateSubAgentAccuracy(token, priceChangePct) {
  ...
  try {
    subScores =
      typeof token.sub_agent_scores === "string"
  ...
  for (const [agent, agentScore] of Object.entries(subScores)) {
    const s =
      typeof agentScore === "object" ? agentScore.score || 0 : agentScore || 0;
    const bullish = s >= 65;
...
function evaluatePersonaAccuracy(token, priceChangePct) {
  ...
  try {
    personaScores =
      typeof token.persona_scores === "string"
  ...
  for (const [persona, pData] of Object.entries(personaScores)) {
    const signal =
      typeof pData === "object" ? pData.signal || "neutral" : "neutral";
...
function computeSummary(runId, results, periodStart, periodEnd) {
  ...
  const truePositives = bullishResults.filter(
    (r) => r.price_change_pct > 0,
  ).length;
  const precisionRate =
...
function findBestPerformer(results, jsonField) {
  ...
  for (const r of results) {
    ...
    try {
      data =
        typeof r[jsonField] === "string"
          ? JSON.parse(r[jsonField])
          : r[jsonField];
    } catch {
      continue;
    }
