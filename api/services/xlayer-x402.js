      ...
      (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            if (!receipt) {
              resolve({ verified: false, reason: "tx_not_found" });
              return;
            }
            ...
            resolve({ verified: false, reason: "parse_error" });
          }
        });
      },
    );
...
function x402Middleware(req, res, next) {
  const paymentHeader =
    req.headers["x-payment-tx"] || req.headers["x-402-payment"];
      ...
      (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            db.prepare(
              "UPDATE xlayer_transactions SET score_result = ?, status = 'completed' WHERE tx_hash = ?",
            ).run(result.score?.total || 0, txHash);
            resolve(result);
          } catch (e) {
            resolve({ error: "score_failed" });
          }
        });
      },
    );
