...
function signBip322(wif, address, message) {
  ...
  try {
    ...
    throw new Error('bip322-js not installed — run npm install bip322-js inside container');
  }
 ...
 */
async function fileSignalDirect({ beat_slug, headline, body, sources, tags, disclosure }) {
  if (!feature('STREAK_EMERGENCY_FILER')) {
    return { success: false, error: 'STREAK_EMERGENCY_FILER flag is false' };
  }
  ...
  if (!creds) {
    return { success: false, error: 'AIBTC_BTC_WIF or AIBTC_BTC_ADDRESS not configured' };
  }
  ...
  const payload = {
    ...
    sources: sources || [],
    tags: tags || [],
    disclosure: disclosure || 'Container-side filer (Buzz BD Agent / Ionic Nova). Direct HTTPS via bip322-js.'
  };
  ...
  try {
    ...
    if (!res.ok) {
      return { success: false, error: `AIBTC API ${res.status}: ${data.message || JSON.stringify(data)}` };
    }
    ...
    return { success: true, signal_id: signalId, status: data.signal?.status || 'submitted' };
  } catch (e) {
...
function checkFilerReady() {
  const flagOn = feature('STREAK_EMERGENCY_FILER');
  const creds = getWalletCreds();
  let depOk = false;
  try {
    require('bip322-js');
