...
function createSkill({
  ...
  if (fs.existsSync(skillPath)) {
    return {
      success: false,
      error: `Skill "${safeName}" already exists. Use patchSkill to update.`,
    };
  }
...
function patchSkill({ name, oldString, newString, reason = "improvement" }) {
  ...
  if (!content.includes(oldString))
    return { success: false, error: "oldString not found in skill content" };
...
function listSkills() {
  ...
  const skills = [];
  const dirs = fs
    .readdirSync(LEARNED_SKILLS_DIR, { withFileTypes: true })
...
function loadPatchLog() {
  try {
    ...
    if (fs.existsSync(SKILL_PATCHES_LOG))
      return JSON.parse(fs.readFileSync(SKILL_PATCHES_LOG, "utf8"));
  } catch (e) {}
  return [];
}
