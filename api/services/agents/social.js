...
async function runSocialAgent({ address, chain, requestId }) {
  ...
  console.log(
    `[${requestId}] 🌐 social-agent: Starting for ${address} on ${chain}`,
  );
  ...
  try {
    ...
    const dexSocials = await checkDexScreenerSocials(
      address,
      chain,
      factors,
      requestId,
    );
    const cgListed = await checkCoinGeckoListed(
      address,
      chain,
      factors,
      requestId,
    ...
    if (atvResult.status === "fulfilled" && atvResult.value) {
      ...
      if (atv.verified) {
        factors.push({
          name: "atv_verified",
          impact: 20,
          detail: `Identity verified: ${atv.name || "team confirmed"}`,
        });
      } else if (atv.found) {
        factors.push({
          name: "atv_found",
          impact: 10,
          detail: "Identity record exists but unverified",
        });
      } else {
        factors.push({
          name: "atv_not_found",
          impact: -5,
          detail: "No identity record in ATV",
        });
      }
    }
    ...
    if (serperResult.status === "fulfilled" && serperResult.value) {
      ...
      if (web.resultCount > 10) {
        factors.push({
          name: "strong_web_presence",
          impact: 10,
          detail: `${web.resultCount}+ web results`,
        });
      } else if (web.resultCount > 3) {
        factors.push({
          name: "moderate_web_presence",
          impact: 5,
          detail: `${web.resultCount} web results`,
        });
      } else {
        factors.push({
          name: "weak_web_presence",
          impact: -10,
          detail: `Only ${web.resultCount} web results`,
        });
      }
      ...
      if (web.hasScamMentions) {
        factors.push({
          name: "scam_mentions",
          impact: -25,
          detail: "Scam/rug mentions found in web results",
        });
      }
    }
    ...
    if (grokResult.status === "fulfilled" && grokResult.value) {
      ...
      if (sentiment.hasTwitter) {
        factors.push({
          name: "twitter_active",
          impact: 10,
          detail: "Active Twitter/X presence",
        });

        if (sentiment.followerCount > 10000) {
          factors.push({
            name: "large_following",
            impact: 10,
            detail: `${sentiment.followerCount.toLocaleString()} followers`,
          });
        } else if (sentiment.followerCount > 1000) {
          factors.push({
            name: "growing_following",
            impact: 5,
            detail: `${sentiment.followerCount.toLocaleString()} followers`,
          });
        }
        ...
        if (sentiment.engagementRate > 0.05) {
          factors.push({
            ...
            impact: 5,
            detail: `${(sentiment.engagementRate * 100).toFixed(1)}% engagement`,
          });
        } else if (sentiment.engagementRate < 0.01) {
          factors.push({
            name: "low_engagement",
            impact: -5,
            detail: "Suspiciously low engagement",
          });
        }
      } else {
        factors.push({
          name: "no_twitter",
          impact: -5,
          detail: "No Twitter/X account found",
        });
      }
      ...
      if (sentiment.hasTelegram) {
        factors.push({
          ...
          impact: 5,
          detail: "Telegram community exists",
        });
      }
      ...
      if (sentiment.sentimentScore > 0.7) {
        factors.push({
          ...
          impact: 10,
          detail: "Strong positive sentiment",
        });
      } else if (sentiment.sentimentScore < 0.3) {
        factors.push({
          name: "negative_sentiment",
          impact: -15,
          detail: "Predominantly negative sentiment",
        });
      }
    }
    ...
    const verdict =
      finalScore >= 70 ? "STRONG" : finalScore >= 40 ? "MODERATE" : "WEAK";
...
async function checkAtvIdentity(address, chain, factors, requestId) {
  ...
  if (!address || !address.startsWith("0x")) {
    factors.push({
      name: "atv_skipped",
      impact: 0,
      detail: `ATV ENS only supports EVM addresses (got ${chain})`,
    });
  ...
  try {
    const baseUrl =
      process.env.ATV_API_URL || ATV_BASE || "https://api.web3identity.com";
    ...
    if (!res.ok) {
      ...
      if (status === 402) {
        factors.push({
          name: "atv_quota_exceeded",
          impact: 0,
          detail: "ATV free tier exhausted (100/day)",
        });
      } else if (status === 429) {
        factors.push({
          name: "atv_rate_limited",
          impact: 0,
          detail: "ATV rate limited (10 req/min)",
        });
      } else {
        factors.push({
          name: "atv_error",
          impact: 0,
          detail: `ATV HTTP ${status}`,
        });
      }
      return { found: false, verified: false };
    }
    ...
    if (hasSocialProof) {
      ...
      factors.push({
        name: "atv_verified",
        impact: 20,
        detail: `ENS: ${item.ens} | ${socialDetails.join(", ")}`,
      });
      console.log(
        `[${requestId}] [ATV] ✅ Verified: ${item.ens} (${socialDetails.join(", ")})`,
      );
    } else {
      factors.push({
        name: "atv_found",
        impact: 10,
        detail: `ENS: ${item.ens} (no linked social accounts)`,
      });
      console.log(
        `[${requestId}] [ATV] 🏷️ ENS found: ${item.ens} (no socials)`,
      );
    }

    return {
      found: true,
      verified: hasSocialProof,
      name: item.ens,
      ens: item.ens,
      ...
      socials: [
        ...(hasTwitter
          ? [{ type: "twitter", handle: item.social.twitter }]
          : []),
...
async function checkSocialSentiment(address, chain, factors, requestId) {
  ...
  if (!SERPER_KEY) {
    factors.push({
      name: "social_sentiment_skipped",
      impact: 0,
      detail: "No Serper key for Twitter search",
    });
  ...
  try {
    const res = await fetch("https://google.serper.dev/search", {
      ...
      body: JSON.stringify({
        q: `site:x.com OR site:twitter.com ${address} token`,
        num: 10,
      }),
    ...
    for (const r of organic) {
      const text = `${r.title} ${r.snippet}`.toLowerCase();
      if (text.match(/bullish|moon|gem|based|legit|safu|growing|partnership/))
        positiveSignals++;
      if (text.match(/scam|rug|honeypot|fake|dead|dump|avoid|warning/))
        negativeSignals++;
    }
...
async function checkDexScreenerSocials(address, chain, factors, requestId) {
  try {
    ...
    if (websites.length > 0) {
      factors.push({
        name: "dex_website_found",
        impact: 10,
        detail: `Website: ${websites[0].url}`,
      });
    }
    ...
    for (const s of socials) {
      if (
        ...
        factors.push({
          name: "dex_twitter_found",
          impact: 10,
          detail: `Twitter: ${s.url}`,
        });
      }
      if (s.type === "telegram" || s.url?.includes("t.me")) {
        factors.push({
          name: "dex_telegram_found",
          impact: 5,
          detail: `Telegram: ${s.url}`,
        });
      }
      if (s.type === "discord" || s.url?.includes("discord")) {
        factors.push({
          name: "dex_discord_found",
          impact: 5,
          detail: `Discord: ${s.url}`,
        });
      }
    }
  ...
  try {
    ...
    if (data.id) {
      factors.push({
        ...
        impact: 10,
        detail: `CoinGecko: ${data.name} (${data.symbol})`,
      });
      if (data.links?.twitter_screen_name) {
        factors.push({
          name: "cg_twitter",
          impact: 5,
          detail: `CG Twitter: @${data.links.twitter_screen_name}`,
        });
      }
      if (data.links?.homepage?.[0]) {
        factors.push({
          name: "cg_homepage",
          impact: 5,
          detail: `CG Homepage: ${data.links.homepage[0]}`,
        });
      }
      return true;
    }
