...
const IL_SHIELD_BASE = 'https://flying-whale-il-shield-x402.flying-whale-ai.workers.dev';
...
async function checkILRisk(address, chain) {
  if (!feature('ILSHIELD_ENABLED')) {
    return { adjustment: 0, ilRisk: null, status: 'disabled', source: 'feature-flag' };
  }
  ...
  try {
    ...
    if (resp.status === 402) {
      ...
      console.log(`[IL-SHIELD] 402 Payment Required for ${address} — x402 flow not yet wired`);
      return { adjustment: 0, ilRisk: null, status: 'payment_required', source: 'il-shield' };
    }

    if (!resp.ok) {
      console.log(`[IL-SHIELD] Error ${resp.status} for ${address}`);
      return { adjustment: 0, ilRisk: null, status: 'error', source: 'il-shield' };
    }

    const data = await resp.json();
    const ilRisk = parseFloat(data.il_risk_pct || data.ilRisk || data.risk || 0);
