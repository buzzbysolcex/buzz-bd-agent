...
function evaluateInstitutionalFactors(data) {
  ...
  if (safetyScore >= 80) {
    factors.push({
      ...
      score: 25,
      detail: `Safety score ${safetyScore}/100 — clean contract`,
    });
  } else if (safetyScore >= 60) {
    factors.push({
      name: "contract_safety",
      score: 15,
      detail: `Safety score ${safetyScore}/100 — minor concerns`,
    });
  } else if (safetyScore > 0) {
    factors.push({
      name: "contract_safety",
      score: 5,
      detail: `Safety score ${safetyScore}/100 — significant risks`,
    });
  } else {
    factors.push({
      name: "contract_safety",
      score: 0,
      detail: "Safety score unavailable — cannot verify",
    });
  }

  // Instant kill checks (mint authority, honeypot, blacklist)
  const hasInstantKill =
    safety.instant_kills?.length > 0 ||
    safety.honeypot ||
    safety.mint_not_revoked;
  if (hasInstantKill) {
    factors.push({
      name: "instant_kill",
      score: -30,
      detail: "CRITICAL: Instant kill risk detected — institutional no-go",
    });
  }

  // Audit status
  const isAudited =
    safety.audited || safety.audit_status === "audited" || scanner.audit;
  if (isAudited) {
    factors.push({
      name: "audit_status",
      score: 20,
      detail: "Contract audited — meets institutional standard",
    });
  } else {
    factors.push({
      name: "audit_status",
      score: 0,
      detail: "No audit found — institutional risk flag",
    });
  }
  ...
  const hasTeam =
    social.team_visible ||
    social.team_doxxed ||
    scanner.info?.socials?.length > 2;
  const hasWebsite = scanner.website || scanner.info?.websites?.length > 0;
  if (hasTeam && hasWebsite) {
    factors.push({
      name: "team_credibility",
      score: 15,
      detail: "Team visible with website — credible project",
    });
  } else if (hasWebsite) {
    factors.push({
      name: "team_credibility",
      score: 8,
      detail: "Website exists but team not fully visible",
    });
  } else {
    factors.push({
      name: "team_credibility",
      score: 0,
      detail: "No website or team info — anon project",
    });
  }
  ...
  if (lpLocked) {
    factors.push({
      name: "liquidity_lock",
      score: 15,
      detail: "Liquidity locked — institutional requirement met",
    });
  } else {
    factors.push({
      name: "liquidity_lock",
      score: 0,
      detail: "Liquidity not locked — rug risk for institutional entry",
    });
  }
  ...
  if (mcap > 10000000) {
    factors.push({
      ...
      score: 15,
      detail: `$${(mcap / 1e6).toFixed(1)}M mcap — institutional-grade`,
    });
  } else if (mcap > 1000000) {
    factors.push({
      name: "market_maturity",
      score: 10,
      detail: `$${(mcap / 1e6).toFixed(1)}M mcap — emerging but viable`,
    });
  } else if (mcap > 100000) {
    factors.push({
      name: "market_maturity",
      score: 5,
      detail: `$${(mcap / 1000).toFixed(0)}K mcap — too early for institutional`,
    });
  } else {
    factors.push({
      name: "market_maturity",
      score: 0,
      detail: "Micro cap — below institutional threshold",
    });
  }
  ...
  if (cmcListed) {
    factors.push({
      name: "cmc_listing",
      score: 10,
      detail: "CMC listed — passes basic legitimacy check",
    });
  }

  // Ownership renounced
  const ownershipRenounced =
    safety.ownership_renounced || safety.owner_renounced;
  if (ownershipRenounced) {
    factors.push({
      name: "ownership",
      score: 10,
      detail: "Ownership renounced — decentralized control",
    });
  }
...
function generateInstitutionalReasoning(factors, score) {
  const top = factors
    ...
    .slice(0, 3);
  const risks = factors.filter(
    (f) => f.score <= 0 && f.name !== "instant_kill",
  );
