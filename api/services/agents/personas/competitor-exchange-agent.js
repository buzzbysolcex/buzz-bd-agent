...
function evaluateCompetitiveFactors(data) {
  ...
  if (mcap > 0 && mcap < 10000000 && volume > 50000) {
    factors.push({
      name: "first_mover",
      score: 25,
      detail: "Sub-$10M mcap with volume — first-mover CEX listing advantage",
    });
  } else if (mcap > 0 && mcap < 50000000 && volume > 100000) {
    factors.push({
      name: "first_mover",
      score: 18,
      detail: "Mid-cap with strong volume — competitive listing opportunity",
    });
  } else if (mcap >= 50000000) {
    factors.push({
      name: "first_mover",
      score: 5,
      detail: "Large cap — likely already on multiple CEXs",
    });
  } else {
    factors.push({
      name: "first_mover",
      score: 8,
      detail: "Early stage — potential first-mover but risky",
    });
  }
  ...
  if (liquidity > 100000 && volume > 200000) {
    factors.push({
      name: "validation_signal",
      score: 20,
      detail: "Strong DEX metrics — listing validates token for other CEXs",
    });
  } else if (liquidity > 50000) {
    factors.push({
      name: "validation_signal",
      score: 12,
      detail: "Decent liquidity — moderate validation signal",
    });
  } else {
    factors.push({
      name: "validation_signal",
      score: 3,
      detail: "Low liquidity — weak validation signal",
    });
  }
  ...
  if (twitterFollowers > 50000) {
    factors.push({
      name: "cex_ready_community",
      score: 20,
      detail: `${twitterFollowers} followers — CEX-ready community`,
    });
  } else if (twitterFollowers > 10000) {
    factors.push({
      name: "cex_ready_community",
      score: 12,
      detail: `${twitterFollowers} followers — growing community`,
    });
  } else if (twitterFollowers > 1000) {
    factors.push({
      name: "cex_ready_community",
      score: 5,
      detail: `${twitterFollowers} followers — early community`,
    });
  } else {
    factors.push({
      name: "cex_ready_community",
      score: 0,
      detail: "Minimal community — not CEX-ready",
    });
  }
  ...
  if (safetyScore >= 80) {
    factors.push({
      name: "cex_safety",
      score: 20,
      detail: `Safety ${safetyScore}/100 — meets CEX listing standards`,
    });
  } else if (safetyScore >= 60) {
    factors.push({
      name: "cex_safety",
      score: 10,
      detail: `Safety ${safetyScore}/100 — borderline for CEX`,
    });
  } else {
    factors.push({
      name: "cex_safety",
      score: 0,
      detail: `Safety ${safetyScore}/100 — would not pass CEX review`,
    });
  }
  ...
  if (volume > 1000000) {
    factors.push({
      name: "volume_growth",
      score: 15,
      detail: `$${(volume / 1e6).toFixed(1)}M daily — CEX listing adds volume`,
    });
  } else if (volume > 100000) {
    factors.push({
      name: "volume_growth",
      score: 10,
      detail: `$${(volume / 1000).toFixed(0)}K daily — CEX listing would boost significantly`,
    });
  } else {
    factors.push({
      name: "volume_growth",
      score: 3,
      detail: "Low volume — CEX listing may not generate meaningful volume",
    });
  }
