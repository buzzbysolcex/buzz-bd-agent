...
function evaluateWhaleFactors(data) {
  ...
  const topHolderPct = parseFloat(
    wallet.top10_holder_pct || wallet.top_holders_pct || 0,
  );
  if (topHolderPct > 0 && topHolderPct < 30) {
    factors.push({
      name: "holder_distribution",
      score: 20,
      detail: `Top 10 hold ${topHolderPct.toFixed(1)}% — well distributed, whale-safe`,
    });
  } else if (topHolderPct >= 30 && topHolderPct < 50) {
    factors.push({
      name: "holder_distribution",
      score: 12,
      detail: `Top 10 hold ${topHolderPct.toFixed(1)}% — moderate concentration`,
    });
  } else if (topHolderPct >= 50) {
    factors.push({
      name: "holder_distribution",
      score: 0,
      detail: `Top 10 hold ${topHolderPct.toFixed(1)}% — high concentration risk`,
    });
  } else {
    factors.push({
      name: "holder_distribution",
  ...
  const holders = parseInt(
    wallet.unique_holders || wallet.holders || scanner.holders || 0,
  );
  if (holders > 5000) {
    factors.push({
      name: "holder_count",
      score: 20,
      detail: `${holders} holders — strong adoption`,
    });
  } else if (holders > 1000) {
    factors.push({
      name: "holder_count",
      score: 14,
      detail: `${holders} holders — growing base`,
    });
  } else if (holders > 200) {
    factors.push({
      name: "holder_count",
      score: 7,
      detail: `${holders} holders — early stage`,
    });
  } else {
    factors.push({
      name: "holder_count",
      score: 0,
      detail: `${holders || "Unknown"} holders — too thin`,
    });
  }
  ...
  if (liquidity > 500000) {
    factors.push({
      ...
      score: 20,
      detail: `$${(liquidity / 1e6).toFixed(2)}M liquidity — whale-grade depth`,
    });
  } else if (liquidity > 100000) {
    factors.push({
      name: "liquidity_depth",
      score: 14,
      detail: `$${(liquidity / 1000).toFixed(0)}K liquidity — adequate for medium positions`,
    });
  } else if (liquidity > 25000) {
    factors.push({
      name: "liquidity_depth",
      score: 7,
      detail: `$${(liquidity / 1000).toFixed(0)}K liquidity — thin for whale entry`,
    });
  } else {
    factors.push({
      name: "liquidity_depth",
      score: 0,
      detail: "Insufficient liquidity for whale positions",
    });
  }
  ...
  if (smartMoneyIn > smartMoneyOut && smartMoneyIn > 0) {
    factors.push({
      ...
      score: 15,
      detail: "Net smart money inflow detected",
    });
  } else if (smartMoneyOut > smartMoneyIn && smartMoneyOut > 0) {
    factors.push({
      name: "smart_money_flow",
      score: 0,
      detail: "Smart money outflow — whales exiting",
    });
  } else {
    factors.push({
      name: "smart_money_flow",
      score: 5,
      detail: "No clear smart money signal",
    });
  }
  ...
  if (dexToCex > 0) {
    factors.push({
      name: "dex_cex_flow",
      score: 10,
      detail: "DEX→CEX flow detected — potential CEX listing prep",
    });
  }

  // Pair age (older = more established = safer for whales)
  const pairAge =
    scanner.pair_age_hours || scanner.pairCreatedAt
      ? Math.floor(
          (Date.now() - new Date(scanner.pairCreatedAt).getTime()) / 3600000,
        )
      : 0;
  if (pairAge > 720) {
    // 30+ days
    factors.push({
      name: "pair_maturity",
      score: 15,
      detail: `Pair age ${Math.floor(pairAge / 24)}d — established and tested`,
    });
  } else if (pairAge > 168) {
    // 7+ days
    factors.push({
      name: "pair_maturity",
      score: 10,
      detail: `Pair age ${Math.floor(pairAge / 24)}d — survived first week`,
    });
  } else if (pairAge > 24) {
    factors.push({
      name: "pair_maturity",
      score: 5,
      detail: `Pair age ${Math.floor(pairAge / 24)}d — very new`,
    });
  } else {
    factors.push({
      name: "pair_maturity",
      score: 0,
      detail: "Pair < 24h old — too early for whale entry",
    });
  }
