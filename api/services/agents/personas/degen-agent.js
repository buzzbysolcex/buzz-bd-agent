...
function evaluateDegenFactors(data) {
  ...
  if (volLiqRatio > 3) {
    factors.push({
      name: "volume_surge",
      score: 20,
      detail: `Vol/Liq ratio ${volLiqRatio.toFixed(1)}x — massive trading activity`,
    });
  } else if (volLiqRatio > 1.5) {
    factors.push({
      name: "volume_surge",
      score: 12,
      detail: `Vol/Liq ratio ${volLiqRatio.toFixed(1)}x — healthy momentum`,
    });
  } else if (volLiqRatio > 0.5) {
    factors.push({
      name: "volume_surge",
      score: 5,
      detail: `Vol/Liq ratio ${volLiqRatio.toFixed(1)}x — moderate activity`,
    });
  } else {
    factors.push({
      name: "volume_surge",
      score: 0,
      detail: "Low volume relative to liquidity",
    });
  }
  ...
  if (priceChange > 50) {
    factors.push({
      name: "price_momentum",
      score: 20,
      detail: `+${priceChange.toFixed(1)}% in 24h — parabolic`,
    });
  } else if (priceChange > 20) {
    factors.push({
      name: "price_momentum",
      score: 15,
      detail: `+${priceChange.toFixed(1)}% in 24h — strong pump`,
    });
  } else if (priceChange > 5) {
    factors.push({
      name: "price_momentum",
      score: 8,
      detail: `+${priceChange.toFixed(1)}% in 24h — trending up`,
    });
  } else if (priceChange > -10) {
    factors.push({
      name: "price_momentum",
      score: 3,
      detail: `${priceChange.toFixed(1)}% in 24h — stable`,
    });
  } else {
    factors.push({
      name: "price_momentum",
      score: 0,
      detail: `${priceChange.toFixed(1)}% in 24h — dumping`,
    });
  }
  ...
  if (mcap > 0 && mcap < 500000) {
    factors.push({
      name: "early_stage",
      score: 20,
      detail: `$${(mcap / 1000).toFixed(0)}K mcap — micro cap gem potential`,
    });
  } else if (mcap > 0 && mcap < 2000000) {
    factors.push({
      name: "early_stage",
      score: 15,
      detail: `$${(mcap / 1e6).toFixed(1)}M mcap — small cap with room`,
    });
  } else if (mcap > 0 && mcap < 10000000) {
    factors.push({
      name: "early_stage",
      score: 8,
      detail: `$${(mcap / 1e6).toFixed(1)}M mcap — mid cap`,
    });
  } else {
    factors.push({
      name: "early_stage",
      score: 2,
      detail: "Large cap — limited degen upside",
    });
  }
  ...
  const hasMemeSignal = (scanner.name || "").match(
    /pepe|doge|shib|inu|cat|frog|wojak|chad|moon/i,
  );
  if (hasMemeSignal) {
    factors.push({
      name: "meme_potential",
      score: 15,
      detail: "Meme narrative detected — degen magnet",
    });
  } else if (twitterFollowers > 10000) {
    factors.push({
      name: "meme_potential",
      score: 10,
      detail: `${twitterFollowers} Twitter followers — growing community`,
    });
    ...
    factors.push({
      name: "meme_potential",
      score: 0,
      detail: "No meme narrative or social presence",
    });
  }
  ...
  if (buyRatio > 0.65) {
    factors.push({
      name: "buy_pressure",
      score: 15,
      detail: `${(buyRatio * 100).toFixed(0)}% buys — heavy accumulation`,
    });
  } else if (buyRatio > 0.55) {
    factors.push({
      name: "buy_pressure",
      score: 8,
      detail: `${(buyRatio * 100).toFixed(0)}% buys — mild buy pressure`,
    });
  } else {
    factors.push({
      name: "buy_pressure",
      score: 2,
      detail: `${(buyRatio * 100).toFixed(0)}% buys — sell pressure dominant`,
    });
  }
  ...
  if (scanner.boosted || scanner.boostCount > 0) {
    factors.push({
      name: "boosted",
      score: 10,
      detail: "DexScreener boosted — active promotion",
    });
  }
