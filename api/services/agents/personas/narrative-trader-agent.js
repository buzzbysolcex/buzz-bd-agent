...
function evaluateNarrativeFactors(data) {
  ...
  const hasMeme = name.match(
    /pepe|doge|shib|inu|cat|frog|wojak|chad|moon|elon|trump|ai|gpt|agent/i,
  );
  const hasTrend = name.match(/ai|agent|rwa|depin|meme|l2|restaking|bags/i);

  if (hasMeme && hasTrend) {
    factors.push({
      name: "viral_narrative",
      score: 25,
      detail: "Multi-narrative token — meme + trend overlap (CT magnet)",
    });
  } else if (hasTrend) {
    factors.push({
      name: "viral_narrative",
      score: 18,
      detail: "Trend narrative detected — potential CT momentum",
    });
  } else if (hasMeme) {
    factors.push({
      name: "viral_narrative",
      score: 15,
      detail: "Meme narrative — viral potential but shorter shelf life",
    });
  } else {
    factors.push({
      name: "viral_narrative",
      score: 3,
      detail: "No obvious narrative — harder to generate CT buzz",
    });
  }

  // Listing event alpha (price impact of CEX listing)
  const mcap = parseFloat(
    scanner.market_cap || scanner.mcap || scanner.fdv || 0,
  );
  if (mcap > 0 && mcap < 5000000) {
    factors.push({
      name: "listing_alpha",
      score: 25,
      detail: "Sub-$5M mcap — CEX listing could 2-5x from exposure alone",
    });
  } else if (mcap > 0 && mcap < 20000000) {
    factors.push({
      name: "listing_alpha",
      score: 18,
      detail: "$5-20M mcap — meaningful listing pump expected",
    });
  } else if (mcap > 0 && mcap < 100000000) {
    factors.push({
      name: "listing_alpha",
      score: 10,
      detail: "$20-100M mcap — moderate listing impact",
    });
  } else {
    factors.push({
      name: "listing_alpha",
      score: 2,
      detail: "Large cap — listing already priced in",
    });
  }

  // Momentum (price + buy pressure)
  const priceChange = parseFloat(
    scanner.price_change_24h || scanner.priceChange?.h24 || 0,
  );
  ...
  if (priceChange > 30 && buyRatio > 0.6) {
    factors.push({
      name: "momentum_play",
      score: 20,
      detail: `+${priceChange.toFixed(0)}% with ${(buyRatio * 100).toFixed(0)}% buys — narrative momentum confirmed`,
    });
  } else if (priceChange > 10 && buyRatio > 0.55) {
    factors.push({
      name: "momentum_play",
      score: 12,
      detail: `+${priceChange.toFixed(0)}% — mild momentum`,
    });
  } else if (priceChange > -10) {
    factors.push({
      name: "momentum_play",
      score: 5,
      detail: "Sideways — waiting for catalyst",
    });
  } else {
    factors.push({
      name: "momentum_play",
      score: 0,
      detail: `${priceChange.toFixed(0)}% — negative momentum, narrative dead`,
    });
  }
  ...
  if (twitterFollowers > 50000) {
    factors.push({
      name: "ct_presence",
      score: 15,
      detail: `${twitterFollowers} followers — CT narrative ready`,
    });
  } else if (twitterFollowers > 10000) {
    factors.push({
      name: "ct_presence",
      score: 10,
      detail: `${twitterFollowers} followers — growing CT presence`,
    });
  } else if (twitterFollowers > 2000) {
    factors.push({
      name: "ct_presence",
      score: 5,
      detail: `${twitterFollowers} followers — early CT buzz`,
    });
  } else {
    factors.push({
      name: "ct_presence",
      score: 0,
      detail: "No CT presence — hard to trade the narrative",
    });
  }
  ...
  if (volLiqRatio > 3) {
    factors.push({
      name: "volume_spike",
      score: 15,
      detail: `${volLiqRatio.toFixed(1)}x vol/liq — narrative catalyst in progress`,
    });
  } else if (volLiqRatio > 1.5) {
    factors.push({
      name: "volume_spike",
      score: 8,
      detail: `${volLiqRatio.toFixed(1)}x vol/liq — above average interest`,
    });
  } else {
    factors.push({
      name: "volume_spike",
      score: 2,
      detail: "Normal volume — no narrative catalyst detected",
    });
  }
