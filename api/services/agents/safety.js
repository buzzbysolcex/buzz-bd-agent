...
async function runSafetyAgent({ address, chain, requestId }) {
  ...
  console.log(
    `[${requestId}] 🛡️ safety-agent: Starting for ${address} on ${chain}`,
  );
  ...
  try {
    ...
    try {
      ...
      if (rugRes.ok) {
        ...
        factors.push({
          name: "rugcheck_unavailable",
          impact: -5,
          detail: `HTTP ${rugRes.status}`,
        });
        rawScore -= 5;
      }
    } catch (rugErr) {
    ...
    if (rugcheckData) {
      ...
      const riskLevel = String(
        rugcheckData.score_normalised || rugcheckData.score || 0,
      );
      ...
      if (rugcheckData.tokenMeta?.createdOn) {
        const ageMs =
          Date.now() - new Date(rugcheckData.tokenMeta.createdOn).getTime();
        ...
        if (ageDays < 1) {
          factors.push({
            name: "very_new_token",
            impact: -15,
            detail: `Created ${Math.round(ageDays * 24)}h ago`,
          });
          rawScore -= 15;
        } else if (ageDays < 7) {
          factors.push({
            name: "new_token",
            impact: -5,
            detail: `Created ${Math.round(ageDays)}d ago`,
          });
          rawScore -= 5;
        } else if (ageDays > 30) {
          factors.push({
      ...
      if (rugcheckData.tokenMeta?.mintAuthority) {
        factors.push({
          name: "mint_active",
          impact: -20,
          detail: "Mint authority not revoked",
        });
        rawScore -= 20;
      } else {
        factors.push({
      ...
      if (rugcheckData.tokenMeta?.freezeAuthority) {
        factors.push({
          name: "freeze_active",
          impact: -15,
          detail: "Freeze authority active — can freeze holder wallets",
        });
        rawScore -= 15;
      }
    ...
    factors.push({
      name: "dflow_pending",
      impact: 0,
      detail: "DFlow MCP integration pending for REST API",
    });

    // Clamp score
    const finalScore = Math.max(0, Math.min(100, rawScore));
    const verdict =
      finalScore >= 70 ? "SAFE" : finalScore >= 40 ? "CAUTION" : "UNSAFE";
