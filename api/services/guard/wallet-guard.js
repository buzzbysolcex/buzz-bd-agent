...
const AION_ENDPOINT = process.env.AION_GUARD_URL || 'http://localhost:7700/api/v1/evaluate';

// Evaluate an action before execution
async function evaluate(action) {
  if (!feature('WALLET_GUARD')) {
    return { decision: 'ALLOW', bypassed: true, reason: 'WALLET_GUARD flag disabled' };
  }
  ...
  try {
    ...
    const eventType = decision === 'ALLOW' ? 'guard.allow'
      : decision === 'BLOCK' ? 'guard.block'
      : 'guard.warn';
