...
function createTask(name, agent, payload = {}, dependsOn = []) {
  const db = getDB();
  const pending = db.prepare(`SELECT COUNT(*) as c FROM buzz_tasks WHERE status = 'PENDING'`).get();
  if (pending.c >= 50) return { error: 'circuit_breaker', message: 'Max 50 pending tasks' };

  const status = dependsOn.length === 0 ? 'READY' : _allDepsComplete(db, dependsOn) ? 'READY' : 'PENDING';
  const result = db.prepare(
    `INSERT INTO buzz_tasks (name, agent, payload, depends_on, status) VALUES (?, ?, ?, ?, ?)`
  ).run(name, agent, JSON.stringify(payload), JSON.stringify(dependsOn), status);
...
function completeTask(taskId, result = {}) {
  ...
  const promoted = [];
  const pending = db.prepare(`SELECT id, depends_on FROM buzz_tasks WHERE status = 'PENDING'`).all();
  for (const task of pending) {
    const deps = JSON.parse(task.depends_on || '[]');
    if (deps.includes(taskId) && _allDepsComplete(db, deps)) {
      db.prepare(`UPDATE buzz_tasks SET status = 'READY' WHERE id = ?`).run(task.id);
...
function createPipeline(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    const result = createTask(steps[i].name, steps[i].agent, steps[i].payload || {}, deps);
    ids.push(result.id);
  }
  return { pipeline: ids };
}
...
function createFanOut(parentId, steps) {
  const ids = steps.map(s => createTask(s.name, s.agent, s.payload || {}, [parentId]).id);
...
module.exports = { initTasks, createTask, completeTask, getReadyTasks, claimTask, failTask, getTaskStatus, createPipeline, createFanOut, cleanupTasks };
