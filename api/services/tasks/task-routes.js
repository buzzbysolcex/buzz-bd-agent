...
router.post('/pipeline', (req, res) => {
  if (!req.body.steps?.length) return res.status(400).json({ error: 'steps required' });
  res.json(tm.createPipeline(req.body.steps));
});

router.post('/fanout', (req, res) => {
  if (!req.body.parentId || !req.body.steps?.length) return res.status(400).json({ error: 'parentId + steps required' });
  res.json(tm.createFanOut(req.body.parentId, req.body.steps));
});

router.post('/cleanup', (req, res) => { res.json(tm.cleanupTasks()); });
