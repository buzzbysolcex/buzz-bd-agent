/**
 * Buzz Shield — Initial 20 Drain Patterns
...
const SEED_PATTERNS = [
  {
    ...
    description: 'assign instruction transfers account ownership to attacker program, followed by SOL drain',
  ...
  },
  {
    ...
    description: 'Enumerates all token accounts then transfers each SPL token to attacker address',
    instruction_sequence: JSON.stringify(['getTokenAccountsByOwner', 'transfer_checked']),
    severity: 'critical',
  ...
  },
  {
    ...
    description: 'Uses AdvanceNonceAccount to create never-expiring transaction that can be submitted later after program upgrade',
    instruction_sequence: JSON.stringify(['advanceNonceAccount', 'programCall']),
    severity: 'high',
  ...
  },
  {
    ...
    description: 'Program upgraded within 1 block of transaction, changing execution behavior after simulation',
  ...
  },
  {
    ...
    description: 'Solana Action endpoint returns assign instruction hidden in seemingly normal transaction',
  ...
  },
  {
    ...
    description: 'Creates token account then transfers victim SOL disguised as mint operation',
  ...
  },
  {
    ...
    description: 'Multiple small benign transactions followed by one malicious transaction in sequence',
  ...
  },
  {
    ...
    description: 'npm/pip dependency modified to exfiltrate wallet keys and credentials via postinstall script',
  ...
  },
  {
    ...
    description: 'MCP tool returns manipulated data to trick agent into signing malicious transaction',
  ...
  },
  {
    ...
    description: 'Malicious instructions injected into agent long-term memory to alter future behavior',
  ...
  },
  {
    ...
    description: 'Closes all token accounts to reclaim SOL rent to attacker address',
  ...
  },
  {
    ...
    description: 'Compromised agent sends malicious instructions to other agents in network',
    instruction_sequence: JSON.stringify(['agent_message', 'propagate', 'drain']),
    severity: 'critical',
  ...
  },
  {
    ...
    description: 'Transaction passes wallet simulation but executes differently on-chain',
    instruction_sequence: JSON.stringify(['simulate_benign', 'execute_malicious']),
    severity: 'high',
  ...
  },
  {
    ...
    description: 'Program upgraded, waits 1 block, then drains via new code path',
    instruction_sequence: JSON.stringify(['upgradeProgram', 'wait_block', 'drain']),
    severity: 'critical',
  ...
  },
  {
    ...
    description: 'More than 5 instructions where malicious one is hidden among benign ones',
  ...
  },
  {
    ...
    description: 'Claim airdrop transaction that actually transfers tokens OUT from victim',
    instruction_sequence: JSON.stringify(['claimAirdrop_facade', 'transfer_out']),
    severity: 'medium',
  ...
  },
  {
    ...
    description: 'Program given delegate authority on token account then uses it to drain',
    instruction_sequence: JSON.stringify(['approve_delegate', 'transferChecked']),
    severity: 'high',
  ...
  },
  {
    ...
    description: 'Agent claims to be a known trusted agent with wrong ERC-8004 identity',
  ...
  },
  {
    ...
    description: 'Transaction memo field contains instructions targeting the signing agent LLM',
...
function seedDrainPatterns(db) {
  ...
  const tx = db.transaction(() => {
    for (const p of SEED_PATTERNS) {
      insert.run(p.pattern_id, p.name, p.description, p.instruction_sequence,
                 p.severity, p.source, p.confirmed, p.first_seen);
    }
  });
