...
function initMailbox() {
  ...
  `);
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_mailbox_inbox ON agent_mailbox(to_agent, acked_at) WHERE acked_at IS NULL`,
  );
}
...
function ack(messageId) {
  const db = getDB();
  return db
    .prepare(`UPDATE agent_mailbox SET acked_at = datetime('now') WHERE id = ?`)
    .run(messageId);
}
