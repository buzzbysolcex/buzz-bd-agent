  ...
  try {
    ...
    if (scoringPattern) {
      ...
      if (r) {
        ...
        r.action === "created"
          ? results.skills_created++
          : results.skills_patched++;
      }
    }
...
function analyzeVelocity(db) {
  try {
    ...
    for (const r of rows) {
      stageStats[r.stage] = {
        count: r.count,
        avg_days: Math.round((r.avg_days || 0) * 10) / 10,
      };
      totalTokens += r.count;
    }

    const bottleneck = rows.reduce(
      (max, r) => (r.count > (max?.count || 0) ? r : max),
      null,
    );
    ...
    return {
      ...
      bottleneck: bottleneck
        ? { stage: bottleneck.stage, count: bottleneck.count }
        : null,
...
function applyPattern(skillName, pattern) {
  ...
  if (existing) {
    ...
    return result.success
      ? { skill: skillName, action: "patched", pattern: pattern.observation }
      : null;
  } else {
    // Create new skill
    ...
    return result.success
      ? { skill: skillName, action: "created", pattern: pattern.observation }
      : null;
  }
}
...
function getReflectStatus() {
  try {
    if (fs.existsSync(REFLECT_LOG)) {
      ...
      return {
        success: true,
        last_run: log[log.length - 1] || null,
