...
async function getSmartMoney(chain = "solana") {
  return nansenCmd(
    `research smart-money --chain ${chain} --fields address,label,netflow_7d,token_count`,
  );
}

async function getTokenHolders(address, chain = "solana") {
  return nansenCmd(
    `research token holders ${address} --chain ${chain} --fields address,label,balance,pct`,
  );
}

async function getWalletProfile(address, chain = "solana") {
  return nansenCmd(
    `research profiler labels ${address} --chain ${chain} --fields label,pnl_30d,tokens_held,first_seen`,
  );
}
...
async function enrichToken(address, chain = "solana") {
  ...
  const smartMoneyCount = holderList.filter(
    (h) => h.label && h.label !== "unknown",
  ).length;
  const whaleHolders = holderList.filter(
    (h) => parseFloat(h.pct || 0) > 1,
  ).length;
  const top10Pct = holderList
    .slice(0, 10)
    .reduce((sum, h) => sum + parseFloat(h.pct || 0), 0);
  const labels = holderList
    .filter((h) => h.label && h.label !== "unknown")
    .map((h) => h.label);
  const netflow = holderList.reduce(
    (sum, h) => sum + parseFloat(h.netflow_7d || 0),
    0,
  );
  ...
  `,
    ...
    enrichment.token_address,
    enrichment.chain,
    enrichment.smart_money_netflow,
    enrichment.smart_money_count,
    enrichment.whale_holders,
    enrichment.top10_concentration,
    enrichment.nansen_labels,
    enrichment.raw_json,
  );
...
function getScorerSignals(enrichment) {
  ...
  if (d.smart_money_netflow > 100000)
    signals.push({
      name: "NANSEN_SMART_MONEY_INFLOW",
      impact: 8,
      detail: `Net inflow $${Math.round(d.smart_money_netflow).toLocaleString()}`,
    });
  if (d.smart_money_netflow < -50000)
    signals.push({
      name: "NANSEN_SMART_MONEY_OUTFLOW",
      impact: -10,
      detail: `Net outflow $${Math.round(Math.abs(d.smart_money_netflow)).toLocaleString()}`,
    });
  if (d.smart_money_count >= 3)
    signals.push({
      name: "NANSEN_WHALE_ACCUMULATION",
      impact: 5,
      detail: `${d.smart_money_count} smart money wallets holding`,
    });
  if (d.top10_concentration > 60)
    signals.push({
      name: "NANSEN_HIGH_CONCENTRATION",
      impact: -5,
      detail: `Top 10 hold ${d.top10_concentration}%`,
    });

  const labels = JSON.parse(d.nansen_labels || "[]");
  if (labels.some((l) => /fund|venture|capital|vc/i.test(l)))
    signals.push({
      name: "NANSEN_LABELED_FUND",
      impact: 5,
      detail: `VC/fund detected: ${labels.filter((l) => /fund|venture|capital|vc/i.test(l)).join(", ")}`,
    });
