...
function getState(key) {
  const row = db().prepare('SELECT value FROM pulse_state WHERE key = ?').get(key);
...
function getTickCount() { return parseInt(getState('tick_count') || '0'); }
function getConsecutiveIdle() { return parseInt(getState('consecutive_idle') || '0'); }
function getCurrentInterval() { return parseInt(getState('current_interval_ms') || String(TICK_INTERVAL_MS)); }
...
function decideAction(ctx) {
  ...
  if (feature('PULSE_LOAD_AWARE') && ctx.system_load_pct > HIGH_LOAD_THRESHOLD) {
...
async function executeAction(action) {
  ...
  const timeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error('Blocking budget exceeded (15s)')), BLOCKING_BUDGET_MS)
  );
    ...
    switch (action.action) {
      ...
      case 'process-ready-tasks':
        ...
        for (const task of ready.slice(0, 5)) {
          mailbox.send('pulse-engine', task.agent, 'ALERT', {
            type: 'TASK_READY', taskId: task.id, name: task.name
          });
        }
      ...
      case 'fire-due-crons':
        for (const cronId of (action.cron_ids || []).slice(0, 5)) {
          recordRun(cronId);
          const cron = db().prepare('SELECT * FROM dynamic_crons WHERE id = ?').get(cronId);
          if (cron) {
            mailbox.send('pulse-engine', cron.agent, 'EVENT', {
              type: 'CRON_FIRED', cronId: cron.id, name: cron.name,
...
async function evaluateTick() {
  ...
  if (action.type === 'ACT') {
    ...
    logObservation({
      tick: tickCount, timestamp, decision: 'ACT',
      reason: action.reason, action: action.action, result,
      system_load_pct: ctx.system_load_pct
    });

    emit('pulse-engine', 'pulse.act', {
      tick: tickCount, action: action.action, reason: action.reason
    });
    ...
    logObservation({
      tick: tickCount, timestamp, decision: 'SLEEP',
    ...
    if (consecutiveIdle >= MAX_CONSECUTIVE_IDLE) {
      emit('pulse-engine', 'autodream.trigger', {
        tick: tickCount, idle_ticks: consecutiveIdle
      });
      setState('consecutive_idle', 0);
    }
  }

  return { tick: tickCount, decision: action.type, interval: getCurrentInterval() };
}
...
function initPulse() {
  ...
  logObservation({
    tick: tickCount, timestamp: new Date().toISOString(),
    decision: 'ACT',
  ...
  pulseActive = true;
  console.log(`[PULSE] Engine initialized (reboot #${restarts}). Resuming from tick ${tickCount}. Next tick in ${resumeInterval / 1000}s.`);
