...
router.get('/scores/components/:address', async (req, res) => {
  ...
  try {
    ...
    const safety = safetyResult.status === 'fulfilled' ? safetyResult.value : { score: 0, data: null };
    const wallet = walletResult.status === 'fulfilled' ? walletResult.value : { score: 0, data: null };
    const social = socialResult.status === 'fulfilled' ? socialResult.value : { score: 0, data: null };
    ...
    res.json({
      ...
      components: {
        scanner: { score: 0, data: scannerResult.data, duration_ms: scannerResult.duration_ms },
        safety: { score: safety.score, data: safety.data, duration_ms: safety.duration_ms },
        wallet: { score: wallet.score, data: wallet.data, duration_ms: wallet.duration_ms },
        social: { score: social.score, data: social.data, duration_ms: social.duration_ms },
        scorer: { score: scorerResult.score, data: scorerResult.data, duration_ms: scorerResult.duration_ms },
...
router.get('/simulate/data/:address', async (req, res) => {
  ...
  try {
    ...
    const scanner = scannerResult.status === 'fulfilled' ? scannerResult.value : { score: 0, data: null };
    const safety = safetyResult.status === 'fulfilled' ? safetyResult.value : { score: 0, data: null };
    const wallet = walletResult.status === 'fulfilled' ? walletResult.value : { score: 0, data: null };
    const social = socialResult.status === 'fulfilled' ? socialResult.value : { score: 0, data: null };
    ...
    try {
      pipelineToken = db.prepare('SELECT * FROM pipeline_tokens WHERE address = ?').get(address);
    } catch {}
    try {
      previousSimulations = db.prepare(
...
router.post('/outcomes/:address', (req, res) => {
  const { address } = req.params;
  const { chain, day, price_usd, market_cap, volume_24h, holder_count, notes } = req.body;
  const db = getDB();
  ...
  try {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(address, chain || 'solana', day || 30, price_usd, market_cap, volume_24h, holder_count, notes);
  ...
  try {
    ...
    for (const token of tokens) {
      ...
      if (prediction && outcomes.length > 0) {
        ...
        const actualPerformance = latestOutcome.price_usd > 0 ? 'positive' : 'negative';
        ...
        calibrationResults.push({
          ...
          accuracy: (predictedScore >= 50 && actualPerformance === 'positive') ||
                    (predictedScore < 50 && actualPerformance === 'negative') ? 'correct' : 'incorrect',
        });
      }
    }
    ...
    `).run(
      ...
      calibrationResults.length > 0
        ? Math.round((calibrationResults.filter(r => r.accuracy === 'correct').length / calibrationResults.length) * 100)
        : 0,
    ...
    res.json({
      ...
      correct: calibrationResults.filter(r => r.accuracy === 'correct').length,
      incorrect: calibrationResults.filter(r => r.accuracy === 'incorrect').length,
      accuracy_pct: calibrationResults.length > 0
        ? Math.round((calibrationResults.filter(r => r.accuracy === 'correct').length / calibrationResults.length) * 100)
        : 0,
...
router.get('/calibration/results', (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: err.message, code: 'CALIBRATION_RESULTS_ERROR' });
  }
});
