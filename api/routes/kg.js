...
router.get("/entity/:id", (req, res) => {
  try {
    ...
    const entity = db
      .prepare("SELECT * FROM kg_entities WHERE entity_id = ?")
      .get(entityId);
    if (!entity) {
      return res
        .status(404)
  ...
  try {
    ...
    if (!entities.length) {
      return res.json({
        ...
        address,
        message:
          "No graph data for this token. Use ?build=true to build on demand.",
  ...
  try {
    ...
    const enriched = deployers.map((d) => {
      ...
      const stmt = db.prepare(
        "SELECT entity_id, name, metadata_json FROM kg_entities WHERE entity_id = ?",
      );
