...
router.post("/", (req, res) => {
  ...
  if (!id || !name || !schedule) {
    return res.status(400).json({
      ...
      message: "id, name, schedule required",
    });
  }
  ...
  try {
    ...
    if (err.message.includes("UNIQUE") || err.message.includes("PRIMARY")) {
      return res.status(409).json({
        error: "duplicate",
        message: `Cron job '${id}' already exists`,
      });
    }
    throw err;
  }
...
router.patch("/:id", (req, res) => {
  ...
  if (updates.length === 0) {
    return res
      .status(400)
      .json({ error: "no_updates", message: "No fields to update" });
  }

  const result = db
    .prepare(`UPDATE cron_jobs SET ${updates.join(", ")} WHERE id = ?`)
    .run(...params);
