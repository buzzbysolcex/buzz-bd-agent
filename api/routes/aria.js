...
const { filterTokens, qualifyToken, DEFAULT_CRITERIA } = require('../services/aria/aria-filter');
...
function upsertToken(token) {
  ...
  stmt.run(
    ...
    token.discovery?.source, token.discovery?.discovery_type, token.discovery?.discovered_at,
    token.market?.mcap_circulating, token.market?.mcap_fdv, token.market?.fdv_gap,
    token.market?.volume_24h, token.market?.liquidity_usd, token.market?.pair_age_days,
    token.market?.exchange_count, token.safety?.honeypot ? 1 : 0,
    qual.pass ? 1 : 0, token.classification?.composite_score,
    token.metadata?.enriched_at, JSON.stringify(token.metadata?.enrichment_sources || []),
...
router.get('/filter', (req, res) => {
  try {
    ...
    if (req.query.liquidity_min) criteria.liquidity_min = parseFloat(req.query.liquidity_min);
    if (req.query.age_min_days) criteria.age_min_days = parseFloat(req.query.age_min_days);
...
router.get('/enrich/:address', async (req, res) => {
  try {
    ...
    const db = getDB();
    ensureTable();
    const existing = db.prepare('SELECT raw_json FROM aria_tokens WHERE address = ? AND chain = ?').get(address, chain);
    let token;
    if (existing?.raw_json) {
      try { token = JSON.parse(existing.raw_json); } catch { token = null; }
    }
    if (!token) {
    ...
    try { upsertToken(result.token); } catch (e) { /* best effort */ }
...
router.get('/status', (req, res) => {
  try {
    ...
    const qualified = db.prepare('SELECT COUNT(*) as c FROM aria_tokens WHERE bd_qualified = 1').get();
    const enriched = db.prepare('SELECT COUNT(*) as c FROM aria_tokens WHERE enriched_at IS NOT NULL').get();
    const last24h = db.prepare("SELECT COUNT(*) as c FROM aria_tokens WHERE discovered_at >= datetime('now', '-1 day')").get();
    const lastScan = db.prepare('SELECT MAX(discovered_at) as ts FROM aria_tokens').get();

    const bySource = db.prepare('SELECT source, COUNT(*) as count FROM aria_tokens GROUP BY source').all();
    const byChain = db.prepare('SELECT chain, COUNT(*) as count FROM aria_tokens GROUP BY chain').all();
