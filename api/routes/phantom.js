...
router.get("/price/:token", async (req, res) => {
  ...
  if (!hasSession()) {
    return res.status(503).json({
      ...
      next_action:
        "Run npx @phantom/mcp-server on Mac, copy session.json to Hetzner",
    });
  }
  ...
  try {
    ...
    res.json({
      ...
      message:
        "Phantom session found. MCP bridge module needed to query swap quotes.",
      next_step:
        "Build api/services/phantom-bridge.js to connect to @phantom/mcp-server",
    });
  } catch (err) {
...
router.get("/wallet", async (req, res) => {
  ...
  res.json({
    status: "session_exists_but_mcp_bridge_not_built",
    message:
      "Phantom session found. Build phantom-bridge.js for wallet address queries.",
    app_id: PHANTOM_APP_ID,
  });
});
