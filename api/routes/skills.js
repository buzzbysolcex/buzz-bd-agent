...
const {
  ...
  patchSkill,
  listSkills,
} = require("../services/tools/skill-create");
...
router.get("/", (req, res) => {
  try {
    ...
    if (fs.existsSync(STATIC_SKILLS_DIR)) {
      staticCount = fs
        .readdirSync(STATIC_SKILLS_DIR, { withFileTypes: true })
        .filter((d) => d.isDirectory() && d.name !== "learned").length;
    }
    const learned = listSkills();
    res.json({
      success: true,
      total: staticCount + learned.length,
...
router.get("/:name", (req, res) => {
  try {
    ...
    if (fs.existsSync(learnedPath)) {
      skillPath = learnedPath;
      type = "learned";
    } else if (fs.existsSync(staticPath)) {
      skillPath = staticPath;
      type = "static";
    }
    if (!skillPath)
      return res
        .status(404)
        .json({ success: false, error: `Skill "${name}" not found` });
...
router.get("/:name/history", (req, res) => {
  try {
    ...
    if (!fs.existsSync(SKILL_PATCHES_LOG))
      return res.json({ success: true, name, history: [] });
