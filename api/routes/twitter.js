...
router.get("/status", (req, res) => {
  const dailyCount = readJSON(path.join(TWITTER_DATA_DIR, "daily-count.json"), {
    date: null,
    count: 0,
  });
...
router.get("/stats", (req, res) => {
  const history = readJSON(
    path.join(TWITTER_DATA_DIR, "scan-history.json"),
    [],
  );
  const leads = readJSON(path.join(TWITTER_DATA_DIR, "twitter-leads.json"), []);
  const deploys = readJSON(
    path.join(TWITTER_DATA_DIR, "twitter-deploys.json"),
    [],
  );
...
router.post("/brain/scan", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "twitter_brain_error", message: err.message });
  }
});
