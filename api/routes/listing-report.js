...
router.get('/listing-report/:addressOrTicker', (req, res) => {
  try {
    ...
    if (!param) {
      return res.json({ success: false, error: 'addressOrTicker parameter required' });
    }

    // 1. Look up in pipeline_tokens by address OR ticker
    let token = null;
    try {
      token = db.prepare('SELECT * FROM pipeline_tokens WHERE address = ?').get(param);
    } catch (e) { /* table may not exist */ }

    if (!token) {
      try {
        token = db.prepare('SELECT * FROM pipeline_tokens WHERE UPPER(ticker) = UPPER(?)').get(param);
      } catch (e) { /* table may not exist */ }
    }
    ...
    let scores = null;
    try {
      scores = db.prepare('SELECT * FROM token_scores WHERE address = ?').get(token.address);
    } catch (e) { /* table may not exist */ }

    // 3. Query persona_signals (latest per persona)
    let personaSignals = [];
    try {
      personaSignals = db.prepare(`
    ...
    const report = {
      ...
      persona_signals: personaSignals.length > 0 ? personaSignals.map(ps => ({
