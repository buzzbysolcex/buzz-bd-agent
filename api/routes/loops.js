...
router.get("/runs/:id", (req, res) => {
  const db = getDB();
  const run = db
    .prepare("SELECT * FROM loop_cron_runs WHERE id = ?")
    .get(req.params.id);
  if (!run)
    return res.status(404).json({ error: "not_found", code: "RUN_NOT_FOUND" });
  const outputs = db
    .prepare(
      "SELECT * FROM loop_cron_outputs WHERE run_id = ? ORDER BY created_at",
    )
    .all(run.id);
...
router.post("/trigger/:type", (req, res) => {
  ...
  try {
    ...
    else if (type === "discovery_alert")
      result = loops.generateDiscoveryAlert();
    else if (type === "evening_recap") result = loops.generateEveningRecap();
    else
      return res
        .status(400)
        .json({ error: "invalid_type", code: "INVALID_LOOP_TYPE" });
...
router.get("/authority", (req, res) => {
  ...
  const permissions = agent
    ? authority.getAgentPermissions(agent)
    : authority.getAllPermissions();
...
router.post("/authority/check", (req, res) => {
  ...
  if (!agent_name || !action)
    return res
      .status(400)
      .json({ error: "missing_fields", code: "MISSING_REQUIRED_FIELDS" });
...
router.post("/authority/grant", (req, res) => {
  ...
  if (!agent_name || !action)
    return res
      .status(400)
      .json({ error: "missing_fields", code: "MISSING_REQUIRED_FIELDS" });
  authority.grantPermission(agent_name, action, {
