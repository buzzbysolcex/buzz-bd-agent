...
router.get("/agents", (req, res) => {
  try {
    const db = getDB();
    const agents = db
      .prepare("SELECT name, status, last_heartbeat, last_error FROM agents")
      .all();
...
router.get("/storage", (req, res) => {
  ...
  try {
    ...
    const dfOut = execSync("df -k /data 2>/dev/null", { timeout: 2000 })
      .toString()
      .split("\n")[1];
    if (dfOut) {
  ...
  try {
    ...
    const latestScan = files.includes("latest-scan.json")
      ? fs
          .statSync(path.join(pipelineDir, "latest-scan.json"))
          .mtime.toISOString()
      : null;
    result.pipeline = {
