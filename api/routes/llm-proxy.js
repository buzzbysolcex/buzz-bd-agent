...
module.exports = function (db) {
  ...
  function authMiddleware(req, res, next) {
    ...
    if (!adminKey) {
      return res.status(500).json({
        error: "server_config",
        message: "BUZZ_API_ADMIN_KEY not configured",
      });
    }
  ...
  const CALLER_PATTERNS = [
    ...
    [
      ["prayer", "salat", "sholat", "fajr", "dhuhr", "maghrib", "isha"],
      "prayer",
    ],
    ...
    [
      ["health check", "system health", "cron health", "api health"],
      "health-check",
    ],
  ...
  router.post("/completions", async (req, res) => {
    try {
      const caller =
        req.headers["x-buzz-caller"] ||
        req.body._caller ||
        inferCaller(req.body);
  ...
  router.post("/anthropic/*", async (req, res) => {
    try {
      ...
      const result = await proxy.proxyAnthropicRequest(
        subPath,
        req.body,
        caller,
      );
    ...
    try {
      ...
      if (isNaN(balance) || balance < 0) {
        return res.status(400).json({
          ...
          message: "Provide ?balance=<number>",
        });
      }
