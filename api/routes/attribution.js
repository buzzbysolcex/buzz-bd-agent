...
router.get('/token/:address', (req, res) => {
  ...
  let sql = 'SELECT * FROM pipeline_revenue_attribution WHERE token_address = ?';
  const params = [req.params.address];
  if (chain) { sql += ' AND chain = ?'; params.push(chain); }

  const records = db.prepare(sql).all(...params);
  if (records.length === 0) {
    return res.status(404).json({ error: 'not_found', code: 'TOKEN_NOT_FOUND' });
  }
  res.json(records.length === 1 ? records[0] : { count: records.length, records });
});

router.get('/funnel', (req, res) => {
  const db = getDB();
  const stages = ['discovered', 'scanned', 'scored', 'prospect', 'contacted', 'negotiating', 'approved', 'listed'];
  ...
  const funnel = stages.map((stage, i) => {
    ...
    return {
      stage,
      count,
      drop_off_rate: i === 0 ? 0 : (prevCount > 0 ? Math.round((1 - count / prevCount) * 100) : 100)
    };
  });

  const rejected = countMap['rejected'] || 0;
  res.json({ funnel, rejected, total: Object.values(countMap).reduce((s, v) => s + v, 0) });
});
