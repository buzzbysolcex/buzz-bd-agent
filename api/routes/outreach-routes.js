...
router.post('/queue', (req, res) => {
  const { tokenAddress, chain, contactEmail, subject, body, trustAction } = req.body;
  if (!tokenAddress || !contactEmail || !subject || !body) {
    return res.status(400).json({ error: 'Missing required fields' });
  }
  const result = outreach.queueOutreach(tokenAddress, chain, contactEmail, subject, body, trustAction || 'APPROVAL_REQUIRED');
  res.json(result);
});
