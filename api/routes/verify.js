...
router.get("/status/:contractAddress", (req, res) => {
  ...
  res.json({
    status: "UNKNOWN",
    message:
      "No verification record. Run GET /api/v1/verify/" + contractAddress,
  });
});
...
router.post("/resolve/:contractAddress", (req, res) => {
  ...
  if (result.changes === 0) {
    return res
      .status(404)
      .json({ error: "not_found", code: "NO_ACTIVE_QUARANTINE" });
  }
  res.json({
    status: "resolved",
...
router.get("/log/history", (req, res) => {
  ...
  let sql =
    "SELECT id, contract_address, chain, token_name, token_symbol, check1_pass, check2_pass, check3_pass, overall_status, mismatches, verified_at FROM verification_log";
  const params = [];
  if (status) {
    sql += " WHERE overall_status = ?";
    params.push(status);
  }
