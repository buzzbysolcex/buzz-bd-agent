...
async function runEVMAudit(address, chain, scannerData) {
  ...
  if (scannerData) {
    ...
    if (
      dex.includes("uniswap") ||
      dex.includes("pancake") ||
      dex.includes("sushi")
    )
      selectedSkills.push("evm-audit-defi-amm");
    if (name.includes("vault") || name.includes("yield"))
      selectedSkills.push("evm-audit-erc4626");
    if (name.includes("stake") || name.includes("restake"))
      selectedSkills.push("evm-audit-defi-staking");
    if (name.includes("nft")) selectedSkills.push("evm-audit-erc721");
  }
  const uniqueSkills = [...new Set(selectedSkills)];
  console.log(
    `[ethskills] Loading ${uniqueSkills.length} checklists for ${address} on ${chain}`,
  );
  ...
  const results = await Promise.all(
    uniqueSkills.map(async (skill) => {
      ...
      if (content) {
        checklistsLoaded.push(skill);
        return {
          skill,
          content,
          itemCount: (content.match(/^- \[/gm) || []).length,
        };
      } else {
        checklistsFailed.push(skill);
        return null;
      }
    }),
  );
  ...
  if (scannerData) {
    if (scannerData.websites && scannerData.websites.length > 0) {
      auditScore += 5;
      auditFactors.push({
        name: "has_website",
        impact: 5,
        detail: scannerData.websites[0]?.url || "found",
      });
    }
    if (scannerData.socials && scannerData.socials.length > 0) {
      ...
      if (tw) {
        auditScore += 5;
        auditFactors.push({ name: "has_twitter", impact: 5, detail: tw.url });
      }
      if (tg) {
        auditScore += 3;
        auditFactors.push({ name: "has_telegram", impact: 3, detail: tg.url });
      }
    }
    if (scannerData.cmc_listed) {
      auditScore += 10;
      auditFactors.push({
        name: "cmc_listed",
        impact: 10,
        detail: `CMC rank #${scannerData.cmc?.cmc_rank || "unranked"}`,
      });
    }
    if (scannerData.pairs_count && scannerData.pairs_count > 5) {
      auditScore += 5;
      auditFactors.push({
        name: "multi_pair",
        impact: 5,
        detail: `${scannerData.pairs_count} trading pairs`,
      });
    }
    if (scannerData.pair_created_at) {
      const ageDays = Math.floor(
        (Date.now() - scannerData.pair_created_at) / 86400000,
      );
      if (ageDays > 180) {
        auditScore += 5;
        auditFactors.push({
        ...
        auditFactors.push({
          name: "token_very_new",
          impact: -10,
          detail: `${ageDays}d old — high risk`,
        });
      } else if (ageDays < 30) {
        auditScore -= 5;
        auditFactors.push({
    ...
    if (scannerData.volume_24h && scannerData.liquidity_usd) {
      ...
      if (ratio > 10) {
        auditScore -= 15;
        auditFactors.push({
          name: "wash_trading_risk",
          impact: -15,
          detail: `Vol/Liq ${ratio.toFixed(1)}x — suspicious`,
        });
      } else if (ratio > 5) {
        auditScore -= 5;
        auditFactors.push({
          name: "high_vol_ratio",
          impact: -5,
          detail: `Vol/Liq ${ratio.toFixed(1)}x — elevated`,
        });
      } else if (ratio >= 0.1 && ratio <= 3) {
        auditScore += 5;
        auditFactors.push({
          name: "healthy_vol_ratio",
          impact: 5,
          detail: `Vol/Liq ${ratio.toFixed(1)}x — healthy`,
        });
      }
    }
    if (scannerData.txns_24h_buys && scannerData.txns_24h_sells) {
      const bsr =
        scannerData.txns_24h_buys / Math.max(scannerData.txns_24h_sells, 1);
      if (bsr > 5) {
        auditScore -= 5;
        auditFactors.push({
          name: "buy_pressure_extreme",
          impact: -5,
          detail: `Buy/Sell ${bsr.toFixed(1)}x — potential manipulation`,
        });
      } else if (bsr < 0.2) {
        auditScore -= 10;
        auditFactors.push({
          name: "heavy_selling",
          impact: -10,
          detail: `Buy/Sell ${bsr.toFixed(1)}x — dump risk`,
        });
      }
    }
    if (scannerData.price_change_24h !== undefined) {
      if (scannerData.price_change_24h < -50) {
        auditScore -= 15;
        auditFactors.push({
          name: "price_crash",
          impact: -15,
          detail: `${scannerData.price_change_24h.toFixed(1)}% 24h — possible rug`,
        });
      } else if (scannerData.price_change_24h < -30) {
        auditScore -= 5;
        auditFactors.push({
          name: "price_decline",
          impact: -5,
          detail: `${scannerData.price_change_24h.toFixed(1)}% 24h — significant`,
        });
      }
    }
    if (scannerData.signal_count && scannerData.signal_max) {
      if (scannerData.signal_count / scannerData.signal_max >= 0.8) {
        auditScore += 5;
        auditFactors.push({
          name: "strong_signals",
          impact: 5,
          detail: `${scannerData.signal_count}/${scannerData.signal_max} discovery signals`,
        });
      }
    }
  }
  ...
  console.log(
    `[ethskills] Audit: ${auditScore}/100 | ${checklistsLoaded.length} checklists | ${auditFactors.length} factors | ${duration}ms`,
  );

  return {
    score: auditScore,
    verdict: auditScore >= 70 ? "PASS" : auditScore >= 50 ? "REVIEW" : "FAIL",
    checklists_loaded: checklistsLoaded,
    checklists_failed: checklistsFailed,
    total_checklist_items: loadedChecklists.reduce(
      (sum, c) => sum + (c?.itemCount || 0),
      0,
    ),
    skills_selected: uniqueSkills.length,
...
router.post("/score-token", validateScoreRequest, async (req, res) => {
  ...
  console.log(
    `[${requestId}] 🎯 /score-token START | ${address} on ${chain} | depth=${depth}`,
  );
  ...
  try {
    ...
    if (cached && depth !== "deep") {
      ...
      return res.json({
        ...
        score: {
          total: cached.score_total,
          verdict: cached.verdict,
          verdict_emoji: getVerdictEmoji(cached.verdict),
        },
    ...
    const { score, verdict, breakdown, subAgentResults, agentTimings, errors } =
      orchestrationResult;
    ...
    if (isEVM) {
      ...
      ethskillsResult = await runEVMAudit(
        address,
        chain,
        subAgentResults.scanner?.data || null,
      );
      console.log(
        `[${requestId}] 🔐 ethskills: ${ethskillsResult.score}/100 (${ethskillsResult.verdict}) | ${ethskillsResult.checklists_loaded.length} checklists`,
      );
    }
    ...
    const response = {
      ...
      evm_audit: ethskillsResult
        ? {
            score: ethskillsResult.score,
            verdict: ethskillsResult.verdict,
            ...
            factors: ethskillsResult.factors,
            duration_ms: ethskillsResult.duration_ms,
            source: ethskillsResult.source,
            note: ethskillsResult.note,
      ...
      sub_agents: {
        ...
        safety: {
          ...
          weight: 0.3,
          weighted_score: (subAgentResults.safety?.score || 0) * 0.3,
        ...
        wallet: {
          ...
          weight: 0.3,
          weighted_score: (subAgentResults.wallet?.score || 0) * 0.3,
        ...
        social: {
          ...
          weight: 0.2,
          weighted_score: (subAgentResults.social?.score || 0) * 0.2,
        ...
        scorer: {
          ...
          weight: 0.2,
          weighted_score: (subAgentResults.scorer?.score || 0) * 0.2,
          duration_ms: agentTimings.scorer || 0,
          data: subAgentResults.scorer?.data || null,
        },
      },
      agents_completed: Object.values(subAgentResults).filter(
      ...
      meta: {
        ...
        ethskills_enabled: ethskillsResult !== null,
      },
    };
    ...
    reportToAgentProof({
      ...
      console.error(`[${requestId}] AgentProof report failed:`, err.message),
    );

    // ─── Step 7: Return ───
    const evmSuffix = isEVM ? ` | ethskills:${ethskillsResult?.score}/100` : "";
    console.log(
      `[${requestId}] ✅ Score: ${score}/100 (${verdict}) | ${Date.now() - startTime}ms | ${response.agents_completed}/5 agents${evmSuffix}`,
    );
    ...
    return res.status(500).json({
      ...
      message: err.message,
      meta: { request_id: requestId, duration_ms: Date.now() - startTime },
    });
  }
});
...
router.get("/score-token/:address", async (req, res) => {
  ...
  try {
    ...
    const latest = db
      .prepare(
        "SELECT * FROM token_scores WHERE address = ? AND chain = ? ORDER BY created_at DESC LIMIT 1",
      )
      .get(address.toLowerCase(), chain.toLowerCase());
    if (!latest)
    ...
    return res.json({
      ...
      score: {
        ...
        verdict_emoji: getVerdictEmoji(latest.verdict),
      },
      agents_completed: latest.agents_completed,
...
router.get("/score-token/history/:address", async (req, res) => {
  ...
  try {
    ...
    const history = db
      .prepare(
        "SELECT id, score_total as score, verdict, processing_time_ms as duration_ms, agents_completed, created_at FROM token_scores WHERE address = ? AND chain = ? ORDER BY created_at DESC LIMIT ?",
      )
...
router.get("/score-token/leaderboard", async (req, res) => {
  ...
  try {
    const db = getDB();
    let query =
      "SELECT address, chain, MAX(score_total) as best_score, verdict, COUNT(*) as scan_count, MAX(created_at) as last_scanned FROM token_scores WHERE created_at > datetime('now', ?)";
