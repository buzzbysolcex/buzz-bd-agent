...
router.get("/:address", (req, res) => {
  try {
    ...
    try {
      tech = db
        .prepare(
          "SELECT * FROM technical_analysis_cache WHERE token_address = ?",
        )
        .get(address);
    } catch {}
    ...
    const readiness_pct = Math.min(
      100,
      Math.round(pipelineScore * 0.85 + (sim?.confidence || 0) * 15),
    );
    ...
    if (!mcap && pipelineScore) {
      mcap =
        pipelineScore >= 85 ? 50000000 : pipelineScore >= 70 ? 5000000 : 500000;
      volume =
        pipelineScore >= 85 ? 5000000 : pipelineScore >= 70 ? 1000000 : 100000;
      liquidity =
        pipelineScore >= 85 ? 2000000 : pipelineScore >= 70 ? 500000 : 50000;
    ...
    if (pipelineScore >= 70)
      strengths.push({
        category: "score",
        label: "Strong Pipeline Score (" + pipelineScore + "/100)",
        status: "pass",
      });
    if (volume >= 100000)
      strengths.push({
    ...
    if (holders >= 1000)
      strengths.push({
        category: "holders",
        label: "Good Holder Count (" + holders.toLocaleString() + ")",
        status: "pass",
      });
    if (
      sim?.consensus === "BULLISH" ||
      (sim?.recommendation && sim.recommendation.includes("PROCEED"))
    )
      strengths.push({
        category: "simulation",
        label: "AI Simulation: PROCEED",
        status: "pass",
      });
    ...
    if (pipelineScore < 70)
      weaknesses.push({
        category: "score",
        label: "Low Pipeline Score",
        status: "warning",
        recommendation:
          "Improve safety, wallet distribution, and social metrics",
      });
    if (volume < 100000)
      weaknesses.push({
        category: "volume",
        ...
        status: "warning",
        recommendation: "Sustain daily volume above $100K",
      });
    if (liquidity < 50000)
      weaknesses.push({
        category: "liquidity",
        label: "Low Liquidity",
        status: "warning",
        recommendation: "Add liquidity to DEX pools (target $50K+)",
      });
    if (holders < 1000)
      weaknesses.push({
        category: "holders",
        ...
        status: "warning",
        recommendation: "Grow organic holder base to 1,000+",
      });
    weaknesses.push({
      category: "audit",
      label: "No Formal Audit Detected",
      status: "warning",
      recommendation:
        "Get a smart contract audit from CertiK, Hacken, or similar",
    });
    ...
    const exchange_scores = {
      ...
      community_health:
        pipelineScore >= 85 ? 70 : pipelineScore >= 70 ? 55 : 30,
    ...
    if (tech) {
      try {
        ...
        technical = {
          ...
          macd: ind.macd?.crossover || null,
          volume_trend: ind.volumeTrend?.trend || "unknown",
          score: tech.technical_score || 50,
        };
      } catch {}
    }
    ...
    if (sim) {
      ...
      // Parse JSON fields
      try {
        if (typeof sim.consensus === "string" && sim.consensus.startsWith("{"))
          simulation.consensus = JSON.parse(sim.consensus).overall;
      } catch {}
    }
    ...
    res.json({
      ...
      in_pipeline: !!token,
      disclaimer:
        "Estimated thresholds based on industry research. Actual requirements vary by exchange. Not financial advice.",
    });
  } catch (err) {
