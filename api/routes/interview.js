...
const { listInterviewableAgents, interviewAgent } = require('../lib/agent-interviewer');
...
router.get('/:simulationId/agents', (req, res) => {
  try {
    ...
    if (!simulationId) {
      return res.status(400).json({ error: 'simulationId is required', code: 'MISSING_SIMULATION_ID' });
    }
...
router.post('/:simulationId/:agentIndex', async (req, res) => {
  try {
    ...
    if (isNaN(parsedIndex) || parsedIndex < 0) {
      return res.status(400).json({ error: 'agentIndex must be a non-negative integer', code: 'INVALID_AGENT_INDEX' });
    }

    if (!question || typeof question !== 'string' || question.trim().length === 0) {
      return res.status(400).json({ error: 'question is required and must be a non-empty string', code: 'MISSING_QUESTION' });
    }

    // Validate conversation_history format if provided
    const history = Array.isArray(conversation_history) ? conversation_history : [];

    const result = await interviewAgent(simulationId, parsedIndex, question.trim(), history);
