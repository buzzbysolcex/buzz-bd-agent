...
function scoreAge(ageDays) {
  ...
  if (
    ageDays >= scoring.new_but_stable.min &&
    ageDays < scoring.new_but_stable.max
  ) {
...
function scoreCommunity(socials) {
  ...
  if (socials.twitterFollowers >= factors.twitter_followers.threshold) {
    ...
    score +=
      15 *
      factors.twitter_followers.weight *
      (socials.twitterFollowers / factors.twitter_followers.threshold);
  }
  ...
  if (socials.telegramMembers >= factors.telegram_members.threshold) {
    ...
    score +=
      15 *
      factors.telegram_members.weight *
      (socials.telegramMembers / factors.telegram_members.threshold);
  }
...
function applyCatalysts(catalysts) {
  ...
  // Bonuses
  if (catalysts.hackathonWinner) {
    bonus += bonuses.hackathon_winner;
    applied.push("+hackathon");
  }
  if (catalysts.viralMoment) {
    bonus += bonuses.viral_moment;
    applied.push("+viral");
  }
  if (catalysts.kolMention) {
    bonus += bonuses.kol_mention;
    applied.push("+kol");
  }
  if (catalysts.aixbtHighConviction) {
    bonus += bonuses.aixbt_high_conviction;
    applied.push("+aixbt");
  }
  if (catalysts.dexscreenerTrending) {
    bonus += bonuses.dexscreener_top_trending;
    applied.push("+trending");
  }
  if (catalysts.x402Verified) {
    bonus += bonuses.x402_verified;
    applied.push("+x402");
  }

  // Penalties
  if (catalysts.majorCexListed) {
    bonus += penalties.major_cex_listed;
    applied.push("-cex");
  }
  if (catalysts.liquidityDropping) {
    bonus += penalties.liquidity_dropping_fast;
    applied.push("-liq");
  }
  if (catalysts.teamInactive) {
    bonus += penalties.team_inactive_7d;
    applied.push("-inactive");
  }
  if (catalysts.recentDump) {
    bonus += penalties.recent_dump_50pct;
    applied.push("-dump");
  }
  if (catalysts.suspiciousVolume) {
    bonus += penalties.suspicious_volume;
    applied.push("-sus");
  }
  if (catalysts.x402Blocked) {
    bonus += penalties.x402_blocked;
...
function getAction(score) {
  ...
  if (score >= config.thresholds.qualified.min)
    return config.thresholds.qualified.action;
  if (score >= config.thresholds.watch.min)
    return config.thresholds.watch.action;
...
function shouldAutoReject(token) {
  ...
  if (token.liquidity < criteria.liquidity_below)
    return { reject: true, reason: "liquidity_too_low" };
  if (token.ageDays !== null && token.ageDays * 24 < criteria.age_hours_below)
    return { reject: true, reason: "too_new" };
  if (token.volumeToMcapRatio > criteria.volume_to_mcap_ratio_above)
    return { reject: true, reason: "suspicious_volume" };
