  ...
  agent: {
    ...
    services: [
      ...
      // Buzz wallet on Solana
      {
        ...
        value: "5iC7pGyzqpXD2xTK4Ww7zKRDVo9cceyHNeKBTiemo5Jp",
      },
    ],
...
async function main() {
  ...
  if (!process.env.SOLANA_PRIVATE_KEY) {
    console.error(
      "❌ SOLANA_PRIVATE_KEY not set. Export Lobster wallet key as JSON array.",
    );
  ...
  if (!hasPinata) {
    console.warn(
      "⚠️  PINATA_JWT not set. Will use local IPFS node (http://localhost:5001).",
    );
  ...
  if (atom) {
    ...
    console.log(
      `   - Trust Tier: ${trustTierToString(atom.getTrustTier())} (${atom.trust_tier})`,
    );
  ...
  console.log("  🔗 Explore:");
  console.log(
    `  ├─ 8004scan:    https://8004scan.io/agent/${agentAsset.toBase58()}`,
  );
...
main().catch((err) => {
  ...
  if (err.message?.includes("insufficient")) {
    console.error(
      "   → Need ~0.02 SOL in Lobster wallet for registration + metadata",
    );
