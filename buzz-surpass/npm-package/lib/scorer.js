...
const RULES = {
    ...
    check: (data) => {
      ...
      if (gap > 5)
        return {
          score: -15,
          triggered: true,
          detail: `FDV/mcap gap: ${gap.toFixed(1)}x`,
        };
      if (gap > 3)
        return {
          score: -8,
  ...
  SECURITY_PENALTY: {
    ...
    check: (data) => {
      ...
      if (flags.includes("honeypot"))
        return { score: -50, triggered: true, detail: "Honeypot detected" };
      if (flags.length > 2)
        return {
          score: -25,
          triggered: true,
          detail: `${flags.length} security flags`,
        };
      if (flags.length > 0)
        return { score: -10, triggered: true, detail: flags.join(", ") };
      return { score: 0, triggered: false };
    },
  },
    ...
    check: (data) => {
      if (!data.liquidity || data.liquidity < 10000) {
        return {
          score: -20,
          triggered: true,
          detail: `Low liquidity: $${(data.liquidity || 0).toLocaleString()}`,
        };
      }
      if (data.liquidity > 1000000)
        return { score: 10, triggered: true, detail: "Strong liquidity" };
      return { score: 0, triggered: false };
    },
  },
    ...
    check: (data) => {
      if (data.volume24h < 5000) {
        return {
          score: -15,
          triggered: true,
          detail: `Low volume: $${(data.volume24h || 0).toLocaleString()}`,
        };
      }
      if (data.volume24h > 1000000)
        return { score: 10, triggered: true, detail: "High volume" };
      return { score: 0, triggered: false };
    },
  },
  ...
  CTO_FLAG: {
    ...
    check: (data) => {
      ...
      if (
        data.cto ||
        data.description?.toLowerCase().includes("community takeover")
      ) {
        return {
          score: -10,
          triggered: true,
          detail: "Community takeover detected",
        };
      }
      return { score: 0, triggered: false };
    },
    ...
    check: (data) => {
      ...
      if (ratio > 10) {
        return {
          score: -15,
          triggered: true,
          detail: `Vol/liq ratio: ${ratio.toFixed(1)}x (suspicious)`,
        };
      }
      return { score: 0, triggered: false };
    },
...
function calculateScore(tokenData) {
  ...
  return {
    ...
    breakdown: {
      liquidity: {
        score: Math.min(100, (tokenData.liquidity || 0) / 10000),
        value: `$${(tokenData.liquidity || 0).toLocaleString()}`,
      },
      volume_24h: {
        score: Math.min(100, (tokenData.volume24h || 0) / 50000),
        value: `$${(tokenData.volume24h || 0).toLocaleString()}`,
      },
      security: {
        score: flags.length === 0 ? 90 : Math.max(0, 90 - flags.length * 20),
        penalties: flags,
      },
      deployer: {
        score: tokenData.identityVerified ? 80 : 40,
        identity: tokenData.identityVerified ? "VERIFIED" : "ANON",
      },
      age_days: tokenData.ageDays || 0,
      fdv_gap:
        tokenData.fdv && tokenData.marketCap
          ? (tokenData.fdv / tokenData.marketCap).toFixed(1)
          : "N/A",
    },
    llm_cost: "$0.00",
