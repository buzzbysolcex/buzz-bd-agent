...
<div class="container">
  ...
  <table>
    ...
    ${NEW_TOKENS.map(t => `<tr><td style="color:#fff;font-weight:700">${t.name}</td><td>${t.chain}</td><td style="color:#00ff41">${t.score}</td><td><span class="tag tag-${t.status.toLowerCase()}">${t.status}</span></td></tr>`).join('')}
  </table>
</div>
