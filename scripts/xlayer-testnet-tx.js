...
async function main() {
  ...
  if (!PRIVATE_KEY) {
    console.log('ESCALATE: Ogie manual tx needed — XLAYER_TESTNET_PK env var not set');
    console.log('Steps for Ogie:');
    console.log('1. Get X Layer testnet OKB from faucet: https://www.okx.com/xlayer/faucet');
    console.log('2. Send 0-value tx on X Layer testnet (chain ID 195)');
    console.log('3. Save tx hash for hackathon submission');
    process.exit(0);
  }

  const provider = new ethers.JsonRpcProvider('https://xlayertestrpc.okx.com', 195);
  ...
  try {
    ...
    console.log(`Explorer: https://www.okx.com/explorer/xlayer-test/tx/${tx.hash}`);
  } catch (err) {
