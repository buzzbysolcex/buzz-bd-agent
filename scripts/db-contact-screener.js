  ...
  try {
    ...
    results.website = await page.evaluate(() => {
      ...
      links.forEach(a => {
        ...
        if (href.includes('twitter.com') || href.includes('x.com')) socialLinks.twitter = a.href;
        if (href.includes('t.me') || href.includes('telegram')) socialLinks.telegram = a.href;
        if (href.includes('discord.gg') || href.includes('discord.com')) socialLinks.discord = a.href;
      ...
      // Look for team info
      const teamKeywords = ['team', 'about us', 'founders', 'our team', 'who we are'];
...
if (TWITTER_HANDLE) {
  try {
    ...
    results.twitter = await page.evaluate(() => {
      ...
      const followerMatch = bodyText.match(/([\d,.]+[KMB]?)\s*(?:Followers|followers)/);
      const followingMatch = bodyText.match(/([\d,.]+[KMB]?)\s*(?:Following|following)/);
      ...
      const lastPostDate = timeEls.length > 0 ? timeEls[0].getAttribute('datetime') : null;
...
if (TELEGRAM_LINK) {
  try {
    ...
    results.telegram = await page.evaluate(() => {
      ...
      // Member count
      const memberMatch = bodyText.match(/([\d,. ]+)\s*(?:members|subscribers)/i);

      return {
        title: document.querySelector('.tgme_page_title')?.textContent?.trim() ||
               document.querySelector('meta[property="og:title"]')?.content,
        description: document.querySelector('.tgme_page_description')?.textContent?.trim() ||
...
// Add screening notes
if (!results.website?.hasTeamPage) results.contact_template.notes.push("No visible team page");
if (!results.website?.hasDocs) results.contact_template.notes.push("No documentation found");
if (emails.length === 0) results.contact_template.notes.push("No email found on website");
if (results.twitter?.loaded === false) results.contact_template.notes.push("Twitter profile failed to load");
