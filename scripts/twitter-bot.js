#!/usr/bin/env node
// =============================================================================
// 🐝 BUZZ TWITTER BOT v3.0 — Full Sales Funnel (Scan + LIST + DEPLOY)
//
...
//
// v3.0 — Sales Funnel (Feb 28):
//   - ADD: LIST/DEPLOY CTA in every scan reply
//   - ADD: Detect "LIST" replies → auto-reply with SolCex listing offer
//   - ADD: Detect "DEPLOY" replies → auto-reply with Bankr deploy instructions
//   - ADD: Detect token details → auto-deploy via Bankr CLI on container
//   - ADD: Lead tracking (twitter-leads.json) + Telegram alerts
//   - ADD: Deploy tracking (twitter-deploys.json) + 3/day cap
//   - ADD: Conversation-aware mention processing (in_reply_to_tweet_id)
//   - ADD: Scan tweet mapping (our reply ID → scan context) for LIST/DEPLOY
//   - UPDATE: fetchMentions() now includes referenced_tweets for reply detection
//   - UPDATE: run() loop handles 4 command types: scan, LIST, DEPLOY, token details
//
// v2.2 (base):
//   - Fixed scoring flatness, unknown data scores LOW, letter grades, vol/liq bonus
//
...
// v3.0 — LIST/DEPLOY tracking
const LEADS_FILE        = `${DATA_DIR}/twitter-leads.json`;
const DEPLOYS_FILE      = `${DATA_DIR}/twitter-deploys.json`;
const SCAN_TWEETS_FILE  = `${DATA_DIR}/twitter-scan-tweets.json`;
const MAX_DEPLOYS_DAY   = 3;
const DEPLOY_DAILY_FILE = `${DATA_DIR}/twitter-deploy-daily.json`;
const BANKR_FEE_WALLET  = process.env.BANKR_FEE_WALLET || '0x2Dc03124091104E7798C0273D96FC5ED65F05aA9';
const BANKR_ENABLED     = process.env.BANKR_SIGNALS_ENABLED === 'true';
...
// ── v3.0: Scan Tweet Tracking ───────────────────────────────────────────────
// Maps OUR reply tweet IDs → scan context, so when someone replies "LIST"
// to our scan, we know which token they're interested in
...
// ── v3.0: Command Parsers ───────────────────────────────────────────────────
function parseListCommand(text) {
  const cleaned = text.replace(/@\w+/g, '').replace(/[^A-Za-z!.]/g, '').toUpperCase();
  return /^LIST[!.]?$/.test(cleaned);
}

function parseDeployCommand(text) {
  const cleaned = text.replace(/@\w+/g, '').replace(/[^A-Za-z!.]/g, '').toUpperCase();
  return /^DEPLOY[!.]?$/.test(cleaned);
}

// Parse token details: "TokenName TICKER description" or TokenName TICKER "description"
function parseTokenDetails(text) {
  const cleaned = text.replace(/@\w+/g, '').trim();
  const m = cleaned.match(/^(\S+)\s+([A-Z]{2,10})\s+[""]?(.+?)[""]?\s*$/i);
  if (m && m[3].length >= 3) return { name: m[1], ticker: m[2].toUpperCase(), description: m[3] };
  return null;
}

// ── v3.0: Reply Builders ────────────────────────────────────────────────────
function buildListReply() {
  let t = `Thanks for your interest! 🐝\n\n`;
  t += `SolCex Exchange offers fast-track listing with full market making support.\n\n`;
  t += `📩 DM hidayahanka1 on X for details\n`;
  t += `📱 Or TG: Ogie2\n\n`;
  t += `🐝 Powered by Buzz BD Agent`;
  if (t.length > 280) t = t.slice(0, 277) + '...';
  return t;
}

function buildDeployReply() {
  let t = `Token deploy on Base — Bankr 🚀\n\n`;
  t += `✅ Zero gas\n`;
  t += `✅ Instant liquidity\n`;
  t += `✅ 1.2% swap fee split\n`;
  t += `✅ BaseScan in minutes\n\n`;
  t += `Reply with:\n`;
  t += `TokenName TICKER "description"\n\n`;
  t += `Example:\n`;
  t += `SolCexGold SGOLD "Gold token"\n\n`;
  t += `Powered by Buzz BD Agent 🐝`;
  if (t.length > 280) t = t.slice(0, 277) + '...';
  return t;
}

function buildDeployResultReply(ticker, contractAddr) {
  let t = `✅ $${ticker} is live on Base!\n\n`;
  t += `📍 ${contractAddr}\n`;
  t += `🔗 basescan.org/token/${contractAddr}\n`;
  t += `💰 75% creator / 18% Bankr\n\n`;
  t += `Trading now 🚀🐝\n\n`;
  t += `Powered by Buzz BD Agent`;
  if (t.length > 280) t = t.slice(0, 277) + '...';
  return t;
...
function buildDeployCapReply() {
  return `Daily deploy limit reached (3/3). Try again tomorrow 🐝\n\nPowered by Buzz BD Agent`;
}

function buildDeployFailReply() {
  return `⚠️ Deploy simulation failed. Please check your details and try again 🐝\n\nPowered by Buzz BD Agent`;
}

// ── v3.0: Telegram Lead Notifications ───────────────────────────────────────
async function notifyLead(type, handle, tokenInfo, tweetId) {
  const emoji = type === 'LIST' ? '🔥' : '🚀';
  const label = type === 'LIST' ? 'LISTING LEAD' : 'DEPLOY LEAD';
  let msg = `${emoji} ${label}\n\n`;
  msg += `Twitter: @${handle}\n`;
  msg += `Tweet: https://x.com/BuzzBySolCex/status/${tweetId}\n`;
  if (tokenInfo) {
    msg += `Token: $${tokenInfo.symbol} (${tokenInfo.chain})\n`;
    msg += `Score: ${tokenInfo.score}/100 (${tokenInfo.grade})\n`;
    msg += `CA: ${tokenInfo.ca}\n`;
  }
  msg += `\nAuto-reply sent. Follow up within 24h.`;
...
async function notifyDeployResult(handle, ticker, contractAddr, tweetId) {
  let msg = `✅ DEPLOY COMPLETE\n\n`;
  msg += `Twitter: @${handle}\n`;
  msg += `Token: $${ticker}\n`;
  msg += `Contract: ${contractAddr}\n`;
  msg += `BaseScan: https://basescan.org/token/${contractAddr}\n`;
  msg += `Tweet: https://x.com/BuzzBySolCex/status/${tweetId}\n`;
  ...
  try {
    ...
    log(`  📨 Deploy result notified to Telegram`);
  } catch (e) { log(`  ⚠ Deploy notify error: ${e.message}`); }
}

// ── v3.0: Bankr Deploy via CLI ──────────────────────────────────────────────
// Executes Bankr deploy on the container using child_process
const { execSync } = require('child_process');

async function executeBankrDeploy(name, ticker, description) {
  log(`  [DEPLOY] Attempting: ${name} ${ticker} "${description}"`);

  // Check daily cap
  const deployCount = getDeployDailyCount();
  if (deployCount.count >= MAX_DEPLOYS_DAY) {
    log(`  [DEPLOY] ❌ Daily cap reached (${deployCount.count}/${MAX_DEPLOYS_DAY})`);
    return { success: false, error: 'DAILY_CAP' };
  }

  // Check for offensive names
  const banned = ['fuck','shit','nigger','faggot','nazi','hitler','porn','sex','rape','kill'];
  if (banned.some(w => name.toLowerCase().includes(w) || ticker.toLowerCase().includes(w) || description.toLowerCase().includes(w))) {
    log(`  [DEPLOY] ❌ Rejected: offensive content`);
    return { success: false, error: 'OFFENSIVE' };
  }

  try {
    // Bankr CLI uses natural language prompts for deployment
    const deployPrompt = `deploy a token called ${name} with symbol ${ticker} on base`;
    log(`  [DEPLOY] Prompt: "${deployPrompt}"`);

    const deployCmd = `bankr prompt "${deployPrompt}" 2>&1 | cat`;
    const deployResult = execSync(deployCmd, { timeout: 120000, encoding: 'utf8' });
    log(`  [DEPLOY] Result: ${deployResult.slice(0, 500)}`);

    // Check if Bankr returned a job ID (async deployment)
    const jobMatch = deployResult.match(/job[_\s]*(?:id)?[:\s]*([a-zA-Z0-9_-]+)/i);
    if (jobMatch) {
      const jobId = jobMatch[1];
      log(`  [DEPLOY] Job started: ${jobId}`);

      // Poll job status (max 90 seconds, check every 10s)
      for (let i = 0; i < 9; i++) {
        await new Promise(r => setTimeout(r, 10000));
        try {
          const statusResult = execSync(`bankr status ${jobId} 2>&1 | cat`, { timeout: 15000, encoding: 'utf8' });
          log(`  [DEPLOY] Status: ${statusResult.slice(0, 200)}`);

          // Check for contract address in status
          const addrMatch = statusResult.match(/0x[a-fA-F0-9]{40}/);
          if (addrMatch) {
            incrementDeployDailyCount();
            const contractAddr = addrMatch[0];
            saveDeploy(name, ticker, description, contractAddr);
            return { success: true, contractAddr };
          }

          // Check for completion or failure
          if (statusResult.toLowerCase().includes('completed') || statusResult.toLowerCase().includes('success')) {
            const addrMatch2 = statusResult.match(/0x[a-fA-F0-9]{40}/);
            if (addrMatch2) {
              incrementDeployDailyCount();
              saveDeploy(name, ticker, description, addrMatch2[0]);
              return { success: true, contractAddr: addrMatch2[0] };
            }
          }
          if (statusResult.toLowerCase().includes('failed') || statusResult.toLowerCase().includes('error')) {
            log(`  [DEPLOY] Job failed`);
            return { success: false, error: 'JOB_FAILED' };
          }
        } catch (statusErr) {
          log(`  [DEPLOY] Status check error: ${statusErr.message.slice(0, 100)}`);
        }
      }

      // Timeout — job still running
      log(`  [DEPLOY] ⏳ Job still running after 90s — will check manually`);
      return { success: false, error: 'TIMEOUT', jobId };
    }

    // Check for immediate contract address in output
    const addrMatch = deployResult.match(/0x[a-fA-F0-9]{40}/);
    if (addrMatch) {
      incrementDeployDailyCount();
      const contractAddr = addrMatch[0];
      saveDeploy(name, ticker, description, contractAddr);
    ...
    if (deployResult.toLowerCase().includes('error') || deployResult.toLowerCase().includes('failed')) {
      log(`  [DEPLOY] ❌ Bankr returned error`);
      return { success: false, error: 'BANKR_ERROR' };
    }

    log(`  [DEPLOY] ⚠ No contract address found in output`);
    return { success: false, error: 'NO_ADDRESS' };
  } catch (e) {
    log(`  [DEPLOY] ❌ Error: ${e.message.slice(0, 200)}`);
    return { success: false, error: e.message.slice(0, 100) };
  }
}

function saveDeploy(name, ticker, description, contractAddr) {
  const deploys = loadJSON(DEPLOYS_FILE, []);
  deploys.push({
    name, ticker, description, contractAddr,
    feeWallet: BANKR_FEE_WALLET, mode: 'internal',
    timestamp: new Date().toISOString()
  });
  saveJSON(DEPLOYS_FILE, deploys.slice(-100));
}
...
function l4_scoringAgent(l1, l2, l3) {
  ...
  // RugCheck: lower score = safer. rugResult often null, so use numeric rugScore.
  // Mint disabled + Freeze disabled = strong safety signal.
  if (l1.chain === 'solana') {
    ...
    else if (l2.rugScore !== null && l2.rugScore > 0) {
      // RugCheck numeric: lower = safer. Typical range 0-100000+
      if (l2.rugScore < 5000) factors.safety = 9;
      else if (l2.rugScore < 15000) factors.safety = 7;
      else if (l2.rugScore < 30000) factors.safety = 5;
      else if (l2.rugScore < 50000) factors.safety = 4;
      else factors.safety = 2;
      // Bonus: mint+freeze disabled on top of score
      if (l2.mintAuthority?.includes('DISABLED') && l2.freezeAuthority?.includes('DISABLED')) {
        factors.safety = Math.min(10, factors.safety + 1);
      }
    } else if (l2.mintAuthority?.includes('DISABLED') && l2.freezeAuthority?.includes('DISABLED')) {
      factors.safety = 8;
    } else if (l2.mintAuthority?.includes('DISABLED')) {
      factors.safety = 6;
    } else {
      factors.safety = 3;
    }
  } else {
    factors.safety = l2.isVerified ? 9 : 4;
  }
...
// ── Build Scan Reply (280 char Twitter limit) — v3.0 with LIST/DEPLOY CTA ──
function buildReply(l1, l2, l3, scoring) {
  ...
  // Safety summary
  let safetyGrade;
  if (scoring.factors.safety >= 8) safetyGrade = 'A';
  else if (scoring.factors.safety >= 6) safetyGrade = 'B';
  else if (scoring.factors.safety >= 4) safetyGrade = 'C';
  else safetyGrade = 'D';

  // Mint/Freeze line (Solana)
  let mintFreeze = '';
  if (l1.chain === 'solana') {
    mintFreeze = `🛡️ Mint: ${l2.mintAuthority?.includes('DISABLED') ? '✅ Disabled' : '⚠️ Enabled'} | Freeze: ${l2.freezeAuthority?.includes('DISABLED') ? '✅ Disabled' : '⚠️ Enabled'}`;
  } else {
    mintFreeze = `🛡️ Contract: ${l2.isVerified ? '✅ Verified' : '⚠️ Unverified'}`;
  }

  // LP status
  let lpLine = `🔒 LP: ${l2.lpLocked || 'Unknown'}`;

  // Risks
  let risksLine = '';
  if (l2.risks && l2.risks.length > 0) {
    const topRisks = l2.risks.slice(0, 3).map(r => `${r.name}(${r.level})`).join(', ');
    risksLine = `⚠️ Risks: ${topRisks}`;
  } else {
    risksLine = `✅ Risks: None detected`;
  }

  // Socials
  let socialsLine = '';
  const parts = [];
  if (l1.websites.length > 0) parts.push('Website ✅');
  if (l1.socials.some(s => s.type === 'twitter')) parts.push('Twitter ✅');
  if (l1.socials.some(s => s.type === 'telegram')) parts.push('Telegram ✅');
  if (l1.socials.some(s => s.type === 'discord')) parts.push('Discord ✅');
  socialsLine = parts.length > 0 ? parts.join(' | ') : 'None';

  // Price change direction
  const changeDir = l1.priceChange24h >= 0 ? '+' : '';
  ...
  // L1 Discovery
  t += `L1 Discovery ✅\n`;
  t += `💰 Price: ${fmtPrice(l1.price)}\n`;
  t += `📊 FDV: $${fmtNum(l1.fdv)} | MCap: $${fmtNum(l1.mcap)}\n`;
  t += `📈 Vol 24h: $${fmtNum(l1.vol24h)} | Liq: $${fmtNum(l1.liq)}\n`;
  t += `🔄 Txns: ${fmtNum(l1.txns24h)} | Change: ${changeDir}${l1.priceChange24h.toFixed(1)}%\n`;
  t += `🏦 DEX: ${l1.dex || 'Unknown'}\n\n`;

  // L2 Safety
  t += `L2 Safety (${safetyGrade})\n`;
  t += `${mintFreeze}\n`;
  t += `${lpLine}\n`;
  t += `${risksLine}\n\n`;
  ...
  t += `L3 Identity\n`;
  t += `🪪 ${l3.label}\n`;
  if (l3.holders) t += `👥 Holders: ${fmtNum(l3.holders)}\n`;
  t += `🌐 Socials: ${socialsLine}\n\n`;

  // L4 Score
  t += `L4 Score: ${scoring.score}/100 (${scoring.grade})\n`;
  t += `━━━━━━━━━━━━━━━━━━━━━━\n`;
  t += `Safety: ${f.safety} | Liq: ${f.liquidity} | Vol: ${f.volume} | MCap: ${f.mcap}\n`;
  t += `Social: ${f.social} | Age: ${f.age} | Holders: ${f.holders} | Activity: ${f.activity}\n`;
  t += `LP: ${f.lpLock} | Identity: ${f.identity}\n\n`;

  // CA
  t += `CA: ${l1.ca}\n\n`;

  // CTA
  t += `📋 Reply "LIST" → SolCex Exchange\n`;
  t += `🚀 Reply "DEPLOY" → Base (Bankr)\n`;
  t += `🐝 Powered by Buzz BD Agent`;

  // Premium = 4000 chars, fallback to 280 if somehow over
  if (t.length > 4000) t = t.slice(0, 3997) + '...';
  return t;
}

// ── Twitter: Fetch Mentions (v3.0 — includes reply context) ─────────────────
async function fetchMentions() {
  // v3.0: expansions + tweet.fields for reply detection
  const res = await safeReq({
    hostname: 'api.twitter.com',
    path: `/2/tweets/search/recent?query=%40BuzzBySolCex&max_results=${MAX_RESULTS}&expansions=referenced_tweets.id&tweet.fields=author_id%2Cconversation_id`,
    method: 'GET',
...
// ── Twitter: Look up a single tweet by ID (for author_id on parent) ─────────
async function lookupTweet(tweetId) {
  const res = await safeReq({
    hostname: 'api.twitter.com',
    path: `/2/tweets/${tweetId}?tweet.fields=author_id`,
    method: 'GET',
    headers: { 'Authorization': `Bearer ${process.env.X_BEARER_TOKEN}` }
  });
...
async function lookupUser(userId) {
  ...
  return res.data.data?.username || 'unknown';
}
...
// Buzz's own user ID — to skip our own tweets in mention feed
const BUZZ_USER_ID = process.env.X_BOT_USER_ID || '2018012888196370432';

// ── Main Loop (v3.0 — Scan + LIST + DEPLOY + Token Details) ─────────────────
async function run() {
  log('');
  log('╔═══════════════════════════════════════════════════╗');
  log('║  🐝 BUZZ TWITTER BOT v3.0 — Sales Funnel          ║');
  ...
  log('║  📨: Telegram bridge (scans + leads + deploys)    ║');
  log('║  📋: LIST → SolCex | 🚀 DEPLOY → Bankr            ║');
  ...
  if (!fs.existsSync(REPLIED_FILE)) {
    ...
    try {
      ...
      if (existing.length > 0) {
        ...
        saveReplied(ids);
        log(`  ✅ Marked ${ids.length} existing mentions as seen.`);
      } else {
        saveReplied([]);
        log('  ✅ No existing mentions. Ready.');
      }
    } catch (e) {
      saveReplied([]);
      log(`  ⚠ First run error: ${e.message}. Starting fresh.`);
    }
  } else {
    ...
    try {
      ...
      if (daily.count >= MAX_REPLIES_DAY) {
        ...
        for (const tweet of mentions) {
          ...
          // Skip our own tweets
          if (tweet.author_id === BUZZ_USER_ID) {
          ...
          const parentId = (tweet.referenced_tweets && tweet.referenced_tweets.find(r => r.type === "replied_to"))?.id || null;
          const scanContext = parentId ? loadScanTweets()[parentId] || null : null;

          // ════════════════════════════════════════════════════════════════
          // ROUTE 1: "LIST" reply to our scan tweet
          // ════════════════════════════════════════════════════════════════
          if (parseListCommand(tweet.text) && scanContext) {
            log(`\n📋 LIST REQUEST from tweet ${tweet.id} (re: $${scanContext.symbol})`);

            const replyText = buildListReply();
            const postResult = await postReply(tweet.id, replyText);

            if (postResult && postResult.status === 201) {
              const count = incrementDailyCount();
              log(`  ✅ LIST reply posted (${count}/${MAX_REPLIES_DAY})`);

              // Get user handle for Telegram notification
              const handle = tweet.author_id ? await lookupUser(tweet.author_id) : 'unknown';

              // Track lead
              saveLead({ type: 'LIST', handle, token: scanContext, tweetId: tweet.id });

              // Notify Ogie via Telegram
              await notifyLead('LIST', handle, scanContext, tweet.id);
            } else {
              log(`  ❌ LIST reply failed: ${postResult?.status || 'null'}`);
            }
          ...
          // ════════════════════════════════════════════════════════════════
          // ROUTE 2: "DEPLOY" reply to our scan tweet
          // ════════════════════════════════════════════════════════════════
          else if (parseDeployCommand(tweet.text) && (scanContext || !parentId)) {
            log(`\n🚀 DEPLOY REQUEST from tweet ${tweet.id}`);

            const replyText = buildDeployReply();
            const postResult = await postReply(tweet.id, replyText);

            if (postResult && postResult.status === 201) {
              const count = incrementDailyCount();
              log(`  ✅ DEPLOY reply posted (${count}/${MAX_REPLIES_DAY})`);

              const handle = tweet.author_id ? await lookupUser(tweet.author_id) : 'unknown';
              saveLead({ type: 'DEPLOY', handle, token: scanContext, tweetId: tweet.id });
              await notifyLead('DEPLOY', handle, scanContext, tweet.id);

              // Save this reply so we can detect token details replies
              if (postResult.data?.data?.id) {
                const scanTweets = loadScanTweets();
                scanTweets[postResult.data.data.id] = { _deployPrompt: true, requester: handle, timestamp: new Date().toISOString() };
                saveJSON(SCAN_TWEETS_FILE, scanTweets);
              }
            } else {
              log(`  ❌ DEPLOY reply failed: ${postResult?.status || 'null'}`);
            }
          ...
          // ════════════════════════════════════════════════════════════════
          // ROUTE 3: Token details reply (for deployment execution)
          // ════════════════════════════════════════════════════════════════
          else if (parentId && parseTokenDetails(tweet.text)) {
            const parentContext = loadScanTweets()[parentId];
            if (parentContext && parentContext._deployPrompt) {
              const details = parseTokenDetails(tweet.text);
              log(`\n🏗️ DEPLOY EXECUTION: ${details.name} ${details.ticker} "${details.description}"`);

              // Check deploy cap
              if (getDeployDailyCount().count >= MAX_DEPLOYS_DAY) {
                await postReply(tweet.id, buildDeployCapReply());
                log('  ⚠ Deploy cap reached, notified user');
              } else if (!BANKR_ENABLED) {
                await postReply(tweet.id, '⚠️ Deploy service is offline. DM hidayahanka1 on X for manual deployment 🐝');
                log('  ⚠ Bankr not enabled');
              } else {
                // Execute deploy
                const result = await executeBankrDeploy(details.name, details.ticker, details.description);

                if (result.success) {
                  const replyText = buildDeployResultReply(details.ticker, result.contractAddr);
                  await postReply(tweet.id, replyText);
                  incrementDailyCount();

                  const handle = tweet.author_id ? await lookupUser(tweet.author_id) : 'unknown';
                  await notifyDeployResult(handle, details.ticker, result.contractAddr, tweet.id);
                  log(`  ✅ DEPLOYED: $${details.ticker} at ${result.contractAddr}`);
                } else if (result.error === 'OFFENSIVE') {
                  await postReply(tweet.id, '❌ Token name rejected. Please use appropriate naming 🐝');
                } else if (result.error === 'DAILY_CAP') {
                  await postReply(tweet.id, buildDeployCapReply());
                } else {
                  await postReply(tweet.id, buildDeployFailReply());
          ...
          // ════════════════════════════════════════════════════════════════
          // ROUTE 4: Standard scan command (existing v2.2 logic)
          // ════════════════════════════════════════════════════════════════
          else {
            ...
            if (postResult && postResult.status === 201) {
              ...
              // v3.0: Save our reply tweet ID → scan context for LIST/DEPLOY detection
              if (postResult.data?.data?.id) {
                saveScanTweet(postResult.data.data.id, result);
              }
          ...
          if (processed > 0 && processed < mentions.length) {
            log(`  ⏳ Rate limit delay ${REPLY_DELAY_MS / 1000}s...`);
            await new Promise(r => setTimeout(r, REPLY_DELAY_MS));
          }
        }

        if (processed === 0) log('No new commands.');
        else log(`✅ Processed ${processed} command(s) this cycle.`);
      }
    } catch (err) {
