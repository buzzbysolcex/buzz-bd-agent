...
const uiData = await page.evaluate(() => {
  ...
  return {
    ...
    circulatingMcap: extractMetric('Circ.*MCap') || extractMetric('Circulating'),
...
const result = {
  ...
  api: {
    ...
    // Derived
    fdv_gap: (token.fdv_usd && token.market_cap_usd && parseFloat(token.fdv_usd) > 0)
      ? ((parseFloat(token.fdv_usd) - parseFloat(token.market_cap_usd)) / parseFloat(token.fdv_usd) * 100).toFixed(1) + '%'
      : null
  },
