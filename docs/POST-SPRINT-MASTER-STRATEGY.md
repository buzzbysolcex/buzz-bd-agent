---
name: buzz-post-sprint-strategy
description: >
  Comprehensive post-sprint strategy synthesizing 3 intelligence reports:
  1. AIBTC Top 3 Competitive Analysis (Secret Mars #1, Encrypted Zara #2, Dual Cougar #3)
  2. HeyAnon MCP × ARIA Integration (Intel Source #30, 18 chains, 128 tools)
  3. AIBTC Signal Factory v2.0 (proven patterns, rejection analysis)
  Combined into actionable plan to climb from #8 to Top 3 while expanding ARIA
  from 29 to 30+ sources with cross-chain DeFi intelligence.
...
Imagine our signal: "sBTC Lending Deposits on Zest Protocol Hit $2.1M While
Hyperliquid BTC Perps OI Crosses $500M — Agent Capital Splitting Between
L2 Yield and L1 Derivatives"

That signal has cross-chain data NO OTHER AGENT CAN FILE because they
...
## THE STRATEGY: HYBRID MODEL (Option C from Intel Report)

Neither pure specialist (#1's model) nor pure multi-beat (#2's model).
...
4. **dev-browser** = structured data from sites with no API
5. **MiroFish 50-agent simulation** = prediction capability
6. **Cross-chain DeFi data via HeyAnon** = multi-chain intelligence
...
because we ARE building infrastructure. Not reporting on others' work —
...
These are infrastructure signals backed by OUR deployed contracts and code.
Nobody else can file them because nobody else built them.
...
4. **GitHub activity with commit references** — specific PRs, commit hashes
...
4. **Beat mismatch** — content must match beat definition exactly
...
AFTER: "JingSwap sBTC Sell Pressure Mirrors Cross-Chain Pattern — Hyperliquid
BTC Perps OI Down 12% While Zest Protocol Deposits Rise 8% to $2.1M"
(multi-chain, HeyAnon-enriched, deeper analysis)
```

The enriched signal has MORE data points, CROSS-CHAIN context, and a
DIRECTIONAL ANALYSIS that the brief compiler can't get from any other
correspondent. This is how we go from 32% to 60%+ inclusion rate.

---

## ARIA EXPANSION: 29 → 31 SOURCES

### HeyAnon MCP Integration (Intel Source #30)

```
Connection: https://api.heyanon.ai/mcp
Auth: X-API-Key (free tier, sign up at heyanon.ai)
Protocol: MCP standard
What it adds: 18 chains, 45+ protocols, 5 CEXs, Hyperliquid perps
```

### Phantom MCP Integration (Intel Source #31)

```
Package: @phantom/mcp-server (npm)
App ID: be4a0179-1113-416b-a6e6-45a09191f407 (already registered)
Auth: SSO (one-time from Ogie's Mac, session persists)
What it adds: Wallet control (SOL, ETH, BTC, SUI), swap quotes,
  identity signing, listing fee collection, price verification
```

### Combined ARIA Architecture
...
Layer 1: CHAIN-NATIVE (deep, single-chain)
  ...
  └── Phantom MCP (wallet balances, swap quotes, identity)

Layer 2: AGGREGATION (wide, multi-chain) ← HeyAnon
  └── HeyAnon MCP (18 chains, DeFi + CeFi + Perps + Portfolio)

Layer 3: ENRICHMENT (context, non-price)
  ├── dev-browser scripts (contact screening)
  ├── GeckoTerminal (circulating MCap, gt_score)
  └── Social/Twitter (community metrics)

Layer 4: EXECUTION (future, CEO-approved)
  ├── Phantom MCP (transfer, swap, sign)
...
+2: CEX listing confirmed via HeyAnon portfolio
...
1. **Improve inclusion rate: 32% → 60%**
   ...
   - "So what" analysis on every signal (not just data)
   - File before 06:00 UTC (pre-compilation window)

2. **Maintain streak: never miss a day**
   - 5 pts × 25 days = 125 points
   - Streak reset = devastating (back to 0)
   - File at least 1 signal before midnight UTC

3. **Use corrections: 2 per month**
   - Find weak signals from lower-ranked agents
   - File well-sourced corrections with better data
   - 15 pts each = 30 free points nobody else uses

4. **File timing: 04:00-06:00 UTC**
   - Secret Mars files at 4-6 AM UTC (pre-compilation)
   - Early signals get priority in brief selection
   - Buzz's 06:00 UTC cron is already in this window

5. **1 killer signal per day, not 4 mediocre ones**
   - Secret Mars: 0.9 signals/day, 69.2% inclusion
   - Prime Spoke: 1.1 signals/day, 26.5% inclusion
   - Quality >>> volume for inclusion rate
...
4. Write signal with HeyAnon cross-chain enrichment
5. File on agent-trading or infrastructure beat
...
4. Adjust beat strategy if needed
...
Body: [What deployed] + [technical details] + [on-chain proof] +
[cross-chain context via HeyAnon] + [implication for agent ecosystem]
Beat: infrastructure
Sources: basescan.org, api.heyanon.ai
```
...
Body: [Primary data point] + [HeyAnon cross-chain verification] +
[Hyperliquid perps context if relevant] + [directional analysis]
Beat: agent-trading
Sources: stacks explorer, heyanon.ai, pyth oracle
```
...
Body: [What happened] + [Buzz scoring data] + [on-chain verification] +
[HeyAnon DeFi depth context] + [BD classification]
Beat: deal-flow
Sources: buzzbd.ai, dexscreener, heyanon.ai
```
...
Body: [Original claim] + [Actual verified data] + [Why it matters] +
[Correct interpretation with sources]
Beat: [same as original]
Sources: [stronger sources than original]
```
...
- [ ] Target: 5 brief inclusions this week (currently avg 2/week)
- [ ] File 1 correction on a weak signal
- [ ] Austin follow-up (Mar 30)
...
- [ ] Projected score: ~600+ (challenging #1)
...
**HeyAnon gives us cross-chain data nobody else has, Phantom gives us wallet
control + price verification, infrastructure beat gives us a high-inclusion
filing lane, and corrections give us free points nobody uses — combined,
these four levers take Ionic Nova from #8 to #1 in 30 days.**
