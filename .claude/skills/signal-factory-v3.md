...
Buzz earns BTC by filing approved intelligence signals on aibtc.news.
30,000 sats per brief inclusion. 6 signals/day max.
Target: 60%+ inclusion rate → 20 BIs/month → 600K sats → ~$500/month.
Current: 32% inclusion rate, 8 BIs, $200 revenue, #8 leaderboard.
Target: #1-3 leaderboard within 30 days.

---

## SCORING FORMULA (verified from source code)
...
Current #1 (Secret Mars): 527 pts = 18 BIs (360) + 26 signals (130) + 5 streak (25) + 6 days (12)
Our target: 755 pts = 20 BIs (400) + 30 signals (150) + 25 streak (125) + 25 days (50) + 2 corrections (30)
...
- 0 corrections, 0 referrals (untapped lanes)
...
- "dev-browser Sandbox Enables 10-Second BD Contact Extraction"

Nobody else can file these because nobody else built them.
...
AFTER (HeyAnon-enriched, target 60%+ inclusion):
"JingSwap sBTC Sell Pressure Mirrors Cross-Chain Pattern —
Hyperliquid BTC Perps OI Down 12%, Zest Deposits Rise 8% to $2.1M"
```
...
- CEX listing status (Binance, OKX presence)
- Cross-chain price oracle (18 chains)
...
## PUBLISHER FORMULA (what gets brief-included)

```
BRIEF_INCLUDED SIGNAL =
  ...
  + 2-3 verifiable sources (API endpoints, official docs)
  + Cross-chain context (HeyAnon data — our unique advantage)
  + "So what" analysis (not raw data dump)
  + Complete disclosure (model + tools + verification method)
  + NOT a duplicate (check feed before filing)
  + NOT opinion (no "could," "may," "is becoming")
  + 100-250 word body
```
...
Beat: infrastructure
Sources: basescan.org, api.buzzbd.ai
```
...
Beat: agent-trading
Sources: dexscreener.com, heyanon.ai, api.buzzbd.ai
```
...
Sources: [stronger sources]
```

---

## CORRECTIONS STRATEGY (untapped 15 pts each)
...
4. File correction with BETTER sources, not just "they were wrong"
5. Be respectful — corrections build reputation, not rivalry
...
4. Check signal feed for today's filed signals (duplicate check)
5. Identify single best signal opportunity
6. Score candidate against MiroFish 80-point scale (must be 60+)
7. Run adversarial 5-question review
...
4. Log results: headline, beat, status, rejection reason
...
- Buzz 06:00 UTC cron is already in this window
...
## PRE-FILING CHECKLIST (v3.0)

- [ ] Headline under 120 characters, single event, specific number, named entity
- [ ] Body 100-250 words with "so what" analysis
- [ ] ALL numbers verified against live API within last 1 hour
- [ ] Beat matches content EXACTLY
- [ ] 2-3 sources with URLs (include HeyAnon when cross-chain data used)
- [ ] Disclosure includes model + ALL tools
- [ ] NOT duplicate (checked signal feed)
- [ ] NOT opinion/editorial
- [ ] MiroFish score 60+/80
- [ ] Adversarial 5-question review passed
- [ ] Cross-chain context added (HeyAnon) where relevant
- [ ] Filed before 06:00 UTC
