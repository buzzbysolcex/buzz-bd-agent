...
3. **Bytecode-verify prep** (Veda + Wormhole lesson):
   ...
   - Defer execution until Gate 2 finding, but plan the verification command up-front
     3a. **SUBSTRATE-IDENTITY VERIFICATION — CANONICAL (5-anchor PERMANENT, expanded 2026-05-29 — Sky DSS NEGATIVE-WORKED-EXAMPLE + Hyperlane HypERC4626 positive)**:
   - **Status:** CANONICAL (3 positive anchors 2026-05-28 + 1 NEGATIVE-WORKED-EXAMPLE + 1 positive anchor 2026-05-29 = 5 total). Promoted from RULE (single anchor) on Wormhole NTT G1 3rd-anchor validation; negative-worked-example sub-rule added on Sky DSS Gate 1; 5th (positive) anchor on Hyperlane HypERC4626 Gate 2.
   - **Anchors:**
   ...
   - **Time-cost saving per anchor:** ~30min wasted Foundry/clone investment avoided per future wrapper-class hypothesis. Cumulative across 5 anchors: ~90min Day-28 positive-anchor savings + ~2-3h Day-29 negative-worked-example save (Sky DSS substrate-OOS pre-empt) + Hyperlane HypERC4626 5th positive anchor (location-confirmed, finding NEGATED downstream) demonstrated.
...
12. **Cache-Before-Validate + Identity-Binding-Gap source-read check (detector #166 + Doctrine #44, added 2026-05-29; Zebra GHSA-4m69-67m6-prqp ground-truth anchor, Ogie msg 8021).** On ANY target with a dedup / seen / sent / nonce / processed / packet-receipt / proof cache (bridges, relays, IBC, restaking-replay, signature-nonce): run two checks. **(a) Cache-before-validate (#166):** does the cache INSERT _precede_ validation, and does the validation-FAILURE branch (revert / return-err / early-return) fail to UNWIND the inserted key? If yes → a later legitimate same-id item is rejected as a duplicate = lockout/DoS (CWE-459/460). **(b) Identity-binding-gap (#44):** is the dedup-id / cache-key / merkle-leaf / message-id computed over a STRICT SUBSET of the content the consumer acts on? If yes → attacker holds the id fixed, varies the rest → poisoning (pre-seed a malleated same-id variant) or substitution. The two compose: a malleable id (b) + cache-before-validate (a) = the full Zebra chain. **Both are pure state-logic → locally PoC-able ([EXECUTED] feasible, no precompile)** — high-EV on bridge/relay/IBC dedup. Run on Hyperbridge (#2 queue) for both.
