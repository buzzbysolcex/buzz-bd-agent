 ...
 *
 * Partner Key: process.env.BANKR_PARTNER_KEY (loaded from /home/claude-code/.env)
...
const CONFIG = {
  ...
  // For prompt-based commands (fee checking/claiming)
  agentApiKey: process.env.BANKR_AGENT_API_KEY, // optional, for agent prompt API
  // Paths
...
function bankrRequest(method, requestPath, body = null, useAgentKey = false) {
  return new Promise((resolve, reject) => {
    const headers = {
      ...
      "User-Agent": "BuzzBDAgent/6.0.4",
    };

    // Use partner key for deploy, agent key for prompt API
 ...
 * @param {string} [params.tokenSymbol] - Ticker (1-10 chars, defaults to first 4 of name)
 ...
 * Returns predicted token address
 */
 ...
 * Their Bankr wallet receives the creator's 57% share
 */
...
/**
 * Check accumulated partner fees via Bankr prompt API
 * Requires BANKR_AGENT_API_KEY
 * @param {string} tokenName - Token name to check fees for
 */
...
/**
 * Execute a Bankr prompt (async job polling)
 */
async function executeBankrPrompt(prompt) {
  ...
  // Poll for completion (max 60 attempts, 2s apart = 2 min timeout)
  for (let i = 0; i < 60; i++) {
 ...
 * @returns {Object[]} Array of deployment records
 */
...
/**
 * Get deployment by token symbol
 */
...
/**
 * Format a token launch as a tweet
 * @param {Object} deployResult - Result from deployToken()
 * @param {Object} extra - Extra metadata (description, etc.)
