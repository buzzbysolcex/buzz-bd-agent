...
function getOAuthHeader(method, urlStr) {
  // Strip query params for signature base URL
...
function apiRequest(method, url, body = null) {
  return new Promise((resolve, reject) => {
      ...
      res.on("end", () => {
        try {
          ...
          if (res.statusCode >= 400) {
            const err = new Error(
              `X API ${res.statusCode}: ${JSON.stringify(parsed)}`,
            );
          ...
          reject(
            new Error(
              `X API parse error: ${e.message} | Raw: ${data.slice(0, 200)}`,
            ),
          );
        }
      });
    ...
    req.setTimeout(15000, () => {
      req.destroy();
      reject(new Error("X API timeout (15s)"));
    });
...
// Bearer token request (for app-only endpoints)
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          if (res.statusCode >= 400)
            reject(
              new Error(`X API ${res.statusCode}: ${JSON.stringify(parsed)}`),
            );
          else resolve(parsed);
        } catch (e) {
          reject(new Error(`X API parse: ${e.message}`));
        }
      });
    });
    req.on("error", reject);
...
async function postTweet(text, replyToId = null) {
  ...
  if (stats.tweetsPosted >= CONFIG.maxTweetsPerDay) {
    throw new Error(
      `Daily tweet limit reached (${CONFIG.maxTweetsPerDay}). Reset at midnight UTC.`,
    );
  }
  if (text.length > 280) {
...
async function postThread(tweets) {
  ...
  for (let i = 0; i < tweets.length; i++) {
    ...
    // Space replies 3 seconds apart (algorithm treats as fresh engagement)
    if (i < tweets.length - 1) {
      await new Promise((r) => setTimeout(r, 3000));
    }
  }

  logAction("thread", { count: tweets.length, firstId: results[0]?.data?.id });
  return results;
}
 ...
 * @param {Object} metadata - Extra context (token symbol, scan data, etc.)
...
function queueDraft(content, type = "single", metadata = {}) {
  ...
  console.log(
    `[TWITTER] Draft ${draft.id} queued (${type}) — awaiting Telegram approval`,
  );
  return draft;
}
...
async function approveDraft(draftId) {
  ...
  if (draft.status !== "pending_approval")
    throw new Error(`Draft ${draftId} status: ${draft.status}`);
  ...
  if (draft.type === "thread" && Array.isArray(draft.content)) {
    ...
    result = await postTweet(
      typeof draft.content === "string" ? draft.content : draft.content[0],
    );
  }
...
function listPendingDrafts() {
  ...
  return fs
    ...
    .map((f) =>
      JSON.parse(fs.readFileSync(path.join(CONFIG.draftsDir, f), "utf-8")),
    )
