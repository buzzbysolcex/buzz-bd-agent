...
- **H6 — delegated-signer / role authz. NEGATE `[INSPECTED]` (closeout confirm). → Ethena CLOSED.** Confirmed the true 2-way handshake from verbatim source: a delegate calls `setDelegatedSigner` (→ `PENDING`), and the **principal (benefactor) must call `confirmDelegatedSigner` themselves** (`require status==PENDING` → sets `ACCEPTED`). The principal's OWN on-chain action is required → an attacker **cannot unilaterally** become a benefactor's signer. MINTER/REDEEMER/GATEKEEPER are admin-granted (OOS trusted). **Ethena directed-audit CLOSED (0 candidates).**
