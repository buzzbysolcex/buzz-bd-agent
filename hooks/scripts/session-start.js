...
function main() {
  ...
  if (
    check("CLAUDE.md version", () => {
      ...
      if (!content.includes("v9.3"))
        throw new Error("Not v9.3 — check CLAUDE.md");
      return "v9.3";
    })
  )
    passed++;
  ...
  total++;
  if (
    check("Context Hygiene section", () => {
      const content = fs.readFileSync(
        path.join(WORKSPACE, "CLAUDE.md"),
        "utf8",
      );
      if (!content.includes("CONTEXT HYGIENE"))
        throw new Error("Missing CONTEXT HYGIENE section");
      return "Present";
    })
  )
    passed++;
  ...
  total++;
  if (
    check("context-optimization.md (rule #18)", () => {
  ...
  if (
    check("feature-flags.js", () => {
      ...
      if (!fs.existsSync(flagPath))
        throw new Error("Feature flags file missing");
  ...
  if (
    check("Buzz Shield", () => {
      try {
        const output = execSync(
          "curl -s http://localhost:3000/api/v1/shield/stats 2>/dev/null",
        ).toString();
        const stats = JSON.parse(output);
        if (stats.patterns_known === 0)
          throw new Error(
            "Shield has 0 patterns — tables may not have initialized",
          );
        if (stats.patterns_known < 20)
          throw new Error(
            `Only ${stats.patterns_known} patterns (expected 20+)`,
          );
        return `${stats.patterns_known} patterns, ${stats.total_scans} scans`;
      } catch (e) {
        if (e.message.includes("patterns")) throw e;
        throw new Error(
          "Shield endpoint not responding — check SHIELD_ENGINE flag",
        );
      }
    })
  )
  ...
  total++;
  if (
    check("AIBTC wallet password", () => {
      const envPath = "/home/claude-code/.env.aibtc";
      if (!fs.existsSync(envPath))
        throw new Error(".env.aibtc missing — wallet unlock will fail");
  ...
  console.log(
    "[SessionStart] 📋 AIBTC: Unlock wallet before filing signals (password in .env.aibtc)",
  );
}
