...
function main() {
  ...
  try {
    ...
    if (fs.existsSync(dbPath)) {
      const tables = execSync(`sqlite3 "${dbPath}" "SELECT COUNT(*) FROM sqlite_master WHERE type='table'" 2>/dev/null`).toString().trim();
      state += `- Tables: ${tables}\n`;

      // Pipeline stats
      try {
        const pipeline = execSync(`sqlite3 "${dbPath}" "SELECT COUNT(*) FROM token_pipeline" 2>/dev/null`).toString().trim();
        state += `- Pipeline tokens: ${pipeline}\n`;
      } catch (e) { /* skip */ }

      // Trust level
      try {
        const trust = execSync(`sqlite3 "${dbPath}" "SELECT current_level FROM trust_state LIMIT 1" 2>/dev/null`).toString().trim();
  ...
  // Git status
  try {
    const gitStatus = execSync(`cd "${WORKSPACE}" && git status --short 2>/dev/null | wc -l`).toString().trim();
    const branch = execSync(`cd "${WORKSPACE}" && git branch --show-current 2>/dev/null`).toString().trim();
